//--------------------------------------------------------------------------
// Copyright 2024 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     GeometricAnalysis_AnalysisObjectCollectionEx.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/TaggedObject.hxx>
#include <NXOpen/libnxopencpp_geometricanalysis_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace GeometricAnalysis
    {
        class AnalysisObjectCollectionEx;
    }
    namespace GeometricAnalysis
    {
        class AnalysisObjectCollection;
    }
    class DisplayableObject;
    namespace GeometricAnalysis
    {
        class DistanceAnalysisBuilder;
    }
    namespace GeometricAnalysis
    {
        class DistanceAnalysisBuilder1;
    }
    namespace GeometricAnalysis
    {
        class PerformancePredictor;
    }
    namespace GeometricAnalysis
    {
        class PerformancePredictorBuilder;
    }
    namespace GeometricAnalysis
    {
        class RadiusAnalysisBuilder;
    }
    namespace GeometricAnalysis
    {
        class RadiusAnalysisBuilder1;
    }
    namespace GeometricAnalysis
    {
        class ReflectionAnalysisBuilder;
    }
    namespace GeometricAnalysis
    {
        class ReflectionAnalysisBuilder1;
    }
    namespace GeometricAnalysis
    {
        class SheetBoundaryAnalysis;
    }
    namespace GeometricAnalysis
    {
        class SheetBoundaryAnalysisBuilder;
    }
    namespace GeometricAnalysis
    {
        class SlopeAnalysisBuilder;
    }
    namespace GeometricAnalysis
    {
        class SlopeAnalysisBuilder1;
    }
    namespace GeometricAnalysis
    {
        class AnalysisObjectCollectionExImpl;
        /** Collection of analysis objects  <br> To obtain an instance of this class, refer to @link NXOpen::GeometricAnalysis::AnalysisObjectCollection  NXOpen::GeometricAnalysis::AnalysisObjectCollection @endlink  <br> 
         <br>  Created in NX11.0.0.  <br>  
        */
        class NXOPENCPP_GEOMETRICANALYSISEXPORT  AnalysisObjectCollectionEx
        {
            private: AnalysisObjectCollectionExImpl * m_analysisobjectcollectionex_impl;
            private: NXOpen::GeometricAnalysis::AnalysisObjectCollection* m_owner;
            /// \cond NX_NO_DOC 
            public: explicit AnalysisObjectCollectionEx(NXOpen::GeometricAnalysis::AnalysisObjectCollection *owner);


            /// \endcond 
            public: 
            /**Returns the tag of this object.  */
            tag_t Tag() const; 
            public: ~AnalysisObjectCollectionEx();
            /** Creates a radius analysis builder.  @return  radius analysis builder 
             <br>  @deprecated Deprecated in NX2312.0.0.  Use @link CreateRadiusAnalysisBuilder1 CreateRadiusAnalysisBuilder1@endlink  instead. <br>  

             <br>  Created in NX11.0.0.  <br>  
             <br> License requirements : gateway ("UG GATEWAY") */
            public: NX_DEPRECATED("Deprecated in NX2312.0.0.  Use CreateRadiusAnalysisBuilder1 instead.") NXOpen::GeometricAnalysis::RadiusAnalysisBuilder * CreateRadiusAnalysisBuilder
            (
                NXOpen::DisplayableObject * radiusAO /** RadiusAO */
            );
            /** Creates a slope analysis builder.  @return  slope analysis builder 
             <br>  @deprecated Deprecated in NX2312.0.0.  Use @link CreateSlopeAnalysisBuilder1 CreateSlopeAnalysisBuilder1@endlink  instead. <br>  

             <br>  Created in NX11.0.0.  <br>  
             <br> License requirements : gateway ("UG GATEWAY") */
            public: NX_DEPRECATED("Deprecated in NX2312.0.0.  Use CreateSlopeAnalysisBuilder1 instead.") NXOpen::GeometricAnalysis::SlopeAnalysisBuilder * CreateSlopeAnalysisBuilder
            (
                NXOpen::DisplayableObject * slopeAO /** SlopeAO */
            );
            /** Creates a distance analysis builder.  @return  distance analysis builder 
             <br>  @deprecated Deprecated in NX2312.0.0.  Use @link CreateDistanceAnalysisBuilder1 CreateDistanceAnalysisBuilder1@endlink  instead. <br>  

             <br>  Created in NX11.0.0.  <br>  
             <br> License requirements : gateway ("UG GATEWAY") */
            public: NX_DEPRECATED("Deprecated in NX2312.0.0.  Use CreateDistanceAnalysisBuilder1 instead.") NXOpen::GeometricAnalysis::DistanceAnalysisBuilder * CreateDistanceAnalysisBuilder
            (
                NXOpen::DisplayableObject * distanceAO /** DistanceAO */
            );
            /** Creates a reflection analysis builder.  @return  reflection analysis builder 
             <br>  @deprecated Deprecated in NX2312.0.0.  Use @link CreateReflectionAnalysisBuilder1 CreateReflectionAnalysisBuilder1@endlink  instead. <br>  

             <br>  Created in NX11.0.0.  <br>  
             <br> License requirements : gateway ("UG GATEWAY") */
            public: NX_DEPRECATED("Deprecated in NX2312.0.0.  Use CreateReflectionAnalysisBuilder1 instead.") NXOpen::GeometricAnalysis::ReflectionAnalysisBuilder * CreateReflectionAnalysisBuilder
            (
                NXOpen::DisplayableObject * reflectionAO /** ReflectionAO */
            );
            /** Creates a @link GeometricAnalysis::SheetBoundaryAnalysisBuilder GeometricAnalysis::SheetBoundaryAnalysisBuilder@endlink    @return  sheet boundary analysis builder 
             <br>  Created in NX1953.0.0.  <br>  
             <br> License requirements : gateway ("UG GATEWAY") */
            public: NXOpen::GeometricAnalysis::SheetBoundaryAnalysisBuilder * CreateSheetBoundaryAnalysisBuilder
            (
                NXOpen::GeometricAnalysis::SheetBoundaryAnalysis * sheetBoundaryAnalysisAO /** @link NXOpen::GeometricAnalysis::SheetBoundaryAnalysis NXOpen::GeometricAnalysis::SheetBoundaryAnalysis@endlink  to be edited */
            );
            /** Creates a @link GeometricAnalysis::PerformancePredictorBuilder GeometricAnalysis::PerformancePredictorBuilder@endlink    @return  @link NXOpen::GeometricAnalysis::PerformancePredictorBuilder NXOpen::GeometricAnalysis::PerformancePredictorBuilder@endlink  with this name. 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : nx_perf_predictor (" Performance Predictor") */
            public: NXOpen::GeometricAnalysis::PerformancePredictorBuilder * CreatePerformancePredictorBuilder
            (
                NXOpen::GeometricAnalysis::PerformancePredictor * perfPredictAO /** @link NXOpen::GeometricAnalysis::PerformancePredictor NXOpen::GeometricAnalysis::PerformancePredictor@endlink  */
            );

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : nx_perf_predictor (" Performance Predictor") */
            public: NXOpen::GeometricAnalysis::PerformancePredictor * CreatePerformancePredictorFromJsonFile
            (
                const NXString & fileNameAndPath /** filenameandpath */ 
            );
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : nx_perf_predictor (" Performance Predictor") */
            NXOpen::GeometricAnalysis::PerformancePredictor * CreatePerformancePredictorFromJsonFile
            (
                const char * fileNameAndPath /** filenameandpath */ 
            );

            /// \endcond 
            /** Creates a distance analysis builder.  @return  distance analysis builder 
             <br>  Created in NX2312.0.0.  <br>  
             <br> License requirements : gateway ("UG GATEWAY") */
            public: NXOpen::GeometricAnalysis::DistanceAnalysisBuilder1 * CreateDistanceAnalysisBuilder1
            (
                NXOpen::DisplayableObject * distanceAO /** DistanceAO */
            );
            /** Creates a slope analysis builder.  @return  slope analysis builder 
             <br>  Created in NX2312.0.0.  <br>  
             <br> License requirements : gateway ("UG GATEWAY") */
            public: NXOpen::GeometricAnalysis::SlopeAnalysisBuilder1 * CreateSlopeAnalysisBuilder1
            (
                NXOpen::DisplayableObject * slopeAO /** SlopeAO */
            );
            /** Creates a reflection analysis builder.  @return  reflection analysis builder 
             <br>  Created in NX2312.0.0.  <br>  
             <br> License requirements : gateway ("UG GATEWAY") */
            public: NXOpen::GeometricAnalysis::ReflectionAnalysisBuilder1 * CreateReflectionAnalysisBuilder1
            (
                NXOpen::DisplayableObject * reflectionAO /** ReflectionAO */
            );
            /** Creates a radius analysis builder.  @return  radius analysis builder 
             <br>  Created in NX2312.0.0.  <br>  
             <br> License requirements : gateway ("UG GATEWAY") */
            public: NXOpen::GeometricAnalysis::RadiusAnalysisBuilder1 * CreateRadiusAnalysisBuilder1
            (
                NXOpen::DisplayableObject * radiusAO /** RadiusAO */
            );
        }; //lint !e1712 default constructor not defined for class  

    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
