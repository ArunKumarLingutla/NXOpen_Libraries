//--------------------------------------------------------------------------
// Copyright 2024 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     CAM_RotateToolMoveBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/CAM_ExpressionDouble.hxx>
#include <NXOpen/CAM_MoveBuilder.hxx>
#include <NXOpen/libnxopencpp_cam_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace CAM
    {
        class RotateToolMoveBuilder;
    }
    namespace CAM
    {
        class ExpressionDouble;
    }
    namespace CAM
    {
        class MoveBuilder;
    }
    namespace CAM
    {
        class _RotateToolMoveBuilderBuilder;
        class RotateToolMoveBuilderImpl;
        /** Represents the Rotate Tool Move Builder  <br> To create a new instance of this class, use @link NXOpen::CAM::MoveCollection::CreateRotateToolMoveBuilder  NXOpen::CAM::MoveCollection::CreateRotateToolMoveBuilder @endlink  <br> 
         <br>  Created in NX5.0.0.  <br>  
        */
        class NXOPENCPP_CAMEXPORT  RotateToolMoveBuilder : public NXOpen::CAM::MoveBuilder
        {
            private: RotateToolMoveBuilderImpl * m_rotatetoolmovebuilder_impl;
            private: friend class  _RotateToolMoveBuilderBuilder;
            protected: RotateToolMoveBuilder();
            public: ~RotateToolMoveBuilder();
            /**Returns  the Orient Angle 
             <br>  @deprecated Deprecated in NX1899.0.0.  Use @link CAM::RotateToolMoveBuilder::AngleBuilder CAM::RotateToolMoveBuilder::AngleBuilder@endlink  instead. <br>  

             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NX_DEPRECATED("Deprecated in NX1899.0.0.  Use CAM::RotateToolMoveBuilder::AngleBuilder instead.") double Angle
            (
            );
            /**Sets  the Orient Angle 
             <br>  @deprecated Deprecated in NX1899.0.0.  Use @link CAM::RotateToolMoveBuilder::AngleBuilder CAM::RotateToolMoveBuilder::AngleBuilder@endlink  instead. <br>  

             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NX_DEPRECATED("Deprecated in NX1899.0.0.  Use CAM::RotateToolMoveBuilder::AngleBuilder instead.") void SetAngle
            (
                double angle /** angle */ 
            );
            /**Returns  the X offset Toggle Status 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: bool XoffsetToggle
            (
            );
            /**Sets  the X offset Toggle Status 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetXoffsetToggle
            (
                bool xoffstat /** xoffstat */ 
            );
            /**Returns  the X offset 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: double Xoffset
            (
            );
            /**Sets  the X offset 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetXoffset
            (
                double xoff /** xoff */ 
            );
            /**Returns  the Y offset Toggle Status 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: bool YoffsetToggle
            (
            );
            /**Sets  the Y offset Toggle Status 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetYoffsetToggle
            (
                bool yoffstat /** yoffstat */ 
            );
            /**Returns  the Y offset 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: double Yoffset
            (
            );
            /**Sets  the Y offset 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetYoffset
            (
                double yoff /** yoff */ 
            );
            /**Returns  the Z offset Toggle Status 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: bool ZoffsetToggle
            (
            );
            /**Sets  the Z offset Toggle Status 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetZoffsetToggle
            (
                bool zoffstat /** zoffstat */ 
            );
            /**Returns  the Z offset 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: double Zoffset
            (
            );
            /**Sets  the Z offset 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetZoffset
            (
                double zoff /** zoff */ 
            );
            /**Returns  the Adjust register Toggle Status 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: bool AdjregToggle
            (
            );
            /**Sets  the Adjust register Toggle Status 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetAdjregToggle
            (
                bool adjregstat /** adjregstat */ 
            );
            /**Returns  the Adjust register 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: int Adjreg
            (
            );
            /**Sets  the Adjust register 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetAdjreg
            (
                int adjreg /** adjreg */ 
            );
            /**Returns  the orient angle builder 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::ExpressionDouble * AngleBuilder
            (
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
