//--------------------------------------------------------------------------
// Copyright 2024 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     ModlTemplateTestManager.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Features_Feature.hxx>
#include <NXOpen/TaggedObject.hxx>
#include <NXOpen/libnxopendebugsessioncpp_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    class ModlTemplateTestManager;
    class TemplateTestManager;
    class Axis;
    namespace Features
    {
        class Feature;
    }
    class NXObject;
    class Plane;
    class Point;
    class ModlTemplateTestManagerImpl;

    /// \cond NX_NO_DOC 
    /** Represents a class that is used for autotests.  This class should not
    be made available to customers  <br> To obtain an instance of this class, refer to @link NXOpen::TemplateTestManager  NXOpen::TemplateTestManager @endlink  <br> 
     <br>  Created in NX8.0.0.  <br>  
    */
    class NXOPENDEBUGSESSIONCPPEXPORT  ModlTemplateTestManager
    {
        private: ModlTemplateTestManagerImpl * m_modltemplatetestmanager_impl;
        private: NXOpen::TemplateTestManager* m_owner;
        /// \cond NX_NO_DOC 
        public: explicit ModlTemplateTestManager(NXOpen::TemplateTestManager *owner);


        /// \endcond 
        public: 
        /**Returns the tag of this object.  */
        tag_t Tag() const; 
        public: ~ModlTemplateTestManager();

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        public: void TouchUpdateTemplateTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        void TouchUpdateTemplateTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        public: void DeleteTemplateTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        void DeleteTemplateTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX11.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        public: void RenewTemplateTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX11.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        void RenewTemplateTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        public: void SuppressTemplateTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        void SuppressTemplateTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        public: void CopyPasteTemplateTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.0.0.  <br>  
         <br> License requirements : solid_modeling ("SOLIDS MODELING") */
        void CopyPasteTemplateTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & features /** features */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.5.0.  <br>  
         <br> License requirements : None */
        public: void RunPatternTemplateTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & patternfeature0 /** patternfeature0 */ ,
            NXOpen::Axis * axis /** axis */ ,
            NXOpen::Point * point /** point */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.5.0.  <br>  
         <br> License requirements : None */
        void RunPatternTemplateTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & patternfeature0 /** patternfeature0 */ ,
            NXOpen::Axis * axis /** axis */ ,
            NXOpen::Point * point /** point */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.5.0.  <br>  
         <br> License requirements : None */
        public: void RunMirrorTemplateTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & mirrorfeature0 /** mirrorfeature0 */ ,
            NXOpen::Plane * plane /** plane */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX8.5.0.  <br>  
         <br> License requirements : None */
        void RunMirrorTemplateTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::Features::Feature *> & mirrorfeature0 /** mirrorfeature0 */ ,
            NXOpen::Plane * plane /** plane */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX12.0.0.  <br>  
         <br> License requirements : None */
        public: void RunChecksumTemplateTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            NXOpen::NXObject * part /** part */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX12.0.0.  <br>  
         <br> License requirements : None */
        void RunChecksumTemplateTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            NXOpen::NXObject * part /** part */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX1926.0.0.  <br>  
         <br> License requirements : None */
        public: void RunSketchPreviewDimensionTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::NXObject *> & sketchcurves1 /** sketchcurves1 */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX1926.0.0.  <br>  
         <br> License requirements : None */
        void RunSketchPreviewDimensionTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ ,
            const std::vector<NXOpen::NXObject *> & sketchcurves1 /** sketchcurves1 */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX1953.0.0.  <br>  
         <br> License requirements : None */
        public: void RunTopOptROVETemplateTest
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX1953.0.0.  <br>  
         <br> License requirements : None */
        void RunTopOptROVETemplateTest
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const std::vector<int> & errorCodes /** errorcodes */ 
        );

        /// \endcond 

        /// \cond NX_NO_DOC 
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX2406.0.0.  <br>  
         <br> License requirements : None */
        public: void RunInterpartNavigatorAction
        (
            const NXString & testName /** testname */ ,
            int version /** version */ ,
            const NXString & actionName /** actionname */ ,
            const std::vector<NXOpen::NXObject *> & selectedObjectsInDiagram /** selectedobjectsindiagram */ ,
            const std::vector<NXOpen::NXObject *> & selectedObjectsInDetails /** selectedobjectsindetails */ ,
            std::vector<NXString> & enabledFilters /** enabledfilters */ ,
            std::vector<NXString> & enabledPreferences /** enabledpreferences */ ,
            bool printXMLData /** printxmldata */ 
        );
        /** This is an internal API and can be changed at any time 
         <br>  Created in NX2406.0.0.  <br>  
         <br> License requirements : None */
        void RunInterpartNavigatorAction
        (
            const char * testName /** testname */ ,
            int version /** version */ ,
            const char * actionName /** actionname */ ,
            const std::vector<NXOpen::NXObject *> & selectedObjectsInDiagram /** selectedobjectsindiagram */ ,
            const std::vector<NXOpen::NXObject *> & selectedObjectsInDetails /** selectedobjectsindetails */ ,
            std::vector<NXString> & enabledFilters /** enabledfilters */ ,
            std::vector<NXString> & enabledPreferences /** enabledpreferences */ ,
            bool printXMLData /** printxmldata */ 
        );

        /// \endcond 
    }; //lint !e1712 default constructor not defined for class  


    /// \endcond 
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
