...
static int do_Batch_Meshing__main(UF_UG_BatchMesh_CommandLineOpts_t *pzBatchRunParams)
{
    ...
    try
    {
        ...
        //  <Lavarini>  10-Apr-2023  CID#428569
        //  Compare size of scSolverName with buffer size to insure there are no overwrites
        if ( strlen ( scSolverName.GetUTF8Text ( ) ) < sizeof ( pzBatchRunParams->Solver_Name ) )
        {
            strcpy ( pzBatchRunParams->Solver_Name, scSolverName.GetUTF8Text ( ) );    // NOLINT(qaz-nx-security-fixed-size-strcpy)
        }
        else
        {
            report_msg ( "pzBatchRunParams->Solver_Name buffer is too small to hold scSolverName.GetUTF8Text ( ).\n" );
            write_info_fprintf ( true, UF_UG__BatchMesh_LOG, "ERROR: pzBatchRunParams->Solver_Name buffer is too small to hold scSolverName.GetUTF8Text ( ).\n" );
            return 9999;
        }
    ...
    else if (iNameOpt == 5 || iNameOpt == 6)
    {
        if (iNameOpt == 6)
        {
            ...
            else
            {
                ...
                strcat(scDeckExtn, ".");                                // NOLINT(qaz-nx-security-fixed-size-strcpy)
                strcat(scDeckExtn, pzBatchRunParams->dat_file_extn);    // NOLINT(qaz-nx-security-fixed-size-strcpy)
            }
        }
        else
            strcpy(scDeckExtn, "");
...
static void upper_to_lower_case(char *scString)
{
    ...
    for (i = 0; i<nchar; i++)
    {
        ichar = (unsigned char)scString[i];
