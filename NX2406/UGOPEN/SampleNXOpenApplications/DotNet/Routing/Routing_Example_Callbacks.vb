...
Module Callbacks
    ...
    Sub RegisterCallbacks()
        ...
        customManager.AddCallback(CallbackReason.StockComponentCreated, AddressOf StockComponentCreatedCallback)

    End Sub

    '-----------------------------------------------------------------------------------------------
    Sub AutorouteCallback(ByVal nxObjects As NXObject())
        Echo(vbCrLf + "*** VB AutorouteCallback called with " + nxObjects.Length.ToString() + " objects.")
    ...
    Sub UnrouteCallback(ByVal nxObjects As NXObject())
        Echo(vbCrLf + "*** VB UnrouteCallback called with " + nxObjects.Length.ToString() + " objects.")
    ...
    '-----------------------------------------------------------------------------------------------
    Sub StockComponentCreatedCallback(ByVal nxObjects As NXObject())
        Echo(vbCrLf + "*** VB StockComponentCreatedCallback called with " + nxObjects.Length.ToString() + " objects.")

        For Each nxObject As NXObject In nxObjects
            If Not TypeOf nxObject Is Assemblies.Component Then
                Echo("  NX Object (Tag " + nxObject.Tag.ToString() + ") is NOT an Assemblies Component.")
                Continue For
            End If

            Dim component As Assemblies.Component = CType(nxObject, Assemblies.Component)

            Echo("  Component (Tag " + component.Tag.ToString() + ") named " + component.DisplayName + ".")
        Next
