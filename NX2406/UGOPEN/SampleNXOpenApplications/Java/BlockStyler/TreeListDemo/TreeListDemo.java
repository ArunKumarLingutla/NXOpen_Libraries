...

{
    ...
    //------------------------------------------------------------------------------
    public nxopen.blockstyler.BlockDialog.DialogResponse launch() throws NXException, RemoteException
    {
    ...
    void MoveChildNodesOfDraggedNode(Tree tree, Node[] node, nxopen.blockstyler.Node CopyNode)throws NXException, RemoteException
    {
        try
        {
            ...
            if (childNode != null)
            {
                ...
                while (childNode != null)
                {
                    ...
                    childNodeList.add(copychildNode);
                ...
                for (Node newNode : childNodeList)
                {
                    //If there is no previous node of the childNode, should be inserted first
                    if (PreviousChildNode == null)
                    {
                        tree.insertNode(newNode, CopyNode, null, Tree.NodeInsertOption.FIRST);
                    }
                    else
                    {
    ...
    public boolean onDropCallback(Tree tree, Node[] node, int columnID, Node targetNode,int targetColumnID, nxopen.blockstyler.Node.DropType dropType, int dropMenuItemId) throws NXException,RemoteException
    {
        try
        {
            ...
            if (dropType == Node.DropType.BEFORE || dropType == Node.DropType.ON)
            { 
                //Get the previous node of the target node
                Node PreviousNode = targetNode.previousSiblingNode();
            ...
            else if (dropType == Node.DropType.AFTER || dropType == Node.DropType.BEFORE_AND_AFTER)
            {
