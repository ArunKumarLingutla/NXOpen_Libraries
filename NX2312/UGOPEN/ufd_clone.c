...
/*   switch name                min   handler function          multi seen
    ...
    {"default_type",              9,  default_type,             false, false,
     "=<item_type> specifies default Teamcenter item type. (Not supported in Managed Lite mode)\n"},
    ...
    {"default_project",           9, assign_default_project,    false, false,
     "=yes|no specifies whether the items to be cloned assigned under \nthe default project mentioned in the customer defaults or not. (Not supported in Managed Lite mode)\n"},
...
static logical isMixedNameRule = false;
/*================================================================================
...
static void handle_mixed_name_rule(int argc, char** argv)
{
    logical isNameRuleProvided = false;
    ...
    for (int iargIndx = 1; iargIndx < argc; iargIndx++)
    {
        ...
        if (strcmp(descriptor->switch_name, "name_rule") == 0)
        {
            ...
            char* conversionRuleString = (char*)(find_value(argv[iargIndx]));
            if (conversionRuleString != NULL && conversionRuleString[0] != '\0')
            {
                char* nextToken;
                char* token = strtok(conversionRuleString, ":");
                if (token != NULL && token[0] != '\0')
                {
                    char* nameRuleValues[3] = { NULL };
                    nameRuleValues[0] = token;
                    ...
                    while (token != NULL) {
                        ...
                        token = strtok(NULL, ":");
                        ++count;
                        nameRuleValues[count] = token;
                    }

                    if (is_prefix(nameRuleValues[0], "prepend", 0) && (count == 2))
                    {
                        rule_def.mixed_rule_prefix_string = nameRuleValues[1];
                    }
                    else if (is_prefix(nameRuleValues[0], "append", 0) && (count == 2))
                    {
                        rule_def.mixed_rule_suffix_string = nameRuleValues[1];
                    }
                    else if (is_prefix(nameRuleValues[0], "replace", 3) && (count == 3))
                    {
                        rule_def.base_string = nameRuleValues[1];
                        rule_def.new_string = nameRuleValues[2];
                    }
                }
            }
        }
    ...
    {
        log_and_stderr_printf("\nError: Atleast one -n[ame_rule] switch is required with mixed_name_rule.");
        quit(EXIT_FAILURE);
    }
}
