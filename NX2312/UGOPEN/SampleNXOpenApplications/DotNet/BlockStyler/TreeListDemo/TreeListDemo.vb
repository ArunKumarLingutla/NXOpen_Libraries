...
#End Region
    ...
    Public Function MoveChildNodesOfDraggedNode(ByVal tree As NXOpen.BlockStyler.Tree, ByVal node() As NXOpen.BlockStyler.Node, ByVal copyNode As NXOpen.BlockStyler.Node)
        'child of node to be moved
        Dim childNode As Node = node(0).FirstChildNode
        'if there exist child nodes then move all along with its parent
        If Not childNode Is Nothing Then
            Dim childNodeList As List(Of Node) = New List(Of Node)()
            'Get the previous node of the child node
            While Not childNode Is Nothing
                '---------adding the child nodes to the list------
                Dim childCopy As Node = tree.CopyNode(childNode)
                childNodeList.Add(childCopy)
                childNode = childNode.NextSiblingNode
            End While
            childNode = node(0).FirstChildNode
            '--------if there exist child nodes, move them to copyNode--------------
            Dim previousChildNode As Node = childNode.PreviousSiblingNode
            Dim NewNode As Node
            For Each NewNode In childNodeList
                '---------If there is no previous node of the childNode, should be inserted first--------
                If previousChildNode Is Nothing Then
                    tree.InsertNode(NewNode, copyNode, Nothing, tree.NodeInsertOption.First)
                Else
                    tree.InsertNode(NewNode, copyNode, previousChildNode, tree.NodeInsertOption.Last)
    ...
    Public Function OnDropCallback(ByVal tree As NXOpen.BlockStyler.Tree, ByVal node() As NXOpen.BlockStyler.Node, ByVal columnID As Integer, ByVal targetNode As NXOpen.BlockStyler.Node, ByVal targetColumnID As Integer, ByVal dropType As NXOpen.BlockStyler.Node.DropType, ByVal dropMenuItemId As Integer) As Boolean
        Try
            ...
            '----Node to be moved---------
            Dim copyNode As Node = tree.CopyNode(node(0))

            'Get the parent of the target node
            Dim parentNode As Node = targetNode.ParentNode
            ...
            If dropType = NXOpen.BlockStyler.Node.DropType.Before Or dropType = NXOpen.BlockStyler.Node.DropType.On Then

                'Get the previous node of the target node*/
                Dim previousNode As Node = targetNode.PreviousSiblingNode

                '--------If there is no previous node, it should be inserted first-------------
                If previousNode Is Nothing Then
                    tree.InsertNode(copyNode, parentNode, previousNode, tree.NodeInsertOption.First)
                Else
                    tree.InsertNode(copyNode, parentNode, previousNode, tree.NodeInsertOption.Last)
                End If
                MoveChildNodesOfDraggedNode(tree, node, copyNode)
                tree.DeleteNode(node(0))

            ElseIf dropType = NXOpen.BlockStyler.Node.DropType.After Or dropType = NXOpen.BlockStyler.Node.DropType.BeforeAndAfter Then
                '-----------Inserting the node after the target node--------------------
                tree.InsertNode(copyNode, parentNode, targetNode, tree.NodeInsertOption.Last)
                MoveChildNodesOfDraggedNode(tree, node, copyNode)
                tree.DeleteNode(node(0))
