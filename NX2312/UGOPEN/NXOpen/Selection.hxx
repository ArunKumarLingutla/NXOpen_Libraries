//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Selection.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Select_FilterMember.hxx>
#include <NXOpen/SelectionHandle.hxx>
#include <NXOpen/SelectionSubscriber.hxx>
#include <NXOpen/TaggedObject.hxx>
#include <NXOpen/UserDefinedObjects_UserDefinedClass.hxx>
#include <NXOpen/ugmath.hxx>
#include <NXOpen/libnxopenuicpp_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    class Selection;
    class UI;
    namespace Features
    {
        class Feature;
    }
    class NXObject;
    class SelectionHandle;
    class SelectionSubscriber;
    class TaggedObject;
    namespace UserDefinedObjects
    {
        class UserDefinedClass;
    }
    class View;
    class SelectionImpl;
    /** Defines an interface object for executing selection operations  <br> To obtain an instance of this class, refer to @link NXOpen::UI  NXOpen::UI @endlink  <br> 
     <br>  Created in NX3.0.0.  <br>  
    */
    class NXOPENUICPPEXPORT  Selection
    {
        /**
                Indicates the scope for selecting objects.
            */
        public: enum SelectionScope
        {
            SelectionScopeUseDefault/** Use the default selection scope */,
            SelectionScopeWorkPart/** Use the work part */,
            SelectionScopeAnyInAssembly/** Use any occurrence in the assembly */ = 3,
            SelectionScopeWorkPartAndOccurrence/** Use the work part or any occurrence */,
            SelectionScopeWorkPartAndWorkPartOccurrence/** Use the work part or any occurrence of the work part  */
        };

        /**
                Indicates how the specified filter types or members will be applied to the current or default selection filter (all standard types).
            */
        public: enum SelectionAction
        {
            SelectionActionEnableAll/** Populate the filter with all standard types */,
            SelectionActionEnableSpecific/** Populate the filter with all current or standard types and the specified types or filter members */,
            SelectionActionDisableSpecific/** Populate the filter with all current or standard types except the specified types or filter members */,
            SelectionActionClearAndEnableSpecific/** Populate the filter with just the specified types or filter members */,
            SelectionActionAllAndDisableSpecific/** Populate the filter with all standard types except the specified types or filter members */
        };

        /**
                Indicates the general class of object to filter selection.
            */
        public: enum SelectionType
        {
            SelectionTypeAll/** Filter to select all object types */,
            SelectionTypeFeatures/** Filter to select all feature types */,
            SelectionTypeCurves/** Filter to select all curve types */,
            SelectionTypeFaces/** Filter to select all face types */,
            SelectionTypeEdges/** Filter to select all edge types */,
            SelectionTypeCurvesAndEdges/** Filter to select all curve and edge types */
        };

        /**
                Specify the user response for the selection operation.
            */
        public: enum Response
        {
            ResponseBack/** User selected Back */ = 1,
            ResponseCancel/** User selected Cancel */,
            ResponseOk/** User selected Ok */,
            ResponseObjectSelectedByName/** User selected object by name */,
            ResponseObjectSelected/** User selected an object */
        };

        /**
                Specify the user response for general dialogs.
            */
        public: enum DialogResponse
        {
            DialogResponseNone/** No Response */,
            DialogResponsePick/** User picked object on screen */,
            DialogResponseOk/** User selected Ok */,
            DialogResponseCancel/** User selected Cancel */,
            DialogResponseBack/** User selected Back (if applicable) */,
            DialogResponseApply/** User selected Apply */,
            DialogResponseHelp/** User selected Help */
        };

        /** 
                Specify the type of features to select.
            */
        public: enum SelectionFeatureType
        {
            SelectionFeatureTypeBrowsable/** Select all browsable features */,
            SelectionFeatureTypeNoBooleanUdf/** Select all features except boolean and udf features */
        };

        /**
                Indicates the general class of object to filter selection.
            */
        public: enum UistylerSelectionType
        {
            UistylerSelectionTypeInactiveSelection/** sets selection inactive until this function is called again */,
            UistylerSelectionTypeSingleSelection/** allows single select */,
            UistylerSelectionTypeSingleDeselection/** single deselect */,
            UistylerSelectionTypeRobustSelection/** reselect last */,
            UistylerSelectionTypeSinglePosition/** the position is returned if no object is selected with the
            single select gesture  */,
            UistylerSelectionTypeRectanglePosition/** the position is returned if no object is selected with the
            single select gesture */
        };

         /**  Used in selection methods that take filter triples to set the types of objects that are selectable.

            Commonly used:
             @code 
        For C++:
        in order to select...       set...
        any edge                    type=UF_solid_type, subtype=UF_all_subtype, solid_body_subtype=UF_UI_SEL_FEATURE_ANY_EDGE
        any face                    type=UF_solid_type, subtype=UF_all_subtype, solid_body_subtype=UF_UI_SEL_FEATURE_ANY_FACE

        For .NET:
        in order to select...       set...
        any edge                    type=NXOpen.UF.UFConstants.UF_solid_type, subtype=0, solid_body_subtype=NXOpen.UF.UFConstants.UF_UI_SEL_FEATURE_ANY_EDGE
        any face                    type=NXOpen.UF.UFConstants.UF_solid_type, subtype=0, solid_body_subtype=NXOpen.UF.UFConstants.UF_UI_SEL_FEATURE_ANY_FACE
             @endcode 

            */
        public:
        struct MaskTriple
        {
            public: /** Object type. This can be one of the object types that are listed in 
                                  uf_object_types.h. For example, for point, 
                                  use UF_point_type in C++ and
                                  NXOpen.UF.UFConstants.UF_point_type in .NET. */int Type;
            public: /** Object subtype. This can either be 0 (UF_all_subtype) for any subtype, or a
                                    subtype of the selected type.
                                    The subtypes are listed in uf_object_types.h.
                                    */int Subtype;
            public: /** Solid body subtype. This is only meaningful when the type is 
                                   UF_solid_type.  In that case, this should be set to
                                   one of the solid type constants listed in uf_ui_types.h 
                                   under "Constants for selection solid_type". For example,
                                   to select any face, use UF_UI_SEL_FEATURE_ANY_FACE in C++ and 
                                   NXOpen.UF.UFConstants.UF_UI_SEL_FEATURE_ANY_FACE in .NET */int SolidBodySubtype;
            public: MaskTriple() :
                Type(),
                Subtype(),
                SolidBodySubtype()
            {
            }
            /** Constructor for the MaskTriple struct. */ 
            public: MaskTriple(int typeInitial /** Object type. This can be one of the object types that are listed in 
                                          uf_object_types.h. For example, for point, 
                                          use UF_point_type in C++ and
                                          NXOpen.UF.UFConstants.UF_point_type in .NET. */, 
                    int subtypeInitial /** Object subtype. This can either be 0 (UF_all_subtype) for any subtype, or a
                                            subtype of the selected type.
                                            The subtypes are listed in uf_object_types.h.
                                            */, 
                    int solidBodySubtypeInitial /** Solid body subtype. This is only meaningful when the type is 
                                           UF_solid_type.  In that case, this should be set to
                                           one of the solid type constants listed in uf_ui_types.h 
                                           under "Constants for selection solid_type". For example,
                                           to select any face, use UF_UI_SEL_FEATURE_ANY_FACE in C++ and 
                                           NXOpen.UF.UFConstants.UF_UI_SEL_FEATURE_ANY_FACE in .NET */) :
                Type(typeInitial),
                Subtype(subtypeInitial),
                SolidBodySubtype(solidBodySubtypeInitial)
            {
            }
        };

         /** For a rectangle deselection, the fields <b> Deselection </b>, 
            <b> MultipleSelection </b>, <b> Rectangle </b> 
            bits would be set. 
            For a name selection which selected one object, the fields <b> SelectionDescriptor.Selection </b>, 
            <b> SingleSelection </b>, and <b> NameSelection </b>
            bits would be set.
            If a reselect was done (an object is selected and the previous object selected is deselected), 
            only the field <b> SelectionDescriptor.Reselection </b> bit is set.
            The fields <b> SingleSelection </b> and <b> MultipleSelection </b> 
            bits are not set for position.
            The field <b> Rectangle </b> bit is set for rectangle selection, 
            rectangle deselection, and rectangle position.
            The function to query the descriptor can only be used with UIStyler dialogs and can be called from either the selection
            filter procedure or the selection callback.
            */
        public:
        struct SelectionDescriptor
        {
            public: /** selection */ bool Selection;
            public: /** deselection */ bool Deselection;
            public: /** reselection */ bool Reselection;
            public: /** single selection */ bool SingleSelection;
            public: /** multiple selection */ bool MultipleSelection;
            public: /** single position */ bool SinglePosition;
            public: /** rectangle position */ bool RectanglePosition;
            public: /** name selection */ bool NameSelection;
            public: /** rectangle */ bool Rectangle;
            public: SelectionDescriptor() :
                Selection(),
                Deselection(),
                Reselection(),
                SingleSelection(),
                MultipleSelection(),
                SinglePosition(),
                RectanglePosition(),
                NameSelection(),
                Rectangle()
            {
            }
            /** Constructor for the SelectionDescriptor struct. */ 
            public: SelectionDescriptor(bool selectionInitial /** selection */ , 
                    bool deselectionInitial /** deselection */ , 
                    bool reselectionInitial /** reselection */ , 
                    bool singleSelectionInitial /** single selection */ , 
                    bool multipleSelectionInitial /** multiple selection */ , 
                    bool singlePositionInitial /** single position */ , 
                    bool rectanglePositionInitial /** rectangle position */ , 
                    bool nameSelectionInitial /** name selection */ , 
                    bool rectangleInitial /** rectangle */ ) :
                Selection(selectionInitial),
                Deselection(deselectionInitial),
                Reselection(reselectionInitial),
                SingleSelection(singleSelectionInitial),
                MultipleSelection(multipleSelectionInitial),
                SinglePosition(singlePositionInitial),
                RectanglePosition(rectanglePositionInitial),
                NameSelection(nameSelectionInitial),
                Rectangle(rectangleInitial)
            {
            }
        };

        public: /** A callback which the user can implement to accept or reject selection candidates when selecting tagged objects. 
                Return 1 to accept the object, 0 to reject it. 
         <br>  Created in NX1980.0.0.  <br>  
         <br> License requirements : None */
        typedef NXOpen::Callback1<int, NXOpen::TaggedObject *> FilterTaggedObject;
        public: /** A callback which the user can provide to a UIStyler dialog to accept or reject selection candidates when selecting tagged objects. 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        typedef NXOpen::Callback3<int, NXOpen::TaggedObject *, NXOpen::Selection::MaskTriple *, NXOpen::SelectionHandle *> FilterTaggedObjectCallback;
        public: /** It is an optional callback which the user can provide to perform
                application specific filtering when selecting objects. 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link NXOpen::Selection::FilterTaggedObjectCallback NXOpen::Selection::FilterTaggedObjectCallback@endlink  instead <br>  

         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        typedef NXOpen::Callback3<int, NXOpen::NXObject *, NXOpen::Selection::MaskTriple *, NXOpen::SelectionHandle *> FilterCallback;
        public: /** A callback which the user can provide to a UIStyler dialog to react to the selection of tagged objects. 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        typedef NXOpen::Callback3<int, std::vector<NXOpen::TaggedObject *>, std::vector<NXOpen::TaggedObject *>, NXOpen::SelectionHandle *> TaggedObjectSelectionCallback;
        public: /** It is an optional callback which the user can provide to customize his selection 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link NXOpen::Selection::TaggedObjectSelectionCallback NXOpen::Selection::TaggedObjectSelectionCallback@endlink  instead <br>  

         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        typedef NXOpen::Callback3<int, std::vector<NXOpen::NXObject *>, std::vector<NXOpen::NXObject *>, NXOpen::SelectionHandle *> SelectionCallback;
        private: SelectionImpl * m_selection_impl;
        private: NXOpen::UI* m_owner;
        /// \cond NX_NO_DOC 
        public: explicit Selection(NXOpen::UI *owner);


        /// \endcond 
        public: 
        /**Returns the tag of this object.  */
        tag_t Tag() const; 
        public: ~Selection();
        /** Launch the Class Selection dialog to select tagged objects.

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
               
              @return  Ok, Back, or Cancel 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObjects
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Launch the Class Selection dialog to select tagged objects.

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
               
              @return  Ok, Back, or Cancel 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObjects
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Launch the Single Select dialog to select a tagged object.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObject
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Launch the Single Select dialog to select a tagged object.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObject
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Launch the Class Selection dialog to select tagged objects using a filter defined by Filter Members.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX1980.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObjectsWithFilterMembers
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & filterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Launch the Class Selection dialog to select tagged objects using a filter defined by Filter Members.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX1980.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObjectsWithFilterMembers
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & filterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Launch the Class Selection dialog to select tagged objects using a filter defined by Filter Members and a filter callback.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX1980.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObjectsWithFilterCallback
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & filterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            const NXOpen::Selection::FilterTaggedObject&  filterCallback /** Filter callback for additional user specific filtering. */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Launch the Class Selection dialog to select tagged objects using a filter defined by Filter Members and a filter callback.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX1980.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObjectsWithFilterCallback
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & filterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            const NXOpen::Selection::FilterTaggedObject&  filterCallback /** Filter callback for additional user specific filtering. */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Launch the Class Selection dialog to select tagged objects with full filtering capabilities.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX2007.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObjectsWithFullFiltering
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & supportedFilterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            const std::vector<NXOpen::Select::FilterMember> & initialFilterMembers /** Defines which filter members are enabled when the dialog is launched. 
                                                                                                                            If empty, all supported types will be enabled. */,
            const NXOpen::Selection::FilterTaggedObject&  filterCallback /** Filter callback for additional user specific filtering. */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Launch the Class Selection dialog to select tagged objects with full filtering capabilities.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX2007.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObjectsWithFullFiltering
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & supportedFilterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            const std::vector<NXOpen::Select::FilterMember> & initialFilterMembers /** Defines which filter members are enabled when the dialog is launched. 
                                                                                                                            If empty, all supported types will be enabled. */,
            const NXOpen::Selection::FilterTaggedObject&  filterCallback /** Filter callback for additional user specific filtering. */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Launch the Single Select dialog to select a tagged object using a filter defined by Filter Members.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX1980.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObjectWithFilterMembers
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & filterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Launch the Single Select dialog to select a tagged object using a filter defined by Filter Members.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX1980.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObjectWithFilterMembers
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & filterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Launch the Single Select dialog to select a tagged object using a filter defined by Filter Members and a filter callback.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX1980.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObjectWithFilterCallback
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & filterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            const NXOpen::Selection::FilterTaggedObject&  filterCallback /** Filter callback for additional user specific filtering. */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Launch the Single Select dialog to select a tagged object using a filter defined by Filter Members and a filter callback.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX1980.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObjectWithFilterCallback
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & filterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            const NXOpen::Selection::FilterTaggedObject&  filterCallback /** Filter callback for additional user specific filtering. */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Launch the Single Select dialog to select a tagged object with full filtering capabilities.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX2007.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObjectWithFullFiltering
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & supportedFilterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            const std::vector<NXOpen::Select::FilterMember> & initialFilterMembers /** Defines which filter members are enabled when the dialog is launched. 
                                                                                                                            If empty, all supported types will be enabled. */,
            const NXOpen::Selection::FilterTaggedObject&  filterCallback /** Filter callback for additional user specific filtering. */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Launch the Single Select dialog to select a tagged object with full filtering capabilities.
            
            This method takes the desired Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX2007.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObjectWithFullFiltering
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the specified filter members will modify the default selection filter */,
            const std::vector<NXOpen::Select::FilterMember> & supportedFilterMembers /** Modifies the selection filter as dictated by the Selection Action parameter. */,
            const std::vector<NXOpen::Select::FilterMember> & initialFilterMembers /** Defines which filter members are enabled when the dialog is launched. 
                                                                                                                            If empty, all supported types will be enabled. */,
            const NXOpen::Selection::FilterTaggedObject&  filterCallback /** Filter callback for additional user specific filtering. */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select objects.

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
               
              @return  Ok, Back, or Cancel 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectsWithFilterMembers Selection::SelectTaggedObjectsWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::SelectTaggedObjectsWithFilterMembers instead") NXOpen::Selection::Response SelectObjects
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            std::vector<NXOpen::NXObject *> & objectArray /** The selected objects */
        );
        /** Select objects.

            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
               
              @return  Ok, Back, or Cancel 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectsWithFilterMembers Selection::SelectTaggedObjectsWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectObjects
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            std::vector<NXOpen::NXObject *> & objectArray /** The selected objects */
        );
        /** Select objects using filter defined by mask triples.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectsWithFilterMembers Selection::SelectTaggedObjectsWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::SelectTaggedObjectsWithFilterMembers instead") NXOpen::Selection::Response SelectObjects
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the mask array will modify the selection filter */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::MaskTriple> & maskArray /** Modifies the list of object types that can be selected.
                                                                                                How it modifies the list of object types is determined by the SelectionAction parameter. */,
            std::vector<NXOpen::NXObject *> & objectArray /** The selected objects */
        );
        /** Select objects using filter defined by mask triples.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectsWithFilterMembers Selection::SelectTaggedObjectsWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectObjects
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the mask array will modify the selection filter */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::MaskTriple> & maskArray /** Modifies the list of object types that can be selected.
                                                                                                How it modifies the list of object types is determined by the SelectionAction parameter. */,
            std::vector<NXOpen::NXObject *> & objectArray /** The selected objects */
        );
        /** Select tagged objects using a filter defined by mask triples.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObjects
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the mask array will modify the selection filter */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::MaskTriple> & maskArray /** Modifies the list of object types that can be selected.
                                                                                                How it modifies the list of object types is determined by the SelectionAction parameter. */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Select tagged objects using a filter defined by mask triples.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>.
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObjects
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the mask array will modify the selection filter */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::MaskTriple> & maskArray /** Modifies the list of object types that can be selected.
                                                                                                How it modifies the list of object types is determined by the SelectionAction parameter. */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Select objects using filter defined by selection type.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, or Cancel 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectsWithFilterMembers Selection::SelectTaggedObjectsWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::SelectTaggedObjectsWithFilterMembers instead") NXOpen::Selection::Response SelectObjects
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::SelectionType> & typeArray /** The types that can be selected */,
            std::vector<NXOpen::NXObject *> & objectArray /** The selected objects */
        );
        /** Select objects using filter defined by selection type.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, or Cancel 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectsWithFilterMembers Selection::SelectTaggedObjectsWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectObjects
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::SelectionType> & typeArray /** The types that can be selected */,
            std::vector<NXOpen::NXObject *> & objectArray /** The selected objects */
        );
        /** Select tagged objects using a filter defined by selection type.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObjects
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::SelectionType> & typeArray /** The types that can be selected */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Select tagged objects using a filter defined by selection type.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, or Cancel 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObjects
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::SelectionType> & typeArray /** The types that can be selected */,
            std::vector<NXOpen::TaggedObject *> & objectArray /** The selected objects */
        );
        /** Select single object.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectWithFilterMembers Selection::SelectTaggedObjectWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::SelectTaggedObjectWithFilterMembers instead") NXOpen::Selection::Response SelectObject
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            NXOpen::NXObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select single object.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectWithFilterMembers Selection::SelectTaggedObjectWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectObject
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            NXOpen::NXObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select a single object using a filter defined by mask triples.
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectWithFilterMembers Selection::SelectTaggedObjectWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::SelectTaggedObjectWithFilterMembers instead") NXOpen::Selection::Response SelectObject
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the mask array will modify the selection filter */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::MaskTriple> & maskArray /** Modifies the list of object types that can be selected.
                                                                                                How it modifies the list of object types is determined by the SelectionAction parameter. */,
            NXOpen::NXObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select a single object using a filter defined by mask triples.
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectWithFilterMembers Selection::SelectTaggedObjectWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectObject
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the mask array will modify the selection filter */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::MaskTriple> & maskArray /** Modifies the list of object types that can be selected.
                                                                                                How it modifies the list of object types is determined by the SelectionAction parameter. */,
            NXOpen::NXObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select a single tagged object using a filter defined by mask triples.
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObject
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the mask array will modify the selection filter */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::MaskTriple> & maskArray /** Modifies the list of object types that can be selected.
                                                                                                How it modifies the list of object types is determined by the SelectionAction parameter. */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select a single tagged object using a filter defined by mask triples.
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
            
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObject
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            NXOpen::Selection::SelectionAction action /** Indicates how the mask array will modify the selection filter */,
            bool includeFeatures /** Whether to allow the selection of features */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::MaskTriple> & maskArray /** Modifies the list of object types that can be selected.
                                                                                                How it modifies the list of object types is determined by the SelectionAction parameter. */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select a single object using a filter defined by selection type.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectWithFilterMembers Selection::SelectTaggedObjectWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::SelectTaggedObjectWithFilterMembers instead") NXOpen::Selection::Response SelectObject
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::SelectionType> & typeArray /** The types of objects that can be selected */,
            NXOpen::NXObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select a single object using a filter defined by selection type.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::SelectTaggedObjectWithFilterMembers Selection::SelectTaggedObjectWithFilterMembers@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectObject
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::SelectionType> & typeArray /** The types of objects that can be selected */,
            NXOpen::NXObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select single tagged object using filter defined by selection type.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectTaggedObject
        (
            const NXString & message /** Cue line message to display */,
            const NXString & title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::SelectionType> & typeArray /** The types of objects that can be selected */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select single tagged object using filter defined by selection type.
            
            To enable selection of objects within drawing member views, use SetCursorView API available on
            <a href="../ugopen_doc/uf_ui/index.html"> UFUi</a>. 
              @return  Ok, Back, Cancel, ObjectSelected, or ObjectSelectedByName 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectTaggedObject
        (
            const char * message /** Cue line message to display */,
            const char * title /** Dialog title */,
            NXOpen::Selection::SelectionScope scope /** Selection scope */,
            bool keepHighlighted /** Whether to keep the selection highlighted after it has been selected */,
            const std::vector<NXOpen::Selection::SelectionType> & typeArray /** The types of objects that can be selected */,
            NXOpen::TaggedObject ** object /** The selected object */,
            NXOpen::Point3d* cursor /** Absolute coordinates of cursor position. This is
                                                undefined if object is selected by name */
        );
        /** Select features  @return  Ok, Back, or Cancel 
         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::Response SelectFeatures
        (
            const NXString & message /** Cue line message to display */,
            NXOpen::Selection::SelectionFeatureType featType /** The types of features that are selectable */,
            std::vector<NXOpen::Features::Feature *> & featureArray /** The selected features */
        );
        /** Select features  @return  Ok, Back, or Cancel 
         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::Response SelectFeatures
        (
            const char * message /** Cue line message to display */,
            NXOpen::Selection::SelectionFeatureType featType /** The types of features that are selectable */,
            std::vector<NXOpen::Features::Feature *> & featureArray /** The selected features */
        );
        /** Select screen position on graphics display  @return  Pick, Back, or Cancel 
         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::DialogResponse SelectScreenPosition
        (
            const NXString & message /** Cue line message to display */,
            NXOpen::View ** object /** View of selected screen location */,
            NXOpen::Point3d* screenPosition /** Selected screen position */
        );
        /** Select screen position on graphics display  @return  Pick, Back, or Cancel 
         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        NXOpen::Selection::DialogResponse SelectScreenPosition
        (
            const char * message /** Cue line message to display */,
            NXOpen::View ** object /** View of selected screen location */,
            NXOpen::Point3d* screenPosition /** Selected screen position */
        );
        /** Returns the number of objects currently selected  @return  The number of objects currently selected 
         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: int GetNumSelectedObjects
        (
        );
        /** Returns the n'th object on the selection list     @return  The object at the given position on the selection list. Returns NULL if the index parameter
                                                is greater than the length of the list 
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::GetSelectedTaggedObject Selection::GetSelectedTaggedObject@endlink  instead <br>  

         <br>  Created in NX3.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::GetSelectedTaggedObject instead") NXOpen::NXObject * GetSelectedObject
        (
            int index /** The index. Should be between 0 and @link GetNumSelectedObjects GetNumSelectedObjects@endlink  */
        );
        /** Returns the n'th tagged object on the selection list     @return  The object at the given position on the selection list. Returns NULL if the index parameter
                                                is greater than the length of the list 
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: NXOpen::TaggedObject * GetSelectedTaggedObject
        (
            int index /** The index. Should be between 0 and @link GetNumSelectedObjects GetNumSelectedObjects@endlink  */
        );
        /** Returns the set of Filter Members that support selection of the given object.
            
            This method returns Filter Member enumeration values from
            @link NXOpen::Select::FilterMember NXOpen::Select::FilterMember@endlink .
             
         <br>  Created in NX2212.0.0.  <br>  
         <br> License requirements : None */
        public: void GetFilterMembersForTaggedObject
        (
            NXOpen::TaggedObject * forObject /** The object supported by all returned Filter Members */,
            std::vector<NXOpen::Select::FilterMember> & filterMembers /** The Filter Members
                                          that support the given object, in increasing order of the number
                                          of object classes supported by the Filter Member */
        );
        /** Inquires whether or not Global Selection (no active dialog) is active.   @return  True if Global Selection is active (no active dialog) 
         <br>  Created in NX11.0.0.  <br>  
         <br> License requirements : None */
        public: bool IsGlobalSelectionActive
        (
        );
        /** Clears the Global Selection list 
         <br>  Created in NX11.0.0.  <br>  
         <br> License requirements : None */
        public: void ClearGlobalSelectionList
        (
        );
        /** Sets the specified Filter Member as the only enabled member in the Global Selection filter. 
            The specified Filter Member must be a member of the Global Selection filter. This function is only honored when Global Selection is active. 
         <br>  Created in NX2007.0.0.  <br>  
         <br> License requirements : None */
        public: void SetEnabledGlobalFilterMember
        (
            NXOpen::Select::FilterMember filterMember /** The filter member to be exclusively enabled. */
        );
        /** Sets the specified Filter Members as the only enabled members in the Global Selection filter. 
            The specified Filter Members must be members of the Global Selection filter. This function is only honored when Global Selection is active. 
         <br>  Created in NX2007.0.0.  <br>  
         <br> License requirements : None */
        public: void SetEnabledGlobalFilterMembers
        (
            const std::vector<NXOpen::Select::FilterMember> & filterMembers /** The filter members to be exclusively enabled. */
        );
        /** Resets the set of enabled Global Selection filter members as dictated by the current application. Typically this will be all or most of the supported filter members. 
            This function is only honored when Global Selection is active. 
         <br>  Created in NX2007.0.0.  <br>  
         <br> License requirements : None */
        public: void ResetEnabledGlobalFilterMembers
        (
        );
        /** Submit a request to the current selection owner to select the given objects. Note that the
                selection owner may not allow some objects to be selected. 
         <br>  Created in NX11.0.0.  <br>  
         <br> License requirements : None */
        public: void RequestSelections
        (
            const std::vector<NXOpen::TaggedObject *> & selectList /** selectlist */ 
        );
        /** Submit a request to the current selection owner to deselect the given objects. Note that the
                selection owner may not allow some objects to be deselected. 
         <br>  Created in NX11.0.0.  <br>  
         <br> License requirements : None */
        public: void RequestDeselections
        (
            const std::vector<NXOpen::TaggedObject *> & deselectList /** deselectlist */ 
        );
        /** Sets the types of objects that are selectable in a UIStyler dialog. If this function is not
                called to set the mask, then the default mask is all standard types.
            
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: void SetSelectionMask
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            NXOpen::Selection::SelectionAction action /** Mask action */,
            const std::vector<NXOpen::Selection::MaskTriple> & maskArray /** Mask triples */
        );
        /** Sets the selection type. It can be called from a callback to change 
            the type of selection associated with a UIStyler dialog. 
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: void SetSelectType
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            NXOpen::Selection::UistylerSelectionType type /** Selection type */
        );
        /** Specify selection filter callback for additional filtering based on
                application specific criteria, and/or selection callback to perform
                application specific processing for each selection gesture processed. 
                Both the filter callback and the selection callback are passed object 
                to @link  SelectionHandle   SelectionHandle @endlink  which can be used to inquire 
                other selection data or modify selection. This selection handle object 
                is no longer valid after the filter callback or selection callback is exited.
            
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link NXOpen::Selection::SetTaggedObjectSelectionCallbacks NXOpen::Selection::SetTaggedObjectSelectionCallbacks@endlink  instead <br>  

         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use NXOpen::Selection::SetTaggedObjectSelectionCallbacks instead") void SetSelectionCallbacks
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            const NXOpen::Selection::FilterCallback&  filterproc /** Filter callback for additional user specific
            filtering. */,
            const NXOpen::Selection::SelectionCallback&  selcb /** Selection callback for application specific
            processing. */
        );
        /** For a UIStyler dialog: Specify selection filter callback for additional filtering of tagged objects based on
                application specific criteria, and/or selection callback to perform
                application specific processing for each selection gesture processed. 
                Both the filter callback and the selection callback are passed object 
                to @link  SelectionHandle   SelectionHandle @endlink  which can be used to inquire 
                other selection data or modify selection. This selection handle tagged object 
                is no longer valid after the filter callback or selection callback is exited.
            
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: void SetTaggedObjectSelectionCallbacks
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            const NXOpen::Selection::FilterTaggedObjectCallback&  filterproc /** Filter callback for additional user specific filtering. */,
            const NXOpen::Selection::TaggedObjectSelectionCallback&  selcb /** Selection callback for application specific processing. */
        );
        /** Gets whether or not the user defined class is available for selection.  @return  Allow the selection of this UserDefinedClass 
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: bool GetSelectionStatusOfUserDefinedClass
        (
            NXOpen::UserDefinedObjects::UserDefinedClass * udoClass /** The UserDefinedClass in question */
        );
        /** Sets whether or not the user defined class is available for selection. 
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: void SetSelectionStatusOfUserDefinedClass
        (
            NXOpen::UserDefinedObjects::UserDefinedClass * udoClass /** The UserDefinedClass in question */,
            bool selectionStatus /** Allow the selection of this UserDefinedClass */
        );
        /** Removes objects from the selection list.
            Use this function with UIStyler dialogs. It can be called from the selection callback. 
            It cannot be called from the selection filter procedure.
            
            If any of the objects are not in the list, an error is returned and NO
            objects are removed from the list.
            
            It could be called by the selection callback to remove
            objects from the selection list. For example, based on objects just
            deselected, the application may need to remove other associated
            objects from the selection list.
            
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::RemoveTaggedObjectsFromSelectionList Selection::RemoveTaggedObjectsFromSelectionList@endlink  instead <br>  

         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::RemoveTaggedObjectsFromSelectionList instead") void RemoveFromSelectionList
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            const std::vector<NXOpen::NXObject *> & objs /** Array of object */,
            bool unhighlight /** If true, unhighlight objects */
        );
        /** Removes tagged objects from the selection list.
            Use this function with UIStyler dialogs. It can be called from the selection callback. 
            It cannot be called from the selection filter procedure.
            
            If any of the objects are not in the list, an error is returned and NO
            objects are removed from the list.
            
            It could be called by the selection callback to remove
            objects from the selection list. For example, based on objects just
            deselected, the application may need to remove other associated
            objects from the selection list.
            
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: void RemoveTaggedObjectsFromSelectionList
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            const std::vector<NXOpen::TaggedObject *> & objs /** Array of object */,
            bool unhighlight /** If true, unhighlight objects */
        );
        /** Removes all the objects from the selection list and optionally unhighlights them.
            Use this function with UIStyler dialogs.
            
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: void RemoveAllFromSelectionList
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            bool unhighlight /** If true, unhighlight objects */
        );
        /** Inquires if object is selected.
            Use this function with UIStyler dialogs.
             @return  If true, object is in the selection list, else false. 
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: bool IsObjectInSelectionList
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            NXOpen::TaggedObject * object /** Object */
        );
        /** Returns the number of objects currently selected.
            Use this function with UIStyler dialogs.
             @return  Count of objects selected 
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: int AskSelectionListCount
        (
            NXOpen::SelectionHandle * select /** Selection handle */
        );
        /** Returns the number of objects selected and a pointer to an array of
            tags of the objects selected. Use this function with UIStyler dialogs.
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::AskSelectionTaggedObjectList Selection::AskSelectionTaggedObjectList@endlink  instead <br>  

         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::AskSelectionTaggedObjectList instead") void AskSelectionObjectList
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            std::vector<NXOpen::NXObject *> & objects /** Selected objects. */
        );
        /** Returns the number of tagged objects selected and a pointer to an array of
            tags of the objects selected. Use this function with UIStyler dialogs.
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: void AskSelectionTaggedObjectList
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            std::vector<NXOpen::TaggedObject *> & objects /** Selected objects. */
        );
        /** Add objects to the selection list.
            Any objects already in the selection list are ignored. Use this function with UIStyler dialogs. 
            You can call this from the constructor callback/selection initialization procedure to begin 
            the dialog with objects already selected. 
             The user can then review these objects, and if desired, deselect them.
            
            The application selection callback can also call this function to add other objects 
            to the selection list based on the objects(s) just selected. The selection filter 
            procedure cannot call this function. Based on the object(s) just selected, other 
            objects may need to be selected. For example, all edges of the selected face 
            or all faces tangent to the selected face.
            
         <br>  @deprecated Deprecated in NX8.0.0.  Use @link Selection::AddToTaggedObjectsSelectionList Selection::AddToTaggedObjectsSelectionList@endlink  instead <br>  

         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: NX_DEPRECATED("Deprecated in NX8.0.0.  Use Selection::AddToTaggedObjectsSelectionList instead") void AddToSelectionList
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            const std::vector<NXOpen::NXObject *> & objs /** Objects to be added in the selection list */,
            bool highlightFlag /** If true, highlight objects */
        );
        /** Add objects to the selection list.
            Any objects already in the selection list are ignored. Use this function with UIStyler dialogs. 
            You can call this from the constructor callback/selection initialization procedure to begin 
            the dialog with objects already selected. 
             The user can then review these objects, and if desired, deselect them.
            
            The application selection callback can also call this function to add other objects 
            to the selection list based on the objects(s) just selected. The selection filter 
            procedure cannot call this function. Based on the object(s) just selected, other 
            objects may need to be selected. For example, all edges of the selected face 
            or all faces tangent to the selected face.
            
         <br>  Created in NX7.5.4.  <br>  
         <br> License requirements : None */
        public: void AddToTaggedObjectsSelectionList
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            const std::vector<NXOpen::TaggedObject *> & objs /** Objects to be added in the selection list */,
            bool highlightFlag /** If true, highlight objects */
        );
        /** Returns the information in @link  Selection::SelectionDescriptor   Selection::SelectionDescriptor @endlink  
            describing the selection that was just performed.
             @return  
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Selection::SelectionDescriptor AskSelectionDescriptor
        (
            NXOpen::SelectionHandle * select /** select */ 
        );
        /** Returns the view and absolute coordinates of the cursor position for the associated selection. 
                If an object was selected, the view returned is the view in which the object was selected. 
                For single position, the view is the view of the cursor. If the object was selected by name, 
                view = NULL and the cursor position is undefined.
             @return  
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::Point3d AskSelectionCursorPosition
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            NXOpen::View ** view /** view */ 
        );
        /** Returns the absolute coordinates of the rectangle cursor positions.
            The view that returns is the view of the button down position.
            
            button down position - is the position where you press and hold mouse button 1.
            button up position -   is the position where you release mouse button 1.
            
            Pos1 and Pos2 are the absolute coordinates of the button down and
            button up positions respectively. Pos3 is the absolute coordinates of
            the rectangle corner which, as viewed on the screen, is horizontal with
            pos1. Pos4 is the absolute coordinates of the rectangle corner which,
            as viewed on the screen, is horizontal with pos2.
            Returns an error if the last gesture was not rectangle.
             @return  View of button down position 
         <br>  Created in NX5.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::View * AskSelectionRectanglePosition
        (
            NXOpen::SelectionHandle * select /** Selection handle */,
            NXOpen::Point3d* position1 /** Absolute coordinates of the button down position */,
            NXOpen::Point3d* position2 /** Absolute coordinates of the button up position */,
            NXOpen::Point3d* position3 /** Absolute coordinates of corner of screen rectangle
                                           which is horizontal with button down position*/,
            NXOpen::Point3d* position4 /** Absolute coordinates of corner of screen rectangle
                                           which is horizontal with button up position */
        );
        /** Creates a Selection Subscriber 
         <br>  Created in NX11.0.0.  <br>  
         <br> License requirements : None */
        public: NXOpen::SelectionSubscriber * CreateSelectionSubscriber
        (
        );
    }; //lint !e1712 default constructor not defined for class  

}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
