//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     CAM_DrillToolBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/CAM_MillingToolBuilder.hxx>
#include <NXOpen/libnxopencpp_cam_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace CAM
    {
        class DrillToolBuilder;
    }
    namespace CAM
    {
        class InheritableDoubleBuilder;
    }
    namespace CAM
    {
        class MillingToolBuilder;
    }
    namespace CAM
    {
        class _DrillToolBuilderBuilder;
        class DrillToolBuilderImpl;
        /** Represents a drill Tool Builder  <br> This is an abstract class, and cannot be instantiated.  <br> 
         <br>  Created in NX5.0.0.  <br>  
        */
        class NXOPENCPP_CAMEXPORT  DrillToolBuilder : public NXOpen::CAM::MillingToolBuilder
        {
            private: DrillToolBuilderImpl * m_drilltoolbuilder_impl;
            private: friend class  _DrillToolBuilderBuilder;
            protected: DrillToolBuilder();
            public: ~DrillToolBuilder();
            /**Returns  the Point Angle builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlPointAngBuilder
            (
            );
            /**Returns  the Corner Radius builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlCor1RadBuilder
            (
            );
            /**Returns  the X Offset builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlXOffsetBuilder
            (
            );
            /**Returns  the YZ Offset builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlYzOffsetBuilder
            (
            );
            /**Returns  the coolant through  
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: bool TlCoolantThro
            (
            );
            /**Sets  the coolant through  
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetTlCoolantThro
            (
                bool tlCoolantThroValue /**  the coolant through value */
            );
            /**Returns  the pilot diameter builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlPilotDiaBuilder
            (
            );
            /**Returns  the pilot length builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlPilotLengthBuilder
            (
            );
            /**Returns  the Tip Length builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlTipLengthBuilder
            (
            );
            /**Returns  the Included Angle builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlIncludedAngBuilder
            (
            );
            /**Returns  the Tip Diameter builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlTipDiameterBuilder
            (
            );
            /**Returns  the Designation  
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXString TlDesignation
            (
            );
            /**Sets  the Designation  
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetTlDesignation
            (
                const NXString & tlDesignation /**  the designation value */
            );
            /**Sets  the Designation  
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            void SetTlDesignation
            (
                const char * tlDesignation /**  the designation value */
            );
            /**Returns  the Pitch builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlPitchBuilder
            (
            );
            /**Returns  the Taper Diameter Distance builder (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlTaperDiaDistBuilder
            (
            );
            /**Returns  the Point Length builder (inheritable double) 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlPointLengthBuilder
            (
            );
            /**Returns  the tolerance class 
             <br>  Created in NX1953.0.0.  <br>  
             <br> License requirements : None */
            public: NXString TlToleranceClass
            (
            );
            /**Sets  the tolerance class 
             <br>  Created in NX1953.0.0.  <br>  
             <br> License requirements : None */
            public: void SetTlToleranceClass
            (
                const NXString & tlToleranceClass /** tltoleranceclass */ 
            );
            /**Sets  the tolerance class 
             <br>  Created in NX1953.0.0.  <br>  
             <br> License requirements : None */
            void SetTlToleranceClass
            (
                const char * tlToleranceClass /** tltoleranceclass */ 
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
