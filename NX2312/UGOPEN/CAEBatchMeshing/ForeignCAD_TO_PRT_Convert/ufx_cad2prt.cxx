...
* 23-Jul-2013  Prabhakar Mandlik       Changes for ARCH10354 Internationalization All User Text
* 28-Aug-2013  Keith Hafen          Back out above changes
13-Sep-2016  Lavarini               Include uf_modl header files directly
10-Apr-2023  Lavarini               Fix Coverity security issues
*$HISTORY$
**
*****************************************************************************/
    ...
    while ((c = get_cmdline_opts((int)argc, argv, "i:o:t:u:h")) != EOF)
      {
        if (c == (int)'i')
        {
            //  <Lavarini>  10-Apr-2023  CID#108868
            //  Compare size of optarg with buffer size to insure there are no overwrites
            if ( strlen ( optarg ) < sizeof ( fileIn ) )
            {
                strcpy ( fileIn, optarg ); // NOLINT(qaz-nx-security-fixed-size-strcpy)
            }
            else
            {
                printf ( "fileIn buffer is too small to contain the input file\n" );
                return 0;
            }
        }
        if (c == (int)'o')
        {
            //  <Lavarini>  10-Apr-2023  CID#108868
            //  Compare size of optarg with buffer size to insure there are no overwrites
            if ( strlen ( optarg ) < sizeof ( fileOut ) )
            {
                strcpy ( fileOut, optarg ); // NOLINT(qaz-nx-security-fixed-size-strcpy)
            }
            else
            {
                printf ( "fileOut buffer is too small to contain the output file\n" );
                return 0;
            }
        }
        if (c == (int)'u') {
...
static int get_cmdline_opts(int argc, char *argv[], char *opt)
   ...
   optopt = c = ( unsigned char ) argv[optind][i];
