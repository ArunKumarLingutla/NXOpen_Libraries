...
class TreeListDemo:
    ...
    def OnInsertNodeCallback(self, tree, node):
        try:
            ...
            if TreeListDemo.nodeSuffixNumber == 15000:
                ...
                TreeListDemo.nodeSuffixNumber += 1
    ...
    def OnSelectCallback(self, tree, node, columnID, selected):
        try:
            ...
            if selected == True:
                text = "OnSelectCallback Invoked: Node \"{}\" Selected at column: {:d}".format(node.DisplayText, columnID)
            else:
                text = "OnSelectCallback Invoked: Node \"{}\" Deselected".format(node.DisplayText)
    ...
    # The possible return values are 0, positive and negative value, suggesting respectively that
    # both nodes are the same, the first node is greater than the second, and the first node is 
    ...
    def IsDragAllowedCallback(self, tree, node, columnID):  
        ...
        try:
            ...
            self.theUI.NXMessageBox.Show("Block Styler", NXOpen.NXMessageBox.DialogType.Error, str(ex))
        return result
    #Function to move/drag the child Nodes
    def MoveChildNodes(self, tree, node, copyNode):
        
        #child of node to be moved
        childNode = node[0].FirstChildNode
        # if there exist child nodes then move all along with its parent
        if childNode is not None:
            childNodeList = []
            while childNode is not None:
                copychildNode = tree.CopyNode(childNode)
                #adding the copy of child node to the list
                childNodeList.append(copychildNode)
                childNode=childNode.NextSiblingNode
            
            childNode = node[0].FirstChildNode
            
            #Get the previous node of the child node
            previousChildNode = NXOpen.BlockStyler.Node.Null
            #if there exist child nodes, move them to copyNode
            if childNode.PreviousSiblingNode:
                previousChildNode = childNode.PreviousSiblingNode
            # inserting all the child nodes
            for newNode in childNodeList:
                #If there is no previous node of the childNode, should be inserted first
                if previousChildNode is None:
                    tree.InsertNode(newNode, copyNode, None, NXOpen.BlockStyler.Tree.NodeInsertOption.First)
                else:
                    tree.InsertNode(newNode, copyNode, previousChildNode, NXOpen.BlockStyler.Tree.NodeInsertOption.Last)
                previousChildNode = newNode
    ...
    def OnDropCallback(self, tree, node, columnID, targetNode, targetColumnID, dropType, dropMenuItemId):
        try:
            ...
            self.WriteCallbackToListingWindow("class name: self " + type(self).__name__)
            self.WriteCallbackToListingWindow("node being moved:  " + (node[0].DisplayText))
            self.WriteCallbackToListingWindow("targetNode:  " + (targetNode.DisplayText))

            #Node to be moved or dragged
            copyNode = tree.CopyNode(node[0])

            #Get the parent of the target node
            parentNode = NXOpen.BlockStyler.Node.Null
            
            if targetNode.ParentNode:
                self.WriteCallbackToListingWindow("targetNode.ParentNode:  " + (targetNode.ParentNode.DisplayText))
                parentNode = targetNode.ParentNode
                self.WriteCallbackToListingWindow("parentNode exists for targetNode:  " + (parentNode.DisplayText))
  
            #inserting the copy node at target position 
            #dropType indicates whether the node will be dropped Before, After.
            #for dropType On Node, the implemention is kept same as drop type Before. If a different result is expected, it needs to be implemented
            if dropType is NXOpen.BlockStyler.Node.DropType.Before or dropType is NXOpen.BlockStyler.Node.DropType.On:
                #Get the previous node of the target node
                previousNode = NXOpen.BlockStyler.Node.Null
                if targetNode.PreviousSiblingNode:
                    previousNode = targetNode.PreviousSiblingNode
                #If there is no previous node, it should be inserted first
                if previousNode is None:
                    tree.InsertNode(copyNode, parentNode, previousNode, NXOpen.BlockStyler.Tree.NodeInsertOption.First)
                else:
                    tree.InsertNode(copyNode, parentNode, previousNode, NXOpen.BlockStyler.Tree.NodeInsertOption.Last)
                self.MoveChildNodes(tree, node, copyNode)       
                tree.DeleteNode(node[0])
            
            elif dropType is NXOpen.BlockStyler.Node.DropType.After or dropType is NXOpen.BlockStyler.Node.DropType.BeforeAndAfter:
                tree.InsertNode(copyNode, parentNode, targetNode, NXOpen.BlockStyler.Tree.NodeInsertOption.Last)
                self.MoveChildNodes(tree, node, copyNode)       
                tree.DeleteNode(node[0])
            return True
        except Exception as ex:
