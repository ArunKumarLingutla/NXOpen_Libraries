//--------------------------------------------------------------------------
// Copyright 2025 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Mechatronics_ExportLoadCurveBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Builder.hxx>
#include <NXOpen/libnxopencpp_mechatronics_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace Mechatronics
    {
        class ExportLoadCurveBuilder;
    }
    class Builder;
    namespace Mechatronics
    {
        class PhysicsConstraint;
    }
    namespace Mechatronics
    {
        class _ExportLoadCurveBuilderBuilder;
        class ExportLoadCurveBuilderImpl;
        /** Exports input parameters for motor selection to SIZER drive design to
                support the sizing, configuration, and selection of drive components.  <br> To create a new instance of this class, use @link NXOpen::Mechatronics::SIZERCollection::CreateExportBuilder  NXOpen::Mechatronics::SIZERCollection::CreateExportBuilder @endlink  <br> 
         <br>  Created in NX1926.0.0.  <br>  
        */
        class NXOPENCPP_MECHATRONICSEXPORT  ExportLoadCurveBuilder : public NXOpen::Builder
        {
             /** Defines the mechanical data in MCD mode. */
            public:
            struct MechanicalData
            {
                public: /** Unique ID                    */int Id;
                public: /** Mechanical Name              */NXString Name;
                public: /** Axis Control                 */NXOpen::Mechatronics::PhysicsConstraint * AxisControl;
                public: /** Logical Reference Designator */NXString ReferenceDesignator;
                public: /** Friction Torque value        */double FrictionTorque;
                public: /** System Efficiency value      */double SystemEfficiency;
                public: /** Load Inertia value           */double LoadInertia;
                public: /** Additional Inertia value     */double AdditionalInertia;
                public: MechanicalData() :
                    Id(),
                    Name(),
                    AxisControl(),
                    ReferenceDesignator(),
                    FrictionTorque(),
                    SystemEfficiency(),
                    LoadInertia(),
                    AdditionalInertia()
                {
                }
                /** Constructor for the MechanicalData struct. */ 
                public: MechanicalData(int idInitial /** Unique ID                    */, 
                        const NXString &nameInitial /** Mechanical Name              */, 
                        NXOpen::Mechatronics::PhysicsConstraint * axisControlInitial /** Axis Control                 */, 
                        const NXString &referenceDesignatorInitial /** Logical Reference Designator */, 
                        double frictionTorqueInitial /** Friction Torque value        */, 
                        double systemEfficiencyInitial /** System Efficiency value      */, 
                        double loadInertiaInitial /** Load Inertia value           */, 
                        double additionalInertiaInitial /** Additional Inertia value     */) :
                    Id(idInitial),
                    Name(nameInitial),
                    AxisControl(axisControlInitial),
                    ReferenceDesignator(referenceDesignatorInitial),
                    FrictionTorque(frictionTorqueInitial),
                    SystemEfficiency(systemEfficiencyInitial),
                    LoadInertia(loadInertiaInitial),
                    AdditionalInertia(additionalInertiaInitial)
                {
                }
            };

             /** Defines the gear system parameters */
            public:
            struct MCDGearParameters
            {
                public: /** enable */ bool Enable;
                public: /** gear ratio               */double Ratio;
                public: /** gear efficiency          */double Efficiency;
                public: /** gear inertia             */double Inertia;
                public: MCDGearParameters() :
                    Enable(),
                    Ratio(),
                    Efficiency(),
                    Inertia()
                {
                }
                /** Constructor for the MCDGearParameters struct. */ 
                public: MCDGearParameters(bool enableInitial /** enable */ , 
                        double ratioInitial /** gear ratio               */, 
                        double efficiencyInitial /** gear efficiency          */, 
                        double inertiaInitial /** gear inertia             */) :
                    Enable(enableInitial),
                    Ratio(ratioInitial),
                    Efficiency(efficiencyInitial),
                    Inertia(inertiaInitial)
                {
                }
            };

            private: ExportLoadCurveBuilderImpl * m_exportloadcurvebuilder_impl;
            private: friend class  _ExportLoadCurveBuilderBuilder;
            protected: ExportLoadCurveBuilder();
            public: ~ExportLoadCurveBuilder();
            /** Gets all groups of mechanical data in MCD mode. 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            public: void GetMcdMechanicalData
            (
                std::vector<NXOpen::Mechatronics::ExportLoadCurveBuilder::MechanicalData> & mechanicalData /** Array of mechanical data */
            );
            /** Sets all groups of mechanical data in MCD mode. 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            public: void SetMcdMechanicalData
            (
                const std::vector<NXOpen::Mechatronics::ExportLoadCurveBuilder::MechanicalData> & mechanicalData /** Array of mechanical data */
            );
            /** Processes the simulation data in MCD mode. 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            public: void ProcessMcdSimulationData
            (
                double startTime /** Start Time of simulation */,
                double endTime /** End Time of simulation */
            );
            /**Returns  the output *.mdix file, which is exported to SIZER. 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            public: NXString OutputFile
            (
            );
            /**Sets  the output *.mdix file, which is exported to SIZER. 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            public: void SetOutputFile
            (
                const NXString & outputFile /** outputfile */ 
            );
            /**Sets  the output *.mdix file, which is exported to SIZER. 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            void SetOutputFile
            (
                const char * outputFile /** outputfile */ 
            );
            /**Returns  the output folder of *.csv files, which are exported to SIZER4. 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            public: NXString OutputFolder
            (
            );
            /**Sets  the output folder of *.csv files, which are exported to SIZER4. 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            public: void SetOutputFolder
            (
                const NXString & outputFile /** outputfile */ 
            );
            /**Sets  the output folder of *.csv files, which are exported to SIZER4. 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            void SetOutputFolder
            (
                const char * outputFile /** outputfile */ 
            );
            /** Get gear parameters  @return  Output of gear system parameters
             <br>  Created in NX1953.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            public: NXOpen::Mechatronics::ExportLoadCurveBuilder::MCDGearParameters GetGearParameters
            (
            );
            /** Set gear parameters 
             <br>  Created in NX1953.0.0.  <br>  
             <br> License requirements : nx_mcd_core ("Mechatronics Concept Designer") OR nx_mcd_essential ("Mechatronics Concept Designer Essential") */
            public: void SetGearParameters
            (
                const NXOpen::Mechatronics::ExportLoadCurveBuilder::MCDGearParameters & gearParams /** input of gear system parameters*/
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
