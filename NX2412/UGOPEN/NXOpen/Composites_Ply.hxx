//--------------------------------------------------------------------------
// Copyright 2024 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Composites_Ply.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Composites_Base.hxx>
#include <NXOpen/Composites_IPly.hxx>
#include <NXOpen/INXObject.hxx>
#include <NXOpen/ugmath.hxx>
#include <NXOpen/libcompositesopencpp_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace Composites
    {
        class Ply;
    }
    namespace Assemblies
    {
        class Component;
    }
    class BasePart;
    namespace Composites
    {
        class Base;
    }
    namespace Composites
    {
        class IPly;
    }
    namespace Composites
    {
        class IPlyPiece;
    }
    namespace Composites
    {
        class PlyPiece;
    }
    class INXObject;
    namespace Composites
    {
        class _PlyBuilder;
        class PlyImpl;
        /** Represents a single piece/layer of material to be deposited during manufacturing  <br> Plies are created with a @link Composites::PlyBuilder Composites::PlyBuilder@endlink .  <br> 
         <br>  Created in NX1926.0.0.  <br>  
        */
        class COMPOSITESOPENCPPEXPORT  Ply : public NXOpen::Composites::Base, public virtual NXOpen::Composites::IPly
        {
            /** Insert options for resequencing */
            public: enum ResequenceInsertOption
            {
                ResequenceInsertOptionBefore/** before */ ,
                ResequenceInsertOptionAfter/** after */ 
            };

            private: PlyImpl * m_ply_impl;
            private: friend class  _PlyBuilder;
            protected: Ply();
            public: ~Ply();
            /** Resequences the given ply pieces before or after the provided target ply piece. 
             <br>  @deprecated Deprecated in NX2406.0.0.  Use ResequenceIPlyPieces instead <br>  

             <br>  Created in NX2312.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: NX_DEPRECATED("Deprecated in NX2406.0.0.  Use ResequenceIPlyPieces instead") void ResequencePlyPieces
            (
                const std::vector<NXOpen::Composites::PlyPiece *> & piecesToResequence /** pieces to resequence */ ,
                NXOpen::Composites::PlyPiece * targetPlyPiece /** target ply piece */ ,
                NXOpen::Composites::Ply::ResequenceInsertOption insertOption /** insert option */ 
            );
            /** The ordered list of @link Composites::PlyPiece Composites::PlyPiece@endlink  objects that  this
                    @link Composites::Ply Composites::Ply@endlink  is broken up into 
             <br>  @deprecated Deprecated in NX2406.0.0.  Use GetOrderedIPlyPieces instead <br>  

             <br>  Created in NX2312.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: NX_DEPRECATED("Deprecated in NX2406.0.0.  Use GetOrderedIPlyPieces instead") std::vector<NXOpen::Composites::PlyPiece *> GetOrderedPlyPieces
            (
            );
            /** Resequences the given @link Composites::IPlyPiece Composites::IPlyPiece@endlink  objects before or after the provided target @link Composites::IPlyPiece Composites::IPlyPiece@endlink . 
             <br>  Created in NX2406.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: void ResequenceIPlyPieces
            (
                const std::vector<NXOpen::Composites::IPlyPiece *> & piecesToResequence /** pieces to resequence */ ,
                NXOpen::Composites::IPlyPiece * targetPlyPiece /** target ply piece */ ,
                NXOpen::Composites::Ply::ResequenceInsertOption insertOption /** insert option */ 
            );
            /** The ordered list of @link Composites::IPlyPiece Composites::IPlyPiece@endlink  objects that this
                    @link Composites::Ply Composites::Ply@endlink  is broken up into 
             <br>  Created in NX2406.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: std::vector<NXOpen::Composites::IPlyPiece *> GetOrderedIPlyPieces
            (
            );
            /** Show the ply's rosette-mapped orientation display 
             <br>  Created in NX1953.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: virtual void ShowRosetteMappedOrientationDisplay
            (
            );
            /** Hide the ply's rosette-mapped orientation display 
             <br>  Created in NX1953.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: virtual void HideRosetteMappedOrientationDisplay
            (
            );
            /**Returns  the mass of the @link Composites::IPly Composites::IPly@endlink . 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: virtual double Mass
            (
            );
            /**Returns  the area of the @link Composites::IPly Composites::IPly@endlink . 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: virtual double Area
            (
            );
            /**Returns  the density of the @link Composites::IPly Composites::IPly@endlink . 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: virtual double Density
            (
            );
            /**Returns  the perimeter of the @link Composites::IPly Composites::IPly@endlink . 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: virtual double Perimeter
            (
            );
            /**Returns  the cured thickness of the @link Composites::IPly Composites::IPly@endlink . 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: virtual double CuredThickness
            (
            );
            /**Returns  the center of mass of the @link Composites::IPly Composites::IPly@endlink . 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: virtual NXOpen::Point3d CenterOfMass
            (
            );
            /**Returns  the moment of inertia of the @link Composites::IPly Composites::IPly@endlink . 
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : nx_composites (" Composites") */
            public: virtual NXOpen::Matrix3x3 MomentOfInertia
            (
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
