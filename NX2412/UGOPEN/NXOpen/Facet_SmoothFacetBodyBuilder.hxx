//--------------------------------------------------------------------------
// Copyright 2024 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Facet_SmoothFacetBodyBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/FacetCollector.hxx>
#include <NXOpen/Builder.hxx>
#include <NXOpen/ObjectList.hxx>
#include <NXOpen/SelectObjectList.hxx>
#include <NXOpen/libnxopencpp_facet_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace Facet
    {
        class SmoothFacetBodyBuilder;
    }
    class Builder;
    namespace Facet
    {
        class SelectFacetedBodyList;
    }
    class FacetCollector;
    namespace GeometricUtilities
    {
        class BoundaryDefinitionBuilderList;
    }
    class SelectDisplayableObjectList;
    class SelectNXObjectList;
    namespace Facet
    {
        class _SmoothFacetBodyBuilderBuilder;
        class SmoothFacetBodyBuilderImpl;
        /** Represents a @link NXOpen::Facet::SmoothFacetBodyBuilder NXOpen::Facet::SmoothFacetBodyBuilder@endlink .
            Smooth Facet Body Builder is a function to facilitate the removal of noise from facet bodies while keeping the general shape in tact.  <br> To create a new instance of this class, use @link NXOpen::Facet::FacetedBodyCollection::CreateSmoothFacetBodyBuilder  NXOpen::Facet::FacetedBodyCollection::CreateSmoothFacetBodyBuilder @endlink  <br> 
        Default values.
        <list type="table"> 
         
        <listheader><term> 
        Property</term>  <description> 
        Value</description> </listheader>
        <item><term> 
         
        IsEditCopy </term> <description> 
         
        0 </description> </item> 

        <item><term> 
         
        IsLockBoundary </term> <description> 
         
        0 </description> </item> 

        <item><term> 
         
        IsSmoothOverEdges </term> <description> 
         
        1 </description> </item> 

        <item><term> 
         
        NumberOfIterations </term> <description> 
         
        1 </description> </item> 

        <item><term> 
         
        SmoothFactor </term> <description> 
         
        10 </description> </item> 

        </list> 

         <br>  Created in NX7.5.0.  <br>  
        */
        class NXOPENCPP_FACETEXPORT  SmoothFacetBodyBuilder : public NXOpen::Builder
        {
            /** Smooth Options */
            public: enum Types
            {
                TypesMesh/** mesh */ ,
                TypesEdge/** edge */ 
            };

            private: SmoothFacetBodyBuilderImpl * m_smoothfacetbodybuilder_impl;
            private: friend class  _SmoothFacetBodyBuilderBuilder;
            protected: SmoothFacetBodyBuilder();
            public: ~SmoothFacetBodyBuilder();
            /**Returns  the facet bodies to be smoothed 
             <br>  @deprecated Deprecated in NX11.0.0.  Use @link NXOpen::Facet::SmoothFacetBodyBuilder::Bodies NXOpen::Facet::SmoothFacetBodyBuilder::Bodies@endlink  instead. <br>  

             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : None */
            public: NX_DEPRECATED("Deprecated in NX11.0.0.  Use NXOpen::Facet::SmoothFacetBodyBuilder::Bodies instead.") NXOpen::Facet::SelectFacetedBodyList * FacetBodies
            (
            );
            /**Returns  the facet bodies to be smoothed. Inputs to this command can be convergent objects. 
             <br>  @deprecated Deprecated in NX12.0.0.  Use @link NXOpen::Facet::SmoothFacetBodyBuilder::FacetCollector NXOpen::Facet::SmoothFacetBodyBuilder::FacetCollector@endlink  instead. <br>  

             <br>  Created in NX11.0.0.  <br>  
             <br> License requirements : None */
            public: NX_DEPRECATED("Deprecated in NX12.0.0.  Use NXOpen::Facet::SmoothFacetBodyBuilder::FacetCollector instead.") NXOpen::SelectDisplayableObjectList * Bodies
            (
            );
            /**Returns  an optional list of regions on the facet bodies to be smoothed 
             <br>  @deprecated Deprecated in NX12.0.0.  Use @link NXOpen::Facet::SmoothFacetBodyBuilder::FacetCollector NXOpen::Facet::SmoothFacetBodyBuilder::FacetCollector@endlink  instead. <br>  

             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : None */
            public: NX_DEPRECATED("Deprecated in NX12.0.0.  Use NXOpen::Facet::SmoothFacetBodyBuilder::FacetCollector instead.") NXOpen::GeometricUtilities::BoundaryDefinitionBuilderList * RegionList
            (
            );
            /**Sets  a collector of facets on the facet bodies to be to be smoothed. 
             <br>  Created in NX12.0.0.  <br>  
             <br> License requirements : studio_free_form ("STUDIO FREE FORM") OR nx_polygon_modeling (" NX Polygon Modeling") */
            public: void SetFacetCollector
            (
                NXOpen::FacetCollector * collector /** collector */ 
            );
            /**Returns  a collector of facets on the facet bodies to be to be smoothed. 
             <br>  Created in NX12.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::FacetCollector * FacetCollector
            (
            );
            /**Returns  the smoothing factor 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : None */
            public: int SmoothFactor
            (
            );
            /**Sets  the smoothing factor 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : studio_free_form ("STUDIO FREE FORM") OR nx_polygon_modeling (" NX Polygon Modeling") */
            public: void SetSmoothFactor
            (
                int smoothFactor /** smoothfactor */ 
            );
            /**Returns  the modification percentage 
             <br>  @deprecated Deprecated in NX12.0.2.  This attribute is not very effective, hence this has been deprecated. <br>  

             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : None */
            public: NX_DEPRECATED("Deprecated in NX12.0.2.  This attribute is not very effective, hence this has been deprecated.") int ModifyPercent
            (
            );
            /**Sets  the modification percentage 
             <br>  @deprecated Deprecated in NX12.0.2.  This attribute is not very effective, hence this has been deprecated. <br>  

             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : studio_free_form ("STUDIO FREE FORM") OR nx_polygon_modeling (" NX Polygon Modeling") */
            public: NX_DEPRECATED("Deprecated in NX12.0.2.  This attribute is not very effective, hence this has been deprecated.") void SetModifyPercent
            (
                int modifyPercent /** modifypercent */ 
            );
            /**Returns  the option indicating if the open edges of inner and outer loops in the facet bodies are to be kept intact 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : None */
            public: bool IsLockBoundary
            (
            );
            /**Sets  the option indicating if the open edges of inner and outer loops in the facet bodies are to be kept intact 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : studio_free_form ("STUDIO FREE FORM") OR nx_polygon_modeling (" NX Polygon Modeling") */
            public: void SetLockBoundary
            (
                bool isLockBoundary /** islockboundary */ 
            );
            /**Returns  the option indicating if a copy of the facet body will be smoothed without altering the original 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : None */
            public: bool IsEditCopy
            (
            );
            /**Sets  the option indicating if a copy of the facet body will be smoothed without altering the original 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : studio_free_form ("STUDIO FREE FORM") OR nx_polygon_modeling (" NX Polygon Modeling") */
            public: void SetEditCopy
            (
                bool isEditCopy /** iseditcopy */ 
            );
            /**Returns  the number of iterations 
             <br>  Created in NX12.0.2.  <br>  
             <br> License requirements : None */
            public: int NumberOfIterations
            (
            );
            /**Sets  the number of iterations 
             <br>  Created in NX12.0.2.  <br>  
             <br> License requirements : studio_free_form ("STUDIO FREE FORM") OR nx_polygon_modeling (" NX Polygon Modeling") */
            public: void SetNumberOfIterations
            (
                int numberOfIterations /** numberofiterations */ 
            );
            /**Returns  the option indicating if a copy of the facet body will be smoothed without altering the original 
             <br>  Created in NX12.0.2.  <br>  
             <br> License requirements : None */
            public: NXOpen::SelectNXObjectList * FaceEdge
            (
            );
            /**Returns  an option to apply the smooth operator across face edges 
             <br>  Created in NX12.0.2.  <br>  
             <br> License requirements : None */
            public: bool IsSmoothOverEdges
            (
            );
            /**Sets  an option to apply the smooth operator across face edges 
             <br>  Created in NX12.0.2.  <br>  
             <br> License requirements : studio_free_form ("STUDIO FREE FORM") OR nx_polygon_modeling (" NX Polygon Modeling") */
            public: void SetSmoothOverEdges
            (
                bool isSmoothOverEdges /** issmoothoveredges */ 
            );
            /**Sets  the type 
             <br>  Created in NX12.0.2.  <br>  
             <br> License requirements : studio_free_form ("STUDIO FREE FORM") OR nx_polygon_modeling (" NX Polygon Modeling") */
            public: void SetType
            (
                NXOpen::Facet::SmoothFacetBodyBuilder::Types type /** type */ 
            );
            /**Returns  the type 
             <br>  Created in NX12.0.2.  <br>  
             <br> License requirements : None */
            public: NXOpen::Facet::SmoothFacetBodyBuilder::Types Type
            (
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
