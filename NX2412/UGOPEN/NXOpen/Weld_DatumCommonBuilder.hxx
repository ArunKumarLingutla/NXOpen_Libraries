//--------------------------------------------------------------------------
// Copyright 2024 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Weld_DatumCommonBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Expression.hxx>
#include <NXOpen/Builder.hxx>
#include <NXOpen/SelectObjectList.hxx>
#include <NXOpen/Weld_CharacteristicsBuilder.hxx>
#include <NXOpen/libnxopencpp_weld_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace Weld
    {
        class DatumCommonBuilder;
    }
    namespace Assemblies
    {
        class SelectComponentList;
    }
    class Axis;
    class Builder;
    class Direction;
    class Expression;
    class SelectNXObjectList;
    namespace Weld
    {
        class CharacteristicsBuilder;
    }
    namespace Weld
    {
        class _DatumCommonBuilderBuilder;
        class DatumCommonBuilderImpl;
        /** Used to create or edit a @link NXOpen::Weld::DatumSurface NXOpen::Weld::DatumSurface@endlink   or @link NXOpen::Weld::DatumPin NXOpen::Weld::DatumPin@endlink  feature. <br> This is an abstract class and cannot be directly instantiated  <br> 
         <br>  Created in NX8.5.0.  <br>  
        */
        class NXOPENCPP_WELDEXPORT  DatumCommonBuilder : public NXOpen::Builder
        {
            /** The custom type of the datum specified for creation. */
            public: enum CustomTypes
            {
                CustomTypesDefault/** A datum surface or pin specified in the customer defaults by the Default type. */,
                CustomTypesCustom1/** A datum surface or pin specified in the customer defaults by the Custom1 type. */,
                CustomTypesCustom2/** A datum surface or pin specified in the customer defaults by the Custom2 type. */,
                CustomTypesCustom3/** A datum surface or pin specified in the customer defaults by the Custom3 type. */,
                CustomTypesCustom4/** A datum surface or pin specified in the customer defaults by the Custom4 type. */,
                CustomTypesCustom5/** A datum surface or pin specified in the customer defaults by the Custom5 type. */,
                CustomTypesCustom6/** A datum surface or pin specified in the customer defaults by the Custom6 type. */,
                CustomTypesCustom7/** A datum surface or pin specified in the customer defaults by the Custom7 type. */
            };

            /** Settings for the method used to define the control direction. */
            public: enum ControlMethodTypes
            {
                ControlMethodTypesPrincipalAxis/** A principal axis. Absolute X,Y, or Z */,
                ControlMethodTypesUseSectionPlane/** Not a principal axis, use Section Plane */
            };

            /** Settings for defining the solid type to create for the datum object. */
            public: enum SolidTypes
            {
                SolidTypesSphere/** Datum solid will be a sphere   */,
                SolidTypesCylinder/** Datum solid will be a cylinder */,
                SolidTypesCone/** Datum solid will be a cone     */,
                SolidTypesCuboid/** Datum solid will be a cuboid for Datum Surface only */,
                SolidTypesHollow/** Datum solid will be a hollow cylinder for Datum Surface only */,
                SolidTypesArrow/** Measure solid will be the default solid - an arrow shape */,
                SolidTypesNone/** No solid created for either Datum or Measure */,
                SolidTypesHourglass/** Measure solid will be an hourglass (double cone) shape */
            };

            /** Settings to define the creation direction. */
            public: enum CreationDirectionMethods
            {
                CreationDirectionMethodsDefault/** Use the default construction method */,
                CreationDirectionMethodsOpposite/** Reverse the construction orientation */
            };

            /** The type to specify if the hole or slot size was determined automatically, or if a user keyed in the values. */
            public: enum SizeMethodTypes
            {
                SizeMethodTypesAutomatic/** The hole or slot size was automatically computed. */,
                SizeMethodTypesManual/** User manually entered the hole or slot size. */
            };

            /** The plane to cut sections in to determine datum location. */
            public: enum PlaneMethodTypes
            {
                PlaneMethodTypesInferPlanes/** Determine the best XC,YC, or ZC intersection plane. */,
                PlaneMethodTypesParallelXCPlanes/** Datum is on the XC plane. */,
                PlaneMethodTypesParallelYCPlanes/** Datum is on the YC plane. */,
                PlaneMethodTypesParallelZCPlanes/** Datum is on the ZC plane. */,
                PlaneMethodTypesSelectPlanes/** Datum is on user selected planes. */,
                PlaneMethodTypesUnknown/** Used when plane method is unknown. Used with Connected Face Finder */
            };

            private: DatumCommonBuilderImpl * m_datumcommonbuilder_impl;
            private: friend class  _DatumCommonBuilderBuilder;
            protected: DatumCommonBuilder();
            public: ~DatumCommonBuilder();
            /**Returns  the custom name used to create the datum.  
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: NXString CustomTypeName
            (
            );
            /**Sets  the custom name used to create the datum.  
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetCustomTypeName
            (
                const NXString & customTypeName /** Custom datum type from customer defaults to use  */
            );
            /**Sets  the custom name used to create the datum.  
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            void SetCustomTypeName
            (
                const char * customTypeName /** Custom datum type from customer defaults to use  */
            );
            /**Returns  the indicator if this should be marked as a derived from another datum. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: bool Derived
            (
            );
            /**Sets  the indicator if this should be marked as a derived from another datum. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetDerived
            (
                bool derived /** derived */ 
            );
            /**Returns  the direction axis.  This defines the datum origin and specified axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Axis * DirectionAxis
            (
            );
            /**Sets  the direction axis.  This defines the datum origin and specified axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetDirectionAxis
            (
                NXOpen::Axis * directionAxis /** directionaxis */ 
            );
            /**Returns  the project along direction. Two coordinates can be specified and the third obtained from projection. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: bool ProjectAlongDirection
            (
            );
            /**Sets  the project along direction. Two coordinates can be specified and the third obtained from projection. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetProjectAlongDirection
            (
                bool projectAlongDirection /** projectalongdirection */ 
            );
            /**Returns  the section plane normal. This is sometimes referred to as the clamping plane. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Direction * SectionPlaneNormal
            (
            );
            /**Sets  the section plane normal. This is sometimes referred to as the clamping plane. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetSectionPlaneNormal
            (
                NXOpen::Direction * sectionPlaneNormal /** sectionplanenormal */ 
            );
            /**Returns  the additional references.  Use to define additional parts the datum connects. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Assemblies::SelectComponentList * AdditionalReferences
            (
            );
            /**Returns  the control method. The method for fixing the datum orientation. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Weld::DatumCommonBuilder::ControlMethodTypes ControlMethod
            (
            );
            /**Sets  the control method. The method for fixing the datum orientation. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetControlMethod
            (
                NXOpen::Weld::DatumCommonBuilder::ControlMethodTypes controlMethod /** controlmethod */ 
            );
            /**Returns  the solid body type specified. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Weld::DatumCommonBuilder::SolidTypes SolidType
            (
            );
            /**Sets  the solid body type specified. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetSolidType
            (
                NXOpen::Weld::DatumCommonBuilder::SolidTypes solidType /** solidtype */ 
            );
            /**Returns  the principal axis x. Used to specify datum is controlling the x axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: bool PrincipalAxisX
            (
            );
            /**Sets  the principal axis x. Used to specify datum is controlling the x axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetPrincipalAxisX
            (
                bool principalAxisX /** principalaxisx */ 
            );
            /**Returns  the principal axis y. Used to specify the datum is controlling the y axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: bool PrincipalAxisY
            (
            );
            /**Sets  the principal axis y. Used to specify the datum is controlling the y axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetPrincipalAxisY
            (
                bool principalAxisY /** principalaxisy */ 
            );
            /**Returns  the principal axis z. Used to specify the datum is controlling the z axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: bool PrincipalAxisZ
            (
            );
            /**Sets  the principal axis z. Used to specify the datum is controlling the z axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetPrincipalAxisZ
            (
                bool principalAxisZ /** principalaxisz */ 
            );
            /**Returns  the Measure tolerance for X.   
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : None */
            public: double XTolerance
            (
            );
            /**Sets  the Measure tolerance for X.   
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetXTolerance
            (
                double xTolerance /** xtolerance */ 
            );
            /**Returns  the Measure tolerance for Y.   
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : None */
            public: double YTolerance
            (
            );
            /**Sets  the Measure tolerance for Y.   
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetYTolerance
            (
                double xTolerance /** xtolerance */ 
            );
            /**Returns  the Measure tolerance for Z.   
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : None */
            public: double ZTolerance
            (
            );
            /**Sets  the Measure tolerance for Z.   
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetZTolerance
            (
                double xTolerance /** xtolerance */ 
            );
            /**Returns  the characteristics. Used to specify additional attributes. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Weld::CharacteristicsBuilder * Characteristics
            (
            );
            /**Returns  the associative flag. Indicates if resulting object should move if an input reference is moved. 
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : None */
            public: bool Associative
            (
            );
            /**Sets  the associative flag. Indicates if resulting object should move if an input reference is moved. 
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetAssociative
            (
                bool associative /** associative */ 
            );
            /**Returns  the modeling distance tolerance. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: double ModelingTolerance
            (
            );
            /**Sets  the modeling distance tolerance. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetModelingTolerance
            (
                double modelingTolerance /** modelingtolerance */ 
            );
            /**Returns  the plane height along the direction axis. Controls the boundary of a datum plane. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: double PlaneHeight
            (
            );
            /**Sets  the plane height along the direction axis. Controls the boundary of a datum plane. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetPlaneHeight
            (
                double planeHeight /** planeheight */ 
            );
            /**Returns  the plane width perpendicular to the direction axis. Controls the boundary of a datum plane. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: double PlaneWidth
            (
            );
            /**Sets  the plane width perpendicular to the direction axis. Controls the boundary of a datum plane. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetPlaneWidth
            (
                double planeWidth /** planewidth */ 
            );
            /**Returns  the length of the datum axis vector created. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: double DirectionLength
            (
            );
            /**Sets  the length of the datum axis vector created. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetDirectionLength
            (
                double directionLength /** directionlength */ 
            );
            /**Returns  the creation direction. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Weld::DatumCommonBuilder::CreationDirectionMethods CreationDirection
            (
            );
            /**Sets  the creation direction. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetCreationDirection
            (
                NXOpen::Weld::DatumCommonBuilder::CreationDirectionMethods creationDirection /** creationdirection */ 
            );
            /**Returns  the method used in setting the hole or slot size. 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Weld::DatumCommonBuilder::SizeMethodTypes SizeMethodType
            (
            );
            /**Sets  the method used in setting the hole or slot size. 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetSizeMethodType
            (
                NXOpen::Weld::DatumCommonBuilder::SizeMethodTypes sizeMethodType /** sizemethodtype */ 
            );
            /**Returns  the radius of the solid sphere, cylinder or cone created.   
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: double CustomRadius
            (
            );
            /**Sets  the radius of the solid sphere, cylinder or cone created.   
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetCustomRadius
            (
                double customRadius /** customradius */ 
            );
            /**Returns  the radius expression used for Datum Pins.  
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Expression * RadiusExp
            (
            );
            /**Returns  the rotation angle of the cuboid solid around the normal axis.  
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Expression * RotationAngle
            (
            );
            /**Returns  the length above the datum reference point. This is used if a cylinder, cone, or cuboid (Surface locator only) are created. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: double CustomAboveLength
            (
            );
            /**Sets  the length above the datum reference point. This is used if a cylinder, cone, or cuboid (Surface locator only) are created. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetCustomAboveLength
            (
                double directionLength /** directionlength */ 
            );
            /**Returns  the total length of the cylinder, cone, or cuboid (Surface locator only) along the direction axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: double CustomTotalLength
            (
            );
            /**Sets  the total length of the cylinder, cone, or cuboid (Surface locator only) along the direction axis. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetCustomTotalLength
            (
                double customTotalLength /** customtotallength */ 
            );
            /**Returns  the slot length. 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : None */
            public: double SlotLength
            (
            );
            /**Sets  the slot length. 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetSlotLength
            (
                double slotLength /** slotlength */ 
            );
            /**Returns  the slot width. 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : None */
            public: double SlotWidth
            (
            );
            /**Sets  the slot width. 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetSlotWidth
            (
                double slotWidth /** slotwidth */ 
            );
            /**Returns  the hole diameter. 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : None */
            public: double HoleDiameter
            (
            );
            /**Sets  the hole diameter. 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetHoleDiameter
            (
                double holeDiameter /** holediameter */ 
            );
            /**Returns  the option to control if a reference datum plane should be output with the this feature. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: bool CreatePlane
            (
            );
            /**Sets  the option to control if a reference datum plane should be output with the this feature. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetCreatePlane
            (
                bool createPlane /** createplane */ 
            );
            /**Returns  the option to control if a reference datum axis should be output with this feature. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: bool CreateDirectionVector
            (
            );
            /**Sets  the option to control if a reference datum axis should be output with this feature. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetCreateDirectionVector
            (
                bool createDirectionVector /** createdirectionvector */ 
            );
            /**Returns  the option to control if a reference point should be output with this feature. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : None */
            public: bool CreatePoint
            (
            );
            /**Sets  the option to control if a reference point should be output with this feature. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetCreatePoint
            (
                bool createPoint /** createpoint */ 
            );
            /** Initialize the builder with the inputs from an existing datum. The builder type and derived datum type must be the same. 
             <br>  Created in NX8.5.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void UpdateWithDerivedDatum
            (
            );
            /**Returns  the derived parent ID for a locator created from a reference locator 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : None */
            public: NXString DerivedParentID
            (
            );
            /**Sets  the derived parent ID for a locator created from a reference locator 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetDerivedParentID
            (
                const NXString & parentDatumID /** The string value of the reference locator's ID attribute */
            );
            /**Sets  the derived parent ID for a locator created from a reference locator 
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            void SetDerivedParentID
            (
                const char * parentDatumID /** The string value of the reference locator's ID attribute */
            );
            /**Returns  the custom datum type. This corresponds to an entry in the customer defaults. 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Weld::DatumCommonBuilder::CustomTypes CustomType
            (
            );
            /**Sets  the custom datum type. This corresponds to an entry in the customer defaults. 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetCustomType
            (
                NXOpen::Weld::DatumCommonBuilder::CustomTypes customType /** customtype */ 
            );
            /**Returns   the plane method for the datum edge location 
             <br>  Created in NX2312.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Weld::DatumCommonBuilder::PlaneMethodTypes PlaneMethod
            (
            );
            /**Sets   the plane method for the datum edge location 
             <br>  Created in NX2312.0.0.  <br>  
             <br> License requirements : tooling_locators ("Tooling Locators") */
            public: void SetPlaneMethod
            (
                NXOpen::Weld::DatumCommonBuilder::PlaneMethodTypes planeMethod /** planemethod */ 
            );
            /**Returns   the datum edge intersection plane selected by the user 
             <br>  Created in NX2312.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::SelectNXObjectList * UserPlane
            (
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
