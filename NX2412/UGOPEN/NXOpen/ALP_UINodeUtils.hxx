//--------------------------------------------------------------------------
// Copyright 2025 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     ALP_UINodeUtils.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Part.hxx>
#include <NXOpen/TaggedObject.hxx>
#include <NXOpen/libalpopencpp_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace ALP
    {
        class UINodeUtils;
    }
    class Session;
    class NXObject;
    class Part;
    namespace ALP
    {
        class UINodeUtilsImpl;
        /** Represents a utils of assembly line planner for journaling uinode methods  <br> Use the static method in this class to obtain an instance.  <br> 
         <br>  Created in NX1926.0.0.  <br>  
        */
        class ALPOPENCPPEXPORT  UINodeUtils
        {
            private: UINodeUtilsImpl * m_uinodeutils_impl;
            private: NXOpen::Session* m_owner;
            /// \cond NX_NO_DOC 
            public: explicit UINodeUtils(NXOpen::Session *owner);


            /// \endcond 
            public: 
            /**Returns the tag of this object.  */
            tag_t Tag() const; 
            private: static UINodeUtils* theUINodeUtils;
            /** Returns the UINodeUtils object for the running session which serves as the 'gateway' class for the application API.  
                References to all other objects in this API are obtained either directly or indirectly via methods and properties on this class.  
                Platform Session should be initialized using 'GetSession' method from NXOpen API prior to this call.  
            */
            public: static UINodeUtils* GetUINodeUtils(NXOpen::Session *owner);
            public: ~UINodeUtils();
            /** To get item revision name 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: NXString GetItemRevisionName
            (
                NXOpen::Part * partTag /** parttag */ 
            );
            /** To assign product And station
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void ProductAndStationAssignment
            (
                const std::vector<NXOpen::NXObject *> & selectedProduct /** selectedproduct */ ,
                const std::vector<NXOpen::NXObject *> & selectedStation /** selectedstation */ ,
                const std::vector<NXOpen::NXObject *> & objectToRefresh /** objecttorefresh */ 
            );
            /** To unassign product And station
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void ProductAndStationUnassignment
            (
                const std::vector<NXOpen::NXObject *> & selectedProduct /** selectedproduct */ ,
                int unassignOption /** unassignoption */ ,
                const NXString & label /** label */ 
            );
            /** To unassign product And station
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            void ProductAndStationUnassignment
            (
                const std::vector<NXOpen::NXObject *> & selectedProduct /** selectedproduct */ ,
                int unassignOption /** unassignoption */ ,
                const char * label /** label */ 
            );
            /** To assign features to operation 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void AssignMfgFeature
            (
                const std::vector<NXOpen::NXObject *> & selectedFeature /** selectedfeature */ ,
                NXOpen::NXObject * operationPartOcc /** operationpartocc */ ,
                double totalWeldTime /** totalweldtime */ ,
                bool allowMultipleAssignment /** allowmultipleassignment */ 
            );
            /** To unassign features from operation 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void UnAssignMfgFeature
            (
                const std::vector<NXOpen::NXObject *> & selectedFeature /** selectedfeature */ ,
                const NXString & operationName /** operationname */ ,
                NXOpen::NXObject * selectedOpPartOcc /** selectedoppartocc */ 
            );
            /** To unassign features from operation 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            void UnAssignMfgFeature
            (
                const std::vector<NXOpen::NXObject *> & selectedFeature /** selectedfeature */ ,
                const char * operationName /** operationname */ ,
                NXOpen::NXObject * selectedOpPartOcc /** selectedoppartocc */ 
            );
            /** To Update the Operation Name 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void UpdateOperationName
            (
                NXOpen::NXObject * operationPartOcc /** operationpartocc */ ,
                const NXString & newOperationName /** newoperationname */ 
            );
            /** To Update the Operation Name 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            void UpdateOperationName
            (
                NXOpen::NXObject * operationPartOcc /** operationpartocc */ ,
                const char * newOperationName /** newoperationname */ 
            );
            /** To Update the Operation Time 
             <br>  Created in NX1926.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void UpdateOperationTime
            (
                NXOpen::NXObject * operationPartOcc /** operationpartocc */ ,
                double newOperationTime /** newoperationtime */ 
            );
            /** To set or unset an assembly as special end item 
             <br>  Created in NX1980.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void SetInContextEndItem
            (
                NXOpen::NXObject * operationPartOcc /** operationpartocc */ ,
                bool setOption /** setoption */ 
            );
            /** To add external resource 
             <br>  Created in NX1980.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void AddAdditionalResources
            (
                const std::vector<NXOpen::NXObject *> & targetCompResList /** targetcompreslist */ 
            );
            /** To remove external resource 
             <br>  Created in NX1980.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void RemoveAdditionalResources
            (
                const std::vector<NXOpen::NXObject *> & resToBeRemoved /** restoberemoved */ ,
                bool removingChildOfPackage /** removingchildofpackage */ 
            );
            /** To rotate a given component in clockwise or anticlockwise direction by 90 degrees 
             <br>  Created in NX2007.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void RotateComponent
            (
                const std::vector<NXOpen::NXObject *> & partOccTags /** partocctags */ ,
                bool isClockwise /** isclockwise */ ,
                bool rotateAsGroup /** rotateasgroup */ ,
                double angleStep /** anglestep */ 
            );
            /** To Clear Searched Features 
             <br>  Created in NX2007.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void ClearSearchedFeatures
            (
                NXOpen::NXObject * stationPartOccTag /** stationpartocctag */ 
            );
            /** To Move remaining un assigned features to next station
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void MoveUnAssignedFeaturesToNextStation
            (
                const std::vector<NXOpen::NXObject *> & mfgPartOccTags /** mfgpartocctags */ ,
                NXOpen::NXObject * currentStationPartOccTag /** currentstationpartocctag */ ,
                const NXString & nextStationName /** nextstationname */ 
            );
            /** To Move remaining un assigned features to next station
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            void MoveUnAssignedFeaturesToNextStation
            (
                const std::vector<NXOpen::NXObject *> & mfgPartOccTags /** mfgpartocctags */ ,
                NXOpen::NXObject * currentStationPartOccTag /** currentstationpartocctag */ ,
                const char * nextStationName /** nextstationname */ 
            );
            /** To show/hide features
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void ShowHideFeatures
            (
                bool isHidden /** ishidden */ 
            );
            /** To create new item
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: NXOpen::NXObject * CreateNewItem
            (
                const NXString & tempPartName /** temp part name */ ,
                const NXString & itemType /** itemtype */ ,
                const NXString & applicationName /** applicationname */ ,
                NXOpen::Part::Units partUnits /** partunits */ ,
                const NXString & dbPartNo /** dbpartno */ 
            );
            /** To create new item
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            NXOpen::NXObject * CreateNewItem
            (
                const char * tempPartName /** temp part name */ ,
                const char * itemType /** itemtype */ ,
                const char * applicationName /** applicationname */ ,
                NXOpen::Part::Units partUnits /** partunits */ ,
                const char * dbPartNo /** dbpartno */ 
            );
            /** To create new item
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: NXOpen::NXObject * CreateOperationFromLibrary
            (
                NXOpen::NXObject * operationObjectTag /** operationobjecttag */ ,
                NXOpen::NXObject * clonnedGroupTag /** clonnedgrouptag */ ,
                const NXString & objectName /** objectname */ 
            );
            /** To create new item
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            NXOpen::NXObject * CreateOperationFromLibrary
            (
                NXOpen::NXObject * operationObjectTag /** operationobjecttag */ ,
                NXOpen::NXObject * clonnedGroupTag /** clonnedgrouptag */ ,
                const char * objectName /** objectname */ 
            );
            /** To create new item
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void CreateOperationGroupFromLibrary
            (
                NXOpen::NXObject * operationObjectTag /** operationobjecttag */ ,
                const NXString & objectName /** objectname */ 
            );
            /** To create new item
             <br>  Created in NX2306.0.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            void CreateOperationGroupFromLibrary
            (
                NXOpen::NXObject * operationObjectTag /** operationobjecttag */ ,
                const char * objectName /** objectname */ 
            );
            /** To unassign features from all operations 
             <br>  Created in NX2412.3000.0.  <br>  
             <br> License requirements : alp_planner ("Assembly Line Planner") */
            public: void UnAssignMfgFeatureFromAllOperations
            (
                const std::vector<NXOpen::NXObject *> & selectedFeature /** selectedfeature */ 
            );
        }; //lint !e1712 default constructor not defined for class  

    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
