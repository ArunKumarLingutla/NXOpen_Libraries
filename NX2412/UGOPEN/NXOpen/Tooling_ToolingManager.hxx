//--------------------------------------------------------------------------
// Copyright 2024 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Tooling_ToolingManager.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#pragma once

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/TaggedObject.hxx>
#include <NXOpen/Tooling_ReusableObject.hxx>
#include <NXOpen/Tooling_SelectReuseLibraryListBuilder.hxx>
#include <NXOpen/libnxopencpp_tooling_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace Tooling
    {
        class ToolingManager;
    }
    class Part;
    namespace Tooling
    {
        class AssignPatchCollection;
    }
    namespace Tooling
    {
        class AutoDieCollection;
    }
    namespace Tooling
    {
        class BendInsertDesignCollection;
    }
    namespace Tooling
    {
        class BendOperationCollection;
    }
    namespace Tooling
    {
        class BlankGeneratorCollection;
    }
    namespace Tooling
    {
        class BlankLayoutCollection;
    }
    namespace Tooling
    {
        class BlankNestingCollection;
    }
    namespace Tooling
    {
        class BurringInsertCollection;
    }
    namespace Tooling
    {
        class CalculateAreaCollection;
    }
    namespace Tooling
    {
        class ChannelAdjustCollection;
    }
    namespace Tooling
    {
        class ChannelFittingCollection;
    }
    namespace Tooling
    {
        class ClearanceManagementCollection;
    }
    namespace Tooling
    {
        class ConceptPositionCollection;
    }
    namespace Tooling
    {
        class ConformalCoolingManager;
    }
    namespace Tooling
    {
        class CoolingConnectCollection;
    }
    namespace Tooling
    {
        class CoolingDefineChannelCollection;
    }
    namespace Tooling
    {
        class CoolingExtendCollection;
    }
    namespace Tooling
    {
        class CoolingPatternCollection;
    }
    namespace Tooling
    {
        class CornerDesignCollection;
    }
    namespace Tooling
    {
        class CreateBoxCollection;
    }
    namespace Tooling
    {
        class DesignTrimToolCollection;
    }
    namespace Tooling
    {
        class DieBaseCollection;
    }
    namespace Tooling
    {
        class DieDesignSettingCollection;
    }
    namespace Tooling
    {
        class DirectUnfoldCollection;
    }
    namespace Tooling
    {
        class EWMultiPositionCollection;
    }
    namespace Tooling
    {
        class EdgePatchCollection;
    }
    namespace Tooling
    {
        class EjectorPostProcessingCollection;
    }
    namespace Tooling
    {
        class ElectrodeCopyCollection;
    }
    namespace Tooling
    {
        class ElectrodeDesignCollection;
    }
    namespace Tooling
    {
        class EnlargeSurfaceCollection;
    }
    namespace Tooling
    {
        class ExplosionSimulationCollection;
    }
    namespace Tooling
    {
        class FaceColorManagementCollection;
    }
    namespace Tooling
    {
        class FaceSplitCollection;
    }
    namespace Tooling
    {
        class FamilyMoldCollection;
    }
    namespace Tooling
    {
        class FastenerAssemConfigCollection;
    }
    namespace Tooling
    {
        class FastenerAssyCollection;
    }
    namespace Tooling
    {
        class FastenerAssyCustomizationCollection;
    }
    namespace Tooling
    {
        class FastenerRemoveNodeCollection;
    }
    namespace Tooling
    {
        class ForceCalculationCollection;
    }
    namespace Tooling
    {
        class FormingInsertCollection;
    }
    namespace Tooling
    {
        class GuidedExtensionManager;
    }
    namespace Tooling
    {
        class HoleManufacturingNoteCollection;
    }
    namespace Tooling
    {
        class HoleReportCollection;
    }
    namespace Tooling
    {
        class HoleTableCollection;
    }
    namespace Tooling
    {
        class InitProjCollection;
    }
    namespace Tooling
    {
        class InitProjectCollection;
    }
    namespace Tooling
    {
        class InnerStripperPlateCollection;
    }
    namespace Tooling
    {
        class InsertAuxiliaryCollection;
    }
    namespace Tooling
    {
        class IntermediateStageCollection;
    }
    namespace Tooling
    {
        class LayoutManagementCollection;
    }
    namespace Tooling
    {
        class MWCopySheetsCollection;
    }
    namespace Tooling
    {
        class MWDefineRegionsCollection;
    }
    namespace Tooling
    {
        class MWDefineSheetsCollection;
    }
    namespace Tooling
    {
        class MWDesignGuideLinesCollection;
    }
    namespace Tooling
    {
        class MWDesignPartingSurfaceCollection;
    }
    namespace Tooling
    {
        class MWLayoutCollection;
    }
    namespace Tooling
    {
        class MWMoldedPartValidationManager;
    }
    namespace Tooling
    {
        class MWPartingManagerCollection;
    }
    namespace Tooling
    {
        class MWSearchRegionCollection;
    }
    namespace Tooling
    {
        class MWTraversePartingLinesCollection;
    }
    namespace Tooling
    {
        class ManufacturingGeometryCollection;
    }
    namespace Tooling
    {
        class MoldCsysCollection;
    }
    namespace Tooling
    {
        class MoldDesignCollection;
    }
    namespace Tooling
    {
        class MoldInsertCollection;
    }
    namespace Tooling
    {
        class MoldProcessCollection;
    }
    namespace Tooling
    {
        class MoldwizardManager;
    }
    namespace Tooling
    {
        class MotionSimulationCollection;
    }
    namespace Tooling
    {
        class ObjectAttributeManagementCollection;
    }
    namespace Tooling
    {
        class PiercingInsertCollection;
    }
    namespace Tooling
    {
        class PocketCollection;
    }
    namespace Tooling
    {
        class ProfileSplitCollection;
    }
    namespace Tooling
    {
        class ProgressiveDieManager;
    }
    namespace Tooling
    {
        class QuickQuotationCollection;
    }
    namespace Tooling
    {
        class ReferenceBlendCollection;
    }
    namespace Tooling
    {
        class ReliefDesignCollection;
    }
    namespace Tooling
    {
        class ReplaceSolidCollection;
    }
    namespace Tooling
    {
        class ReusableObject;
    }
    namespace Tooling
    {
        class ReusableObjectManager;
    }
    namespace Tooling
    {
        class ReusablePocketCollection;
    }
    namespace Tooling
    {
        class ReuseStandardPartCollection;
    }
    namespace Tooling
    {
        class RunnerCollection;
    }
    namespace Tooling
    {
        class SIZERCollection;
    }
    namespace Tooling
    {
        class ScrapDesignCollection;
    }
    namespace Tooling
    {
        class SelectReuseLibraryListBuilder;
    }
    namespace Tooling
    {
        class SolidPatchCollection;
    }
    namespace Tooling
    {
        class SpecifyBaffleCollection;
    }
    namespace Tooling
    {
        class SpecifyCircuitCollection;
    }
    namespace Tooling
    {
        class SplitSolidCollection;
    }
    namespace Tooling
    {
        class StandardPartCollection;
    }
    namespace Tooling
    {
        class StandardPartPositionCollection;
    }
    namespace Tooling
    {
        class StockSizeCollection;
    }
    namespace Tooling
    {
        class StripLayoutCollection;
    }
    namespace Tooling
    {
        class SubInsertCollection;
    }
    namespace Tooling
    {
        class ToolingDrawingCollection;
    }
    namespace Tooling
    {
        class TraverseLoopCollection;
    }
    namespace Tooling
    {
        class TrimMoldComponentsCollection;
    }
    namespace Tooling
    {
        class TrimRegionCollection;
    }
    namespace Tooling
    {
        class TrimSolidCollection;
    }
    namespace Tooling
    {
        class UndersizeCollection;
    }
    namespace Tooling
    {
        class UnusedFileManagementCollection;
    }
    namespace Tooling
    {
        class ValidCheckCollection;
    }
    namespace Tooling
    {
        class ValidDFMCollection;
    }
    namespace Tooling
    {
        class WallThicknessCheckerManager;
    }
    namespace Tooling
    {
        class WireHoleCollection;
    }
    namespace Tooling
    {
        class WorkflowManagementCollection;
    }
    namespace Tooling
    {
        class WorkpieceCollection;
    }
    namespace Tooling
    {
        /** Wizard type, some of them should be same as in mpdwshare_uf_mold.h */
        enum ToolingApplication
        {
            ToolingApplicationReuseLibrary/** Reuse Library */,
            ToolingApplicationMoldWizard/** MoldWizard    */,
            ToolingApplicationPdieWizard/** Progressive Die Wizard */,
            ToolingApplicationStampingDie/** Stamping Die */,
            ToolingApplicationElectrodeWizard/** Electrode Wizard */,
            ToolingApplicationEdieWizard/** Engineering Die Wizard */,
            ToolingApplicationPdcastingWizard/** Pressure Die Casting Wizard */
        };

        /** Wizard type status of MoldWizard and Pressure Die Casting Wizard */
        enum ToolingApplicationstatus
        {
            ToolingApplicationstatusMoldwizardAndPdcwOff/** Both MoldWizard and Pressure Die Casting Wizard application OFF*/,
            ToolingApplicationstatusMoldwizardOn/** MoldWizard application ON   */,
            ToolingApplicationstatusPdcastingWizardOn/** Pressure Die Casting Wizard application ON*/
        };

        /** the standard part base data types. */
        enum ToolingBasedatatype
        {
            ToolingBasedatatypeKruCommonData/** kru common data */ ,
            ToolingBasedatatypeMwStandardPartData/** mw standard part data */ ,
            ToolingBasedatatypeMwMoldBaseData/** mw mold base data */ ,
            ToolingBasedatatypeMwSliderData/** mw slider data */ ,
            ToolingBasedatatypeMwCoolingData/** mw cooling data */ ,
            ToolingBasedatatypeMwInsertData/** mw insert data */ ,
            ToolingBasedatatypeMwEjectorData/** mw ejector data */ ,
            ToolingBasedatatypeMwFillData/** mw fill data */ ,
            ToolingBasedatatypeMwPartFamilyData/** mw part family data */ ,
            ToolingBasedatatypeMwProjectTemplateData/** mw project template data */ ,
            ToolingBasedatatypePdwStandardPartData/** pdw standard part data */  = 11,
            ToolingBasedatatypePdwPiercingInsertData/** pdw piercing insert data */ ,
            ToolingBasedatatypePdwBendingInsertData/** pdw bending insert data */ ,
            ToolingBasedatatypePdwBurringInsertData/** pdw burring insert data */ ,
            ToolingBasedatatypePdwMountInsertData/** pdw mount insert data */ ,
            ToolingBasedatatypePdwPartFamilyData/** pdw part family data */ ,
            ToolingBasedatatypePdwDieBaseData/** pdw die base data */ ,
            ToolingBasedatatypePdwFormingInsertData/** pdw forming insert data */ ,
            ToolingBasedatatypePdwChamferingInsertData/** pdw chamfering insert data */ ,
            ToolingBasedatatypePdwLetteringInsertData/** pdw lettering insert data */ ,
            ToolingBasedatatypeEdwStandardPartData/** edw standard part data */ ,
            ToolingBasedatatypeEdwPiercingInsertData/** edw piercing insert data */ ,
            ToolingBasedatatypeEdwBendingInsertData/** edw bending insert data */ ,
            ToolingBasedatatypeEdwBurringInsertData/** edw burring insert data */ ,
            ToolingBasedatatypeEdwMountInsertData/** edw mount insert data */ ,
            ToolingBasedatatypeEdwPartFamilyData/** edw part family data */ ,
            ToolingBasedatatypeEdwFormingInsertData/** edw forming insert data */ ,
            ToolingBasedatatypeEdwChamferingInsertData/** edw chamfering insert data */ ,
            ToolingBasedatatypeEdwLetteringInsertData/** edw lettering insert data */ ,
            ToolingBasedatatypeEdHolderData/** ed holder data */  = 41,
            ToolingBasedatatypeEdPalletData/** ed pallet data */ ,
            ToolingBasedatatypeEdBlankData/** ed blank data */ ,
            ToolingBasedatatypePdcwStandardPartData/** pdcw standard part data */ ,
            ToolingBasedatatypePdcwMoldBaseData/** pdcw mold base data */ ,
            ToolingBasedatatypePdcwSliderData/** pdcw slider data */ ,
            ToolingBasedatatypePdcwCoolingData/** pdcw cooling data */ ,
            ToolingBasedatatypePdcwInsertData/** pdcw insert data */ ,
            ToolingBasedatatypePdcwEjectorData/** pdcw ejector data */ ,
            ToolingBasedatatypePdcwFillData/** pdcw fill data */ ,
            ToolingBasedatatypePdcwPartFamilyData/** pdcw part family data */ ,
            ToolingBasedatatypePdcwProjectTemplateData/** pdcw project template data */ ,
            ToolingBasedatatypePdwCounterboringInsertData/** pdw counterboring insert data */ ,
            ToolingBasedatatypePdwProjectTemplateData/** pdw project template data */ ,
            ToolingBasedatatypeEdwProjectTemplateData/** edw project template data */ ,
            ToolingBasedatatypeEdProjectTemplateData/** ed project template data */ ,
            ToolingBasedatatypeSdStandardPartData/** sd standard part data */ ,
            ToolingBasedatatypeSdReusableFeatureData/** sd reusable feature data */ ,
            ToolingBasedatatypeStandardPartDataLast/** standard part data last */ 
        };

    }
    namespace Tooling
    {
        class ToolingManagerImpl;
        /** Contains the collection objects for creating and iterating over Reuse Library objects.  <br> To obtain an instance of this class, refer to @link NXOpen::Part  NXOpen::Part @endlink  <br> 
         <br>  Created in NX6.0.0.  <br>  
        */
        class NXOPENCPP_TOOLINGEXPORT  ToolingManager
        {
            private: ToolingManagerImpl * m_toolingmanager_impl;
            private: NXOpen::Part* m_owner;
            /// \cond NX_NO_DOC 
            public: explicit ToolingManager(NXOpen::Part *owner);


            /// \endcond 
            public: 
            /**Returns the tag of this object.  */
            tag_t Tag() const; 
            public: ~ToolingManager();
            /** Creates a @link  NXOpen::Tooling::ReusableObject   NXOpen::Tooling::ReusableObject @endlink   @return @link  NXOpen::Tooling::ReusableObject   NXOpen::Tooling::ReusableObject @endlink  
             <br>  Created in NX7.5.2.  <br>  
             <br> License requirements : None */
            public: NXOpen::Tooling::ReusableObject * CreateReusableObject
            (
            );
            /** Creates a @link  NXOpen::Tooling::SelectReuseLibraryListBuilder   NXOpen::Tooling::SelectReuseLibraryListBuilder @endlink   @return @link  NXOpen::Tooling::SelectReuseLibraryListBuilder   NXOpen::Tooling::SelectReuseLibraryListBuilder @endlink  
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Tooling::SelectReuseLibraryListBuilder * CreateSelectReuseLibraryListBuilder
            (
            );
            /** Destroys a @link  NXOpen::Tooling::SelectReuseLibraryListBuilder   NXOpen::Tooling::SelectReuseLibraryListBuilder @endlink  
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void DestroySelectReuseLibraryListBuilder
            (
                NXOpen::Tooling::SelectReuseLibraryListBuilder * reuseListBuilder /**@link  NXOpen::Tooling::SelectReuseLibraryListBuilder   NXOpen::Tooling::SelectReuseLibraryListBuilder @endlink  */
            );


            /** Returns the MWSearchRegionCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::MWSearchRegionCollection *MWSearchRegions();
            /** Returns the MWDefineRegionsCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::MWDefineRegionsCollection *MWDefineRegions();
            /** Returns the MWDefineSheetsCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::MWDefineSheetsCollection *MWDefineSheets();
            /** Returns the MWCopySheetsCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::MWCopySheetsCollection *MWCopySheets();
            /** Returns the MWPartingManagerCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::MWPartingManagerCollection *MWPartingManager();
            /** Returns the MWDesignPartingSurfaceCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::MWDesignPartingSurfaceCollection *MWDesignPartingSurface();
            /** Returns the MWTraversePartingLinesCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::MWTraversePartingLinesCollection *MWTraversePartingLines();
            /** Returns the MWDesignGuideLinesCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::MWDesignGuideLinesCollection *MWDesignGuideLines();
            /** Returns the MWMoldedPartValidationManager instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::MWMoldedPartValidationManager *MWMoldedPartValidation();
            /** Returns the WallThicknessCheckerManager instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::WallThicknessCheckerManager *WallThicknessChecker();
            /** Returns the GuidedExtensionManager instance belonging to this part 
             <br>  Created in NX9.0.0.  <br>  
            */
            public: NXOpen::Tooling::GuidedExtensionManager *GuidedExtension();
            /** Returns the ValidCheckCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::ValidCheckCollection *ValidCheck();
            /** Returns the DirectUnfold instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::DirectUnfoldCollection *DirectUnfold();
            /** Returns the StockSizeCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::StockSizeCollection *StockSizes();
            /** Returns the MoldCsysCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::MoldCsysCollection *MoldCsys();
            /** Returns the FamilyMoldCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::FamilyMoldCollection *FamilyMolds();
            /** Returns the PocketCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::PocketCollection *Pockets();
            /** Returns the ProfileSplitCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::ProfileSplitCollection *ProfileSplits();
            /** Returns the InitProjectCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::InitProjectCollection *InitializeProjects();
            /** Returns the PiercingInsert instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::PiercingInsertCollection *PiercingInsert();
            /** Returns the StripLayout instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::StripLayoutCollection *StripLayout();
            /** Returns the MWLayoutCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::MWLayoutCollection *Layouts();
            /** Returns the SubInsertCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::SubInsertCollection *SubInserts();
            /** Returns the ScrapDesignCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::ScrapDesignCollection *ScrapDesign();
            /** Returns the UndersizeCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::UndersizeCollection *Undersizes();
            /** Returns the WorkpieceCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::WorkpieceCollection *Workpieces();
            /** Returns the ElectrodeCopyCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::ElectrodeCopyCollection *ElectrodeCopys();
            /** Returns the InitProjCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::InitProjCollection *InitProj();
            /** Returns the ConceptPositionCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::ConceptPositionCollection *ConceptPositions();
            /** Returns the WireHole instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::WireHoleCollection *WireHole();
            /** Returns the HoleReport instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::HoleReportCollection *HoleReport();
            /** Returns the ReliefDesign instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::ReliefDesignCollection *ReliefDesign();
            /** Returns the ForceCalculation instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::ForceCalculationCollection *ForceCalculation();
            /** Returns the BendInsertDesign instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::BendInsertDesignCollection *BendInsertDesign();
            /** Returns the UnusedFileManagement instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::UnusedFileManagementCollection *UnusedFileManagement();
            /** Returns the CornerDesign instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::CornerDesignCollection *CornerDesign();
            /** Returns the MoldInsertCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::MoldInsertCollection *MoldInserts();
            /** Returns the MoldProcessCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::MoldProcessCollection *MoldProcesses();
            /** Returns the FastenerAssyCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::FastenerAssyCollection *FastenerAssembly();
            /** Returns the FastenerAssemConfigCollection instance belonging to this part 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Tooling::FastenerAssemConfigCollection *FastenerAssemConfig();
            /** Returns the ReferenceBlendCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::ReferenceBlendCollection *ReferenceBlends();
            /** Returns the CalculateAreaCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::CalculateAreaCollection *CalculateAreas();
            /** Returns the FaceSplitCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::FaceSplitCollection *FaceSplits();
            /** Returns the TraverseLoopCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::TraverseLoopCollection *TraverseLoops();
            /** Returns the EnlargeSurfaceCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::EnlargeSurfaceCollection *EnlargeSurfaces();
            /** Returns the SolidPatchCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::SolidPatchCollection *SolidPatchs();
            /** Returns the SplitSolidCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::SplitSolidCollection *SplitSolids();
            /** Returns the EdgePatchCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::EdgePatchCollection *EdgePatchs();
            /** Returns the AssignPatchCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::AssignPatchCollection *AssignPatchs();
            /** Returns the TrimRegionCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::TrimRegionCollection *TrimRegions();
            /** Returns the CoolingPatternCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::CoolingPatternCollection *CoolingPattern();
            /** Returns the CoolingDefineChannelCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::CoolingDefineChannelCollection *CoolingDefineChannel();
            /** Returns the ChannelAdjustCollection instance belonging to this part
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::ChannelAdjustCollection *ChannelAdjust();
            /** Returns the CoolingConnectCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::CoolingConnectCollection *CoolingConnect();
            /** Returns the BlankGeneratorCollection instance belonging to this part
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::BlankGeneratorCollection *BlankGenerator();
            /** Returns the BlankLayoutCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::BlankLayoutCollection *BlankLayout();
            /** Returns the DieDesignSettingCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::DieDesignSettingCollection *DieDesignSetting();
            /** Returns the EWMultiPositionCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::EWMultiPositionCollection *EWMultiPositions();
            /** Returns the FormingInsertCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::FormingInsertCollection *FormingInsert();
            /** Returns the InsertAuxiliaryCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::InsertAuxiliaryCollection *InsertAuxiliary();
            /** Returns the EjectorPostProcessingCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::EjectorPostProcessingCollection *EjectorPostProcessings();
            /** Returns the TrimMoldComponentsCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::TrimMoldComponentsCollection *TrimMoldComponents();
            /** Returns the DesignTrimToolCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::DesignTrimToolCollection *DesignTrimTools();
            /** Returns the CoolingExtendCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::CoolingExtendCollection *CoolingExtend();
            /** Returns the SIZERCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::SIZERCollection *SIZER();
            /** Returns the RunnerCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::RunnerCollection *Runner();
            /** Returns the MotionSimulationCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::MotionSimulationCollection *MotionSimulation();
            /** Returns the ReusablePocketCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::ReusablePocketCollection *ReusablePocket();
            /** Returns the ChannelFittingCollection instance belonging to this part 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Tooling::ChannelFittingCollection *ChannelFitting();
            /** Returns the AutoDieCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::AutoDieCollection *AutoDie();
            /** Returns the ManufacturingGeometryCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::ManufacturingGeometryCollection *ManufacturingGeometry();
            /** Returns the BuringInsertCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::BurringInsertCollection *BurringInsert();
            /** Returns the ClearanceManagementCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::ClearanceManagementCollection *ClearanceManagement();
            /** Returns the WorkflowManagementCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::WorkflowManagementCollection *WorkflowManagement();
            /** Returns the LayoutManagementCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::LayoutManagementCollection *LayoutManagement();
            /** Returns the ReusableObjectManager instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::ReusableObjectManager *ReusableObjectManager();
            /** Returns the ToolingDrawingCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::ToolingDrawingCollection *ToolingDrawing();
            /** Returns the TrimSolidCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::TrimSolidCollection *TrimSolid();
            /** Returns the DieBaseCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::DieBaseCollection *DieBase();
            /** Returns the SpecifyBaffleCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::SpecifyBaffleCollection *SpecifyBaffle();
            /** Returns the ElectrodeDesignCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::ElectrodeDesignCollection *ElectrodeDesign();
            /** Returns the FastenerAssyCustomizationCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::FastenerAssyCustomizationCollection *FastenerAssyCustomization();
            /** Returns the CreateBoxCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::CreateBoxCollection *CreateBox();
            /** Returns the ReplaceSolidCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::ReplaceSolidCollection *ReplaceSolid();
            /** Returns the SpecifyCircuitCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::SpecifyCircuitCollection *SpecifyCircuit();
            /** Returns the StandardPartCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::StandardPartCollection *StandardPart();
            /** Returns the QuickQuotationCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::QuickQuotationCollection *QuickQuotation();
            /** Returns the FastenerRemoveNodeCollection instance belonging to this part 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Tooling::FastenerRemoveNodeCollection *FastenerRemoveNode();
            /** Returns the MoldDesignCollection instance belonging to this part 
             <br>  Created in NX8.5.0.  <br>  
            */
            public: NXOpen::Tooling::MoldDesignCollection *MoldDesign();
            /** Returns the IntermediateStageCollection instance belonging to this part 
             <br>  Created in NX8.5.0.  <br>  
            */
            public: NXOpen::Tooling::IntermediateStageCollection *IntermediateStage();
            /** Returns the BendOperationCollection instance belonging to this part 
             <br>  Created in NX8.5.0.  <br>  
            */
            public: NXOpen::Tooling::BendOperationCollection *BendOperation();
            /** Returns the StandardPartCollection instance belonging to this part 
             <br>  Created in NX8.5.0.  <br>  
            */
            public: NXOpen::Tooling::StandardPartPositionCollection *StandardPartPosition();
            /** Returns the Progressive Die Manager for this part 
             <br>  Created in NX8.5.0.  <br>  
            */
            public: NXOpen::Tooling::ProgressiveDieManager *ProgressiveDie();
            /** Returns the BlankNestingCollection instance belonging to this part 
             <br>  Created in NX8.5.0.  <br>  
            */
            public: NXOpen::Tooling::BlankNestingCollection *BlankNesting();
            /** Returns the moldwizard manager for this part 
             <br>  Created in NX9.0.0.  <br>  
            */
            public: NXOpen::Tooling::MoldwizardManager *Moldwizard();
            /** Returns the ObjectAttributeManagementCollection instance belonging to this part 
             <br>  Created in NX9.0.0.  <br>  
            */
            public: NXOpen::Tooling::ObjectAttributeManagementCollection *ObjectAttributeManagement();
            /** Returns the FaceColorManagementCollection instance belonging to this part 
             <br>  Created in NX9.0.0.  <br>  
            */
            public: NXOpen::Tooling::FaceColorManagementCollection *FaceColorManagement();
            /** Returns the HoleManufacturingNoteCollection instance belonging to this part 
             <br>  Created in NX9.0.0.  <br>  
            */
            public: NXOpen::Tooling::HoleManufacturingNoteCollection *HoleManufacturingNote();
            /** Returns the HoleTableCollection instance belonging to this part 
             <br>  Created in NX1899.0.0.  <br>  
            */
            public: NXOpen::Tooling::HoleTableCollection *HoleTable();
            /** Returns the ConformalCoolingManager instance belonging to this part 
             <br>  Created in NX1899.0.0.  <br>  
            */
            public: NXOpen::Tooling::ConformalCoolingManager *ConformalCooling();
            /** Returns the ValidDFMCollection instance belonging to this part 
             <br>  Created in NX1953.0.0.  <br>  
            */
            public: NXOpen::Tooling::ValidDFMCollection *ValidDFM();
            /** Returns the InnerStripperPlateCollection instance belonging to this part 
             <br>  Created in NX2206.0.0.  <br>  
            */
            public: NXOpen::Tooling::InnerStripperPlateCollection *InnerStripperPlate();
            /** Creates a @link  NXOpen::Tooling::ReuseStandardPartCollection   NXOpen::Tooling::ReuseStandardPartCollection @endlink  
             <br>  Created in NX2312.0.0.  <br>  
            */
            public: NXOpen::Tooling::ReuseStandardPartCollection *ReuseStandardPart();
            /** Returns the ExplosionSimulationCollection instance belonging to this part 
             <br>  Created in NX2412.0.0.  <br>  
            */
            public: NXOpen::Tooling::ExplosionSimulationCollection *ExplosionSimulation();
        }; //lint !e1712 default constructor not defined for class  

    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
