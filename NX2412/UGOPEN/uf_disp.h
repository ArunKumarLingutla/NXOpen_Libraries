...
In NX11, we introduced a new high end visualization material type - Lightworks Iray+ material.  
These materials are used with the new Iray+ RTS renderer.  The old Author materials 
are used with the Lightworks Author RTS renderer. 
Some of the existing UF_DISP APIs now support both types of materials, some just support 
the Lightworks Author materials particularly the ones that deal with Lightworks lwa archive 
material libraries and some work with materials of the current renderer type.  The later means if user 
is using the new Iray+ RTS renderer, then these APIs will work on Iray+ materials.  
The API support information is provided in the comment area of each of these functions. 
***************************************************************************/
...
/*  */
/* UF_SH_MAX_MAT_NAME_SIZE is being deprecated because NX is moving to support
                           multibyte characters. User written programs should start using the new definitions
                           below:
   UF_SF_MAX_MAT_NAME_BUFSIZE should be used for buffer allocations and memory allocations.
   UF_SF_MAX_MAT_NAME_NCHARS should be used to limit the number of characters in a buffer.
...
#define UF_SH_MAX_MAT_NAME_NCHARS 132
#define UF_SH_MAX_MAT_NAME_BUFSIZE NX_BUFSIZE(132) 

#define UF_SF_MAX_MAT_NAME_NCHARS 133
#define UF_SF_MAX_MAT_NAME_BUFSIZE  NX_BUFSIZE(UF_SF_MAX_MAT_NAME_NCHARS) 

/*  */
#define UF_SH_MAX_MAT_NAME_SIZE  (UF_SF_MAX_MAT_NAME_BUFSIZE-1)
/*  */
 ...
 * History:
*****************************************************************************/
extern UFUNEXPORT int UF_DISP_regenerate_display(
    void   
);
 ...
 * Environment: Internal
 * See Also:
 * History: This function was first released in NX 7.5.5
 * 
*****************************************************************************/
extern UFUNEXPORT int UF_DISP_set_highlights(
          int       object_count, /* <I> The number of objects in the object_list       */
          tag_p_t   object_list,  /* <I,len:object_count> object list to set highlights */
          int       action_switch /* <I> Action Switch
                                         0 = Turn Highlight Off
                                         1 = Turn Highlight On */
);
 ...
 * UF_DISP_CCM_HSV_DISTANCE - this method returns the color which is the
 * least distance away from the given color in the converted HSV color cube.
...
extern UFUNEXPORT int UF_DISP_ask_closest_color(
           ...
           int clr_cmp_mtd ,          /* <I>
                                      ...
                                      and UF_DISP_CCM_HSV_DISTANCE are defined.
                                      */
           int * clr_num              /* <O>
...
extern UFUNEXPORT int UF_DISP_set_srfanl_params(
     UF_DISP_srfanl_data_t * params  /* <I>
                                     New system display parameters
                                     */
);
...
extern UFUNEXPORT int UF_DISP_export_windows_metafile(
     ...
     char * file_spec                      /* <I>
                                           ...
                                           most MAX_FSPEC_NCHARS characters.
                                           */
);
...
extern UFUNEXPORT int UF_DISP_ask_material(
    ...
    char material_name[UF_SF_MAX_MAT_NAME_BUFSIZE]           /* <O> The name of the material */
 ...
 * 
 * Environment: Internal and External
 * History: This function was created in NX11.0.2
 *****************************************************************/
extern UFUNEXPORT int UF_DISP_ask_decals
(
    const tag_t object_tag,   /* <I> The tag of geometry object.  */
    int    *num_decals,/* <O> Number of decals associated to the geometry object*/
    tag_t **decal_materials/* <OF,len:num_decals> 
                              List of decal base materials or NULL if none found.
...
extern UFUNEXPORT int UF_DISP_ask_currently_selected_material(
    ...
    char  material_full_archive_name[ MAX_FSPEC_BUFSIZE ]          /* <O> The full_arachive_name of the material currently selected in the Materials in part 
...
extern UFUNEXPORT int UF_DISP_copy_material(
    ...
    char new_material_name[ MAX_FSPEC_BUFSIZE ]       /* <O> The name of the newly copied material*/
);
...
extern UFUNEXPORT int UF_DISP_update_material_display_of_geometry(
    ...
    const tag_p_t   object_list   /* <I,len:object_count> The tags of objects with material applied to */
);
...
extern UFUNEXPORT int UF_DISP_ask_geometry_of_material(
    ...
    tag_p_t     *object_tags    /* <OF,len:object_count> A pointer to a list of geometry object tags
 ...
 * Environment: Internal and external
 * See Also: 
 * History: This function is created for NX8.5.3
 *****************************************************************/
extern UFUNEXPORT int UF_DISP_open_LWA_archive_materials_library(
    const char* lwa_archive_library_name /*<I> The name including full path of the LWA archive material library to be opened.*/
);
 ...
 * History: This function is created for NX8.5.3
 *****************************************************************/
extern UFUNEXPORT int UF_DISP_copy_LWA_archive_material_to_work_part(
    const char*material_name, /*<I> The name of material in the currently opened archive material library to copy to the work part. */
    tag_p_t material_tag      /*<O> The tag of the copied material. */
);
 ...
 * History: This function is created for NX8.5.3
 *****************************************************************/
extern UFUNEXPORT int UF_DISP_remove_material_assignment(
    tag_t object_tag /*<I> The tag of the geometry object to remove the material from. */
);
 ...
 * Environment: Internal and external
 * See Also: Refer to  UF_DISP_face_facet_data_t
 * History: This function is created for NX11.0.1
*****************************************************************/
extern UFUNEXPORT int UF_DISP_get_facet_data(
    tag_t object_tag,   /* <I> The solid body or a face */
    int* num_faces,     /* <O> The number of faces returned */
    UF_DISP_face_facet_data_t** face_facet_data /* <OF> The facet geometry data array of all faces.
 ...
 * History: This function is created for NX11.0.1
*****************************************************************/
extern UFUNEXPORT int UF_DISP_free_temp_facet_data(void);
 ...
 * History: This function is created for NX11.0.1
*****************************************************************/
extern UFUNEXPORT int UF_DISP_generate_texture_coordinates(
       const UF_DISP_texture_space_info_p_t ts_info,/* <I> texture space info object. */
       const UF_DISP_face_facet_data_p_t face_facet_data, /* <I> facet data of a single face. */
       double **texture_coords   /* <OF> The texture coordinates of the face. This memory need to be freed by the caller. */
       );
...
* History: This function was created in NX11.0.2
*****************************************************************/
extern UFUNEXPORT int UF_DISP_ask_materials_in_part(
    tag_t part_tag,          /* <I> The part you want to get materials.*/
    UF_DISP_material_format_type_t material_format_type,  /* <I> What type of materials should return.*/
    int *material_count,         /* <O> Number of materials in the part */
    tag_p_t *material_tags,  /* <OF,len:material_count>  A pointer to a list of material tags contained in the part.  This must be freed by the caller using UF_free. */
    char ***material_names    /* <OF,len:material_count> A pointer to a list of material names contained in the part (corresponding to the list of material_tags).  This must be freed by the caller using UF_free. */
    );
...
* NOTE: This function only used in iray+ mode.
...
* Environment: Internal and external
* See Also: 
* History: This function is created for NX11.0.2
*****************************************************************/
extern UFUNEXPORT int UF_DISP_create_material(
    const char *studio_system_material_name, /* <I> The name of System Studio Material to create the new material from. */
    tag_p_t new_material_tag, /* <O> The tag of the newly created material. */
    char new_material_name[UF_SH_MAX_MAT_NAME_BUFSIZE]  /* <O> The name of the newly created material. */
    );

/******************************************************************************
* Return legacy material parameters
* Retrieves the specific parameters for a given legacy author material
* Provided a material tag, this function retrieves all the attributes and parameters
* of the associated legacy material and returns them via the legacyMatParams data structure.
* 
* Returns:  0 = Success
*           UF_err_bad_parameter_number_1: materialTag is null or invalid.
*           UF_err_program_not_initialized
*           UF_err_part_not_loaded
* 
* Environment: Internal
* History: This function is created for NX 2212
*****************************************************************/
extern UFUNEXPORT int UF_DISP_ask_legacy_material_parameters(
    tag_t materialTag, 
    UF_DISP_legacy_material_params_p_t legacyMatParams
    );
