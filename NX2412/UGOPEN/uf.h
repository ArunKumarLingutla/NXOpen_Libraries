...
File Description :
   ...
   Note: UF_free/UF_free_string_array is applicable for C/C++ only
...
enum UF_callback_reason_e
{
    ...
    UF_open_part_reason,      /* Callback when an existing part is opened.
                                 The part opened callback is called just after the "target parts
                                 of the load operation" (e.g. the top level part of an assembly
                                 and  its component parts have been loaded), but is only issued
                                 for the "target parts of the load operation".  This callback may
                                 be called if more data is loaded for an already partially
                                 loaded part.*/
