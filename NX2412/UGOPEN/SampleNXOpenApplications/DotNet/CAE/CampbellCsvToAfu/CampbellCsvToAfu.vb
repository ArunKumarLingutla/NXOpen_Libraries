...
Module CampbellCsvToAfu
    ...
    Public csvColNames As New Dictionary(Of Integer, String)
    ...
    Public Sol414 As Boolean = False
...
#End Region
    ...
    End Sub
    '------------------------------------------------------------------------------
    ' This method adapts the UI according to Sol414 context
    ' The CSV file produced by Sol414 contains a header "$ Written by SOL414"
    '------------------------------------------------------------------------------
    Public Sub SetSol414(ByVal CsvFile As String)
        ' Determine the origin of the csv file
        Sol414 = False
        If CsvFile <> "" Then
            Try
                Dim reader As FileIO.TextFieldParser = My.Computer.FileSystem.OpenTextFieldParser(csvFile)
                Dim line As String = reader.PeekChars(13).Trim
                If line = "$ Written by" Then
                    Sol414 = (trim(replace(reader.PeekChars(20).Trim, line, "")) = "SOL414")
                End If
                reader.Dispose()
            Catch ex As Exception
            End Try
        ...
        '
        ' Units
        enum0.GetProperties.SetLogical("Enable", True)
        enum01.GetProperties.SetLogical("Enable", True)
        ' Damping name
        toggle01.GetProperties.SetString("Label", "Damping Values")
        csvColNames(3) = "-Damping_Values"
        toggle03.GetProperties.SetString("Label", "Imaginary part of eigensolution*")
        'Whirl Direction
        toggle04.GetProperties.SetLogical("Enable", True)
        ' Converted System
        label01.GetProperties.SetLogical("Enable", True)
        toggle05.GetProperties.SetLogical("Enable", True)
        toggle06.GetProperties.SetLogical("Enable", True)
        toggle07.GetProperties.SetLogical("Enable", True)
        toggle08.GetProperties.SetLogical("Enable", True)
        label02.GetProperties.SetLogical("Enable", True)
        '
        ' Adapt UI according to Sol414 context
        '
        If Sol414 Then
            ...
            enum01.GetProperties.SetLogical("Enable", False)
            toggle01.GetProperties.SetString("Label", "Loss Factors")
            csvColNames(3) = "-Loss_Factor"
            toggle03.GetProperties.SetString("Label", "Imaginary part of eigensolution")
            'toggle04.GetProperties.SetLogical("Value", False)
            'toggle04.GetProperties.SetLogical("Enable", False)
            label01.GetProperties.SetLogical("Enable", False)
            toggle05.GetProperties.SetLogical("Value", False)
            toggle05.GetProperties.SetLogical("Enable", False)
            toggle06.GetProperties.SetLogical("Value", False)
            toggle06.GetProperties.SetLogical("Enable", False)
            toggle07.GetProperties.SetLogical("Value", False)
            toggle07.GetProperties.SetLogical("Enable", False)
            toggle08.GetProperties.SetLogical("Value", False)
            toggle08.GetProperties.SetLogical("Enable", False)
            label02.GetProperties.SetLogical("Enable", False)
        End If
    End Sub
    '------------------------------------------------------------------------------
