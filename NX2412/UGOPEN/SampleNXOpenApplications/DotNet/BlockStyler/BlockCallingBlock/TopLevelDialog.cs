...
public class TopLevelDialog
{
    ...
    {
        // Determine which mode we are running it.
        // By default we are in test mode
        // If we have any argments we are not in test mode.

        if(args.Length > 0 && args[0] != "")
        {
            Util.m_testMode = false;
        }
        ...
        try
        {
            ...
            if (Util.m_testMode)
            {
                oThread = new Thread(new ThreadStart(TopLevelDialog.DoPerformOK));
