...
--------------------------
    ...
    By default this will be run in TestMode.  To run not in TestMode you must pass a string parameter.
    ...
    follow the steps below:
        ...
        2.3 Invoke Shared Library BlockCallingBlock.dll through  File->Execute->NX Open Menu (Ctrl+U).  Enter any string using the Enter
        User Function Arguments dialog.
        
        2.4 This will launch the TopDialog dialog.  Select "Select Face" button. Which will bring up a dialog to select a Face.  
        Select a single Face, and then hit "OK".  
        
        2.5 Repeat Step 2.4 and select a different face.

        2.6 Hit "OK" and see the faces will be changed to the color on the TopDialog.

    To Run this in "TestMode"

        NOTE : Do not pass in arguments to make sure this runs in test mode.
        ...
        2.4 This will launch the TopDialog dialog.  Select "Select Face" button. This will return quickly with the faced selected.  A sleep is 
        explicitly added to make sure you can see the face being selected.
