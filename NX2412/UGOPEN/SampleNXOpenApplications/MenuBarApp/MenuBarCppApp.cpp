...
#include <NXOpen/MenuBar_ApplicationProperties.hxx>
...
void CppMenuBarApplication::InitializeCallbacks()
{
    try
    {
        if( registered == 0 )
        {
            ...
            MenuBar::ApplicationProperties *applicationPropertiesP = theUI->MenuBarManager()->NewApplicationProperties();
            applicationPropertiesP->SupportsUndoWithinApplication(true);
            applicationPropertiesP->AllowsMultipleWindowViews(true);
            applicationPropertiesP->AllowsMultipleWindowDesigns(true);
            applicationPropertiesP->AllowsUndoOverApplicationChange(true);
            ...
            theUI->MenuBarManager()->RegisterApplication("SAMPLE_CPP_APP",
                    ...
                    applicationPropertiesP);

            delete applicationPropertiesP;
...
MenuBar::MenuBarManager::CallbackStatus CppMenuBarApplication::PrintButtonIdCB( NXOpen::MenuBar::MenuButtonEvent* buttonEvent )
{
    if( !UF_initialize() )
    {
        ...
        if ( response == 3 || response == 5 )
        {
            ...
            // Catch the exception since the GetButtonFromName method throws an unhandled NXException in case the button doesn't exist
            catch (const NXOpen::NXException& ex)
            {
