...
Callbacks::Callbacks()
{
    ...
    customManager->AddCallback( CustomManager::CallbackReasonStockComponentCreated, make_callback( this, &Callbacks::StockComponentCreatedCallback ) );
};
...
//--------------------------------------------------------------------------------------------------
void Callbacks::StockComponentCreatedCallback
(
    StlNXObjectVector objects
)
{
    std::stringstream message;
    message << "\n*** C++ StockComponentCreatedCallback called with " << objects.size() << " objects.";
    echo( message.str() );

    for ( NXObject* object : objects )
    {
        Assemblies::Component* component = dynamic_cast<Assemblies::Component*>(object);

        if ( component == nullptr )
        {
            message.str( "" );
            message << "  NX Object Tag " << object->Tag() << " is NOT a Assemblies Component.";
            throw NXException::Create( message.str().c_str() );
        }

        message.str( "" );
        message << "  Component (Tag " << component->Tag();
        message << ") named " << component->DisplayName().GetText();
        echo( message.str() );
    }
