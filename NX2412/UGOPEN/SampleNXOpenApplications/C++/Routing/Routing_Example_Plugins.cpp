...
Plugins::Plugins()
{
    ...
    RoutingCommon::CustomManager* rsdCustomManager = session->RoutingCustomManager();
    ...
    customManager->SetApplicationEnterPlugin(                  make_callback( this, &Plugins::ApplicationEnterPlugin                  ) );
    customManager->SetApplicationExitPlugin(                   make_callback( this, &Plugins::ApplicationExitPlugin                   ) );
    ...
    rsdCustomManager->SetWiringComponentNamePlugin( make_callback( this, &Plugins::StockComponentNamePlugin ) );
...
//--------------------------------------------------------------------------------------------------
void Plugins::ApplicationEnterPlugin
(
    CustomManager::Application application
)
{
    std::string message = "Entering the Mechanical application";
    if ( application == CustomManager::ApplicationElectrical )
...
//--------------------------------------------------------------------------------------------------
void Plugins::ApplicationExitPlugin
(
    CustomManager::Application application
)
{
    std::string message = "Exiting the Mechanical application";
    if ( application == CustomManager::ApplicationElectrical )
...
)
{
    ...
    dataObject->SetManagedModeItemDescription( "A sample description" );
...
)
{
    *retcod = 0;
    ...
    Session::GetSession()->LicenseManager()->ReleaseAll( nullptr );
}
