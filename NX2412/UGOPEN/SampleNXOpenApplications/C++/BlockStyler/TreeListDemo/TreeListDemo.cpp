...
//------------------------------------------------------------------------------
NXOpen::BlockStyler::BlockDialog::DialogResponse TreeListDemo::Launch()
{
...
void MoveChildNodesOfDraggedNode(NXOpen::BlockStyler::Tree* tree, std::vector<NXOpen::BlockStyler::Node*> node, BlockStyler::Node* copyNode)
{
    //child of node to be moved
    Node* childNode = node[0]->FirstChildNode();
    std::vector<NXOpen::BlockStyler::Node*> childNodeList = std::vector<NXOpen::BlockStyler::Node*>();
    while (childNode != nullptr)
    {
        Node* copychildNode = tree->CopyNode(childNode);

        //adding the child nodes to the vector
        childNodeList.push_back(copychildNode);
    ...
    if (childNode)
    {
        ...
        Node* previousChildNode = childNode->PreviousSiblingNode();
        for (auto newNode : childNodeList)
        {
            //If there is no previous node of the childNode, should be inserted first
            if (previousChildNode == nullptr)
            {
                tree->InsertNode(newNode, copyNode, nullptr, Tree::NodeInsertOptionFirst);
            }
            else
            {
                tree->InsertNode(newNode, copyNode, previousChildNode, Tree::NodeInsertOptionLast);
            }
...
bool TreeListDemo::OnDropCallback(NXOpen::BlockStyler::Tree* tree, std::vector<NXOpen::BlockStyler::Node*> node, int columnID, NXOpen::BlockStyler::Node* targetNode, int targetColumnID, NXOpen::BlockStyler::Node::DropType dropType, int dropMenuItemId)
{
    try
    {
        ...
        WriteCallbackToListingWindow("node being moved:  " + (node[0]->DisplayText()));
        WriteCallbackToListingWindow("targetNode:  " + (targetNode->DisplayText()));

        //Node to be moved or dragged
        Node* copyNode = tree->CopyNode(node[0]);

        //Get the parent of the target node
        //WriteCallbackToListingWindow("targetNode.ParentNode:  " + (targetNode->ParentNode->DisplayText()));
        ...
        if (dropType == BlockStyler::Node::DropTypeBefore || dropType == BlockStyler::Node::DropTypeOn)
        {
            //Get the previous node of the target node
            BlockStyler::Node* previousNode = targetNode->PreviousSiblingNode();

            //If there is no previous node, it should be inserted first
            if (previousNode == nullptr)
            {
                tree->InsertNode(copyNode, parentNode, previousNode, Tree::NodeInsertOptionFirst);
            }
            else
            {
                tree->InsertNode(copyNode, parentNode, previousNode, Tree::NodeInsertOptionLast);
            }
            MoveChildNodesOfDraggedNode(tree, node, copyNode);
            tree->DeleteNode(node[0]);
        }
        else if (dropType == BlockStyler::Node::DropTypeAfter || dropType == BlockStyler::Node::DropTypeBeforeAndAfter)
        {
            //Inserting the copy node after the target node
            tree->InsertNode(copyNode, parentNode, targetNode, Tree::NodeInsertOptionLast);
            MoveChildNodesOfDraggedNode(tree, node, copyNode);
            tree->DeleteNode(node[0]);
        }
