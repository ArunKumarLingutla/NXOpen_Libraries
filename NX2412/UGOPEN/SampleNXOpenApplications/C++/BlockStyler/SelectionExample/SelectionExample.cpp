...
//------------------------------------------------------------------------------
NXOpen::BlockStyler::BlockDialog::DialogResponse SelectionExample::Launch()
{
...
int SelectionExample::deleteObject(NXOpen::NXObject* selectedObject)
{
    ...
    try
    {
        std::vector<TaggedObject* >  obj(1);
        obj[0] = selectedObject;
        int nErrs = theSession->UpdateManager()->AddObjectsToDeleteList(obj);
        ...
        if(nErrs > 0)
        {
            ...
            std::stringstream tmpString;
            tmpString << "nErrs =" << nErrs;
            SelectionExample::theUI->NXMessageBox()->Show("Add Objects To Delete List Errors", NXOpen::NXMessageBox::DialogTypeError, tmpString.str() );
        }
    }
    catch(exception& ex)
    {
