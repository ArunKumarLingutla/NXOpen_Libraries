...
:: 1. Call :Reg_Query subroutine to find the INSTALLDIR for NX.
...
::
call :Reg_Query "Unigraphics V2212" "Unigraphics Solutions\Installed Applications"
::
...
if not defined VAL (
  ...
  echo NX is not installed. Registry Key was not found.
  set RETURN_CODE=2
  goto :END_SCRIPT
)
set VAL=%VAL:"=%
...
if not exist "%VAL%\ugopen\vs_files" (
  echo %VAL%. NX UGOPEN folder does not exist. It may not be installed.
  set RETURN_CODE=3
  goto :END_SCRIPT
)
::
...
::
  ...
  set VS_Tools_Path=%VS160COMNTOOLS%
  if not defined VS_Tools_Path (
  Rem Got nothing for %VAL. That means we couldn't find a registry key.
  echo. VS160COMNTOOLS environment variable is not set with Visual Studio %VC_VER% installed path or Visual Studio %VC_VER% is not installed. Visual Studio %VC_VER% install path was not found.
  set RETURN_CODE=2
  goto :END_SCRIPT
)
  
  set VAL=%VS_Tools_Path:~0,-14%
  :: If we got here we didn't find either key value, so return with nothing.
  goto :EOF
  ::
:: End of Get_VS_InstallPath
...
:Reg_Query
  ...
  set ARG2=%ARG2:"=%
  ::
  :: Make 64 bit keys by prepending HKLM\Software... to arg2.
  ...
  reg query "%KEY64%" 1>nul 2>&1 && (
    for /F "tokens=1,2,3*" %%A in ('reg query "%KEY64%" /V %NAME% ^| findstr /ri "REG_SZ"') do (
