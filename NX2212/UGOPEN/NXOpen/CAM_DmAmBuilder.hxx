#ifndef NXOpen_CAM_DMAMBUILDER_HXX_INCLUDED
#define NXOpen_CAM_DMAMBUILDER_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     CAM_DmAmBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/CAM_AreaMillingNonSteepContainment.hxx>
#include <NXOpen/TaggedObject.hxx>
#include <NXOpen/ObjectList.hxx>
#include <NXOpen/libnxopencpp_cam_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace CAM
    {
        class DmAmBuilder;
    }
    namespace CAM
    {
        class AreaMillingNonSteepContainment;
    }
    namespace CAM
    {
        class AreaMillingSteepContainment;
    }
    namespace CAM
    {
        class CutAngle;
    }
    namespace CAM
    {
        class CutPatternBuilder;
    }
    namespace CAM
    {
        class InheritableDoubleBuilder;
    }
    namespace CAM
    {
        class InheritableToolDepBuilder;
    }
    namespace CAM
    {
        class StepoverBuilder;
    }
    class Point;
    class SectionList;
    namespace CAM
    {
        class _DmAmBuilderBuilder;
        class DmAmBuilderImpl;
        /** Represents a Area Milling Drive Method Builder 
         <br>  Created in NX5.0.0.  <br>  
        */
        class NXOPENCPP_CAMEXPORT  DmAmBuilder : public NXOpen::TaggedObject
        {
            /** Available types for Area Milling cut pattern in FASurfaceContour */
            public: enum AmCutPatternTypes
            {
                AmCutPatternTypesFollowPeriphery/** Follow Periphery */,
                AmCutPatternTypesProfile/** Profile */,
                AmCutPatternTypesSep1/** separate one */,
                AmCutPatternTypesParallelLines/** Parallel Lines */,
                AmCutPatternTypesRadialLines/** Radial Lines */,
                AmCutPatternTypesConcArcs/** Concentric Arcs */
            };

            /** Available types for Area Milling cut traversal in FASurfaceContour */
            public: enum AmCutTraversalTypes
            {
                AmCutTraversalTypesZigZag/** Zig-Zag */,
                AmCutTraversalTypesZigZagWithLifts/** Zig-Zag with Lifts */,
                AmCutTraversalTypesZig/** Zig */,
                AmCutTraversalTypesContour/** Zig with Contour */,
                AmCutTraversalTypesStepover/** Zig with Stepover */
            };

            /** Available types for Area Milling Pocketing Direction in FASurfaceContour */
            public: enum AmPocketDirTypes
            {
                AmPocketDirTypesOutward/** Outward */,
                AmPocketDirTypesInward/** Inward */
            };

            /** Available types for  in FASurfaceContour */
            public: enum AmCustomStartTypes
            {
                AmCustomStartTypesCustom/** Area Milling Custom Start Custom */,
                AmCustomStartTypesAutomatic/** Area Milling Custom Start Automatic */
            };

            /** Available types for area milling Steep Option*/
            public: enum SteepOptTypes
            {
                SteepOptTypesSteepContainNone/** steep contain none */ ,
                SteepOptTypesNonSteepNonDirectional/** non steep non directional */ ,
                SteepOptTypesSteepDirectional/** steep directional */ ,
                SteepOptTypesSteepAndNonsteep/** steep and nonsteep */ ,
                SteepOptTypesSteep/** steep */ 
            };

            /** Available types for area milling Steep Overlap Option*/
            public: enum SteepOverlapOptTypes
            {
                SteepOverlapOptTypesNone/**  No Overlap */,
                SteepOverlapOptTypesAngle/**  Angle Overlap */,
                SteepOverlapOptTypesDistance/**  Distance Overlap */
            };

            /** Available type for area milling Order Regions Option*/
            public: enum OrderRegionsOptTypes
            {
                OrderRegionsOptTypesDefault/** Default, Steep First */,
                OrderRegionsOptTypesTopdown/** Top Down Level First approach */,
                OrderRegionsOptTypesTopdownDepthFirst/** Top Down Depth First approach */
            };

            /** Available types for area milling Auto Pattern Center*/
            public: enum AutoPatCenterTypes
            {
                AutoPatCenterTypesAutomatic/** Automatic */,
                AutoPatCenterTypesSpecify/** Specify */
            };

            private: DmAmBuilderImpl * m_dmambuilder_impl;
            private: friend class  _DmAmBuilderBuilder;
            protected: DmAmBuilder();
            public: ~DmAmBuilder();
            /**Returns  the Cut Pattern value 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::DmAmBuilder::AmCutPatternTypes AmCutPattern
            (
            );
            /**Sets  the Cut Pattern value 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetAmCutPattern
            (
                NXOpen::CAM::DmAmBuilder::AmCutPatternTypes amCutPattern /** the Cut Pattern value */
            );
            /**Returns  the Cut Traversal value 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::DmAmBuilder::AmCutTraversalTypes AmCutTraversal
            (
            );
            /**Sets  the Cut Traversal value 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetAmCutTraversal
            (
                NXOpen::CAM::DmAmBuilder::AmCutTraversalTypes amCutTraversal /** the Cut Traversal value */
            );
            /**Returns  the cut angle 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::CutAngle * CutAngle
            (
            );
            /**Returns  the Pocketing Deriction value 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::AreaMillingNonSteepContainment::AmPocketDirTypes AmPocketDir
            (
            );
            /**Sets  the Pocketing Deriction value 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetAmPocketDir
            (
                NXOpen::CAM::AreaMillingNonSteepContainment::AmPocketDirTypes pocketDirValue /** the Pocketing Deriction value */
            );
            /**Returns  the Additional Passes 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: int AmAddPasses
            (
            );
            /**Sets  the Additional Passes 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetAmAddPasses
            (
                int addPassesValue /** the Additional Passes value */
            );
            /**Returns  the Cut Region Start Point
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::DmAmBuilder::AmCustomStartTypes AmCutRegionStartEnum
            (
            );
            /**Sets  the Cut Region Start Point
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetAmCutRegionStartEnum
            (
                NXOpen::CAM::DmAmBuilder::AmCustomStartTypes amCutRegionStart /** the Cut Region Start Point */
            );
            /**Returns  the Area Milling Steep Option
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::DmAmBuilder::SteepOptTypes AmSteepOption
            (
            );
            /**Sets  the Area Milling Steep Option
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetAmSteepOption
            (
                NXOpen::CAM::DmAmBuilder::SteepOptTypes amSteepOption /** the Area Milling Steep Option */
            );
            /**Returns  the steep angle 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::InheritableDoubleBuilder * SteepAngle
            (
            );
            /**Returns  the Area Milling Auto Pattern Center
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::DmAmBuilder::AutoPatCenterTypes AmAutoPatCenter
            (
            );
            /**Sets  the Area Milling Auto Pattern Center
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetAmAutoPatCenter
            (
                NXOpen::CAM::DmAmBuilder::AutoPatCenterTypes autoPatCenter /** autopatcenter */ 
            );
            /**Returns  the Pattern Center Point 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Point * PatternCenterPoint
            (
            );
            /**Sets  the Pattern Center Point 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void SetPatternCenterPoint
            (
                NXOpen::Point * centerPoint /** centerpoint */ 
            );
            /**Returns  the stepover builder 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::StepoverBuilder * StepoverBuilder
            (
            );
            /**Returns  the Region Connection 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: bool DmAmRegionCon
            (
            );
            /**Sets  the Region Connection 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetDmAmRegionCon
            (
                bool dmAmRegionCon /** the Region Connection value */
            );
            /**Returns  the Profile Pass 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: bool ProfilePass
            (
            );
            /**Sets  the Profile Pass 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void SetProfilePass
            (
                bool profilePass /** the Profile Pass value */
            );
            /**Returns  the cut pattern builder 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::CutPatternBuilder * CutPatternBuilder
            (
            );
            /**Returns  the Non-steep Cutting Builder 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::AreaMillingNonSteepContainment * NonSteepCutting
            (
            );
            /**Returns  the Steep Cutting Builder 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::AreaMillingSteepContainment * SteepCutting
            (
            );
            /**Returns  the Recognize Flat Option 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : None */
            public: bool RecognizeFlatOption
            (
            );
            /**Sets  the Recognize Flat Option 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void SetRecognizeFlatOption
            (
                bool recognizeFlatOption /** the Recognize Flat Option */
            );
            /**Returns  the Steep Overlap Option 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : None */
            public: bool SteepOverlapOption
            (
            );
            /**Sets  the Steep Overlap Option 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void SetSteepOverlapOption
            (
                bool steepOverlapOption /** the Steep Overlap Option */
            );
            /**Returns  the Steep Overlap Angle 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::InheritableDoubleBuilder * SteepOverlapAngle
            (
            );
            /**Returns  the Steep Overlap Distance 
             <br>  Created in NX9.0.3.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::InheritableToolDepBuilder * SteepOverlapDistance
            (
            );
            /**Returns  the Steep Overlap Type 
             <br>  Created in NX9.0.3.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::DmAmBuilder::SteepOverlapOptTypes SteepOverlapType
            (
            );
            /**Sets  the Steep Overlap Type 
             <br>  Created in NX9.0.3.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void SetSteepOverlapType
            (
                NXOpen::CAM::DmAmBuilder::SteepOverlapOptTypes steepOverlapType /** the Steep Overlap Type */
            );
            /**Returns  the Order Regions Type 
             <br>  Created in NX11.0.1.  <br>  
             <br> License requirements : None */
            public: NXOpen::CAM::DmAmBuilder::OrderRegionsOptTypes OrderRegionsType
            (
            );
            /**Sets  the Order Regions Type 
             <br>  Created in NX11.0.1.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void SetOrderRegionsType
            (
                NXOpen::CAM::DmAmBuilder::OrderRegionsOptTypes orderRegionType /** the Order Regions Type */
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
