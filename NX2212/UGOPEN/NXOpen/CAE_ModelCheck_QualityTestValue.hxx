#ifndef NXOpen_CAE_MODELCHECK_QUALITYTESTVALUE_HXX_INCLUDED
#define NXOpen_CAE_MODELCHECK_QUALITYTESTVALUE_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     CAE_ModelCheck_QualityTestValue.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/CAE_ModelCheck_ElementSpecificTestValue.hxx>
#include <NXOpen/CAE_ModelCheck_TestValueTypes.hxx>
#include <NXOpen/CAE_ModelCheck_ITestValue.hxx>
#include <NXOpen/NXObject.hxx>
#include <NXOpen/libnxopencpp_cae_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace CAE
    {
        namespace ModelCheck
        {
            class QualityTestValue;
        }
    }
    namespace CAE
    {
        namespace ModelCheck
        {
            class ElementSpecificTestValue;
        }
    }
    namespace CAE
    {
        namespace ModelCheck
        {
            class ITestValue;
        }
    }
    class NXObject;
    class Unit;
    namespace CAE
    {
        namespace ModelCheck
        {
            class _QualityTestValueBuilder;
            class QualityTestValueImpl;
            /**  @brief  Represents quality test settings of a specified test type  

               <br> Not to support KF  <br> 
             <br>  Created in NX8.5.0.  <br>  
            */
            class NXOPENCPP_CAEEXPORT  QualityTestValue : public NXOpen::NXObject, public virtual NXOpen::CAE::ModelCheck::ITestValue
            {
                private: QualityTestValueImpl * m_qualitytestvalue_impl;
                private: friend class  _QualityTestValueBuilder;
                protected: QualityTestValue();
                public: ~QualityTestValue();
                /**Returns  the count of @link NXOpen::CAE::ModelCheck::ElementSpecificTestValue NXOpen::CAE::ModelCheck::ElementSpecificTestValue@endlink  in this test setting 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: int ElementSpecificTestCount
                (
                );
                /** Returns @link NXOpen::CAE::ModelCheck::ElementSpecificTestValue NXOpen::CAE::ModelCheck::ElementSpecificTestValue@endlink  of specified index in this test setting 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: NXOpen::CAE::ModelCheck::ElementSpecificTestValue * GetElementSpecificTestByIndex
                (
                    int index /** index */ 
                );
                /** Returns the test type 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual NXOpen::CAE::ModelCheck::TestValueTypes::TestType GetTestType
                (
                );
                /** Returns the validator type 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual NXOpen::CAE::ModelCheck::TestValueTypes::Validator GetValidator
                (
                );
                /**Returns  an option value indicating whether to do element quality check for this test  
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual bool DoTest
                (
                );
                /**Sets  an option value indicating whether to do element quality check for this test  
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual void SetDoTest
                (
                    bool doTest /** dotest */ 
                );
                /** Tells whether there is criteria value associated with this test 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual bool HasCriteriaValue
                (
                );
                /** Returns the criteria value. An exception will be thrown if there is no criteria value associated with this test 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual NXOpen::Unit * GetCriteriaValue
                (
                    NXOpen::CAE::ModelCheck::TestValueTypes::CriteriaType criteriaType /** criteriatype */ ,
                    double* criteriaValue /** criteriavalue */ 
                );
                /**Sets the criteria value. An exception will be thrown if there is no criteria value associated with this test 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual void SetCriteriaValue
                (
                    NXOpen::CAE::ModelCheck::TestValueTypes::CriteriaType criteriaType /** criteriatype */ ,
                    double criteriaValue /** criteriavalue */ ,
                    NXOpen::Unit * unit /** unit */ 
                );
                /**Sets the criteria value. An exception will be thrown if there is no criteria value associated with this test 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual void SetCriteriaValue
                (
                    NXOpen::CAE::ModelCheck::TestValueTypes::CriteriaType criteriaType /** criteriatype */ ,
                    const NXString & criteriaValue /** criteriavalue */ ,
                    NXOpen::Unit * unit /** unit */ 
                );
                /**Sets the criteria value. An exception will be thrown if there is no criteria value associated with this test 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                virtual void SetCriteriaValue
                (
                    NXOpen::CAE::ModelCheck::TestValueTypes::CriteriaType criteriaType /** criteriatype */ ,
                    const char * criteriaValue /** criteriavalue */ ,
                    NXOpen::Unit * unit /** unit */ 
                );
                /** To know whether this quality test is a solver specific test and a system test 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual bool GetIsSolverSpecificTest
                (
                    bool* isSystemTest /** issystemtest */ 
                );
                /** Returns the element reference definitions associated with this test 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual std::vector<NXOpen::CAE::ModelCheck::TestValueTypes::ElementReference> GetElementReferences
                (
                );
                /** Reset as customer default setting 
                 <br>  Created in NX8.5.0.  <br>  
                 <br> License requirements : None */
                public: virtual void ResetToCustomerDefault
                (
                );
            };
        }
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
