#ifndef NXOpen_CAE_RESPONSESIMULATION_NODALFUNCTIONEXCITATIONBUILDER_HXX_INCLUDED
#define NXOpen_CAE_RESPONSESIMULATION_NODALFUNCTIONEXCITATIONBUILDER_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     CAE_ResponseSimulation_NodalFunctionExcitationBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Expression.hxx>
#include <NXOpen/CAE_ResponseSimulation_ExcitationBuilder.hxx>
#include <NXOpen/libnxopencpp_cae_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace CAE
    {
        namespace ResponseSimulation
        {
            class NodalFunctionExcitationBuilder;
        }
    }
    namespace CAE
    {
        namespace ResponseSimulation
        {
            class ExcitationBuilder;
        }
    }
    class Expression;
    namespace CAE
    {
        namespace ResponseSimulation
        {
            class _NodalFunctionExcitationBuilderBuilder;
            class NodalFunctionExcitationBuilderImpl;
            /** Represents the manager to @link NXOpen::CAE::ResponseSimulation::NodalFunctionExcitation NXOpen::CAE::ResponseSimulation::NodalFunctionExcitation@endlink . 
                The objects of @link NXOpen::CAE::ResponseSimulation::NodalFunctionExcitation NXOpen::CAE::ResponseSimulation::NodalFunctionExcitation@endlink  
                can be created or edited on through this class  <br> This is an abstract class, and cannot be created.  <br> 
             <br>  Created in NX5.0.0.  <br>  
            */
            class NXOPENCPP_CAEEXPORT  NodalFunctionExcitationBuilder : public NXOpen::CAE::ResponseSimulation::ExcitationBuilder
            {
                private: NodalFunctionExcitationBuilderImpl * m_nodalfunctionexcitationbuilder_impl;
                private: friend class  _NodalFunctionExcitationBuilderBuilder;
                protected: NodalFunctionExcitationBuilder();
                public: ~NodalFunctionExcitationBuilder();
                /**Returns  the option to inherit frequency Range from function 
                 <br>  Created in NX1953.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: bool InheritFrequencyRangeFromFunction
                (
                );
                /**Sets  the option to inherit frequency Range from function 
                 <br>  Created in NX1953.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: void SetInheritFrequencyRangeFromFunction
                (
                    bool inheritFromFunction /** inherit from function */ 
                );
                /**Returns  the lower bound frequency 
                 <br>  Created in NX1953.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: double FrequencyRangeLowerBound
                (
                );
                /**Sets  the lower bound frequency 
                 <br>  Created in NX1953.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: void SetFrequencyRangeLowerBound
                (
                    double lowerBound /** lower bound */ 
                );
                /**Returns  the upper bound frequency 
                 <br>  Created in NX1953.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: double FrequencyRangeUpperBound
                (
                );
                /**Sets  the upper bound frequency 
                 <br>  Created in NX1953.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: void SetFrequencyRangeUpperBound
                (
                    double upperBound /** upper bound */ 
                );
                /**Returns  the user specified initial excitation conditions toggle 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: bool UserSpecifiedInitialExcitationConditionsToggle
                (
                );
                /**Sets  the user specified initial excitation conditions toggle 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: void SetUserSpecifiedInitialExcitationConditionsToggle
                (
                    bool initialConditionsToggle /** initial conditions toggle */ 
                );
                /**Returns  the inital user defined direction Displacement 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialUserDefDirDisplacement
                (
                );
                /**Returns  the inital user defined direction Velocity 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialUserDefDirVelocity
                (
                );
                /**Returns  the inital user defined direction Acceleration 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialUserDefDirAcceleration
                (
                );
                /**Returns  the inital X Displacement 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialDisplacementX
                (
                );
                /**Returns  the inital Y Displacement 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialDisplacementY
                (
                );
                /**Returns  the inital Z Displacement 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialDisplacementZ
                (
                );
                /**Returns  the initial velocity X 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialVelocityX
                (
                );
                /**Returns  the initial velocity Y 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialVelocityY
                (
                );
                /**Returns  the initial velocity Z 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialVelocityZ
                (
                );
                /**Returns  the initial acceleration X 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialAccelerationX
                (
                );
                /**Returns  the initial acceleration Y 
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialAccelerationY
                (
                );
                /**Returns  the initial acceleration Z
                 <br>  Created in NX2007.0.0.  <br>  
                 <br> License requirements : nx_response_anlys ("NX Response Analysis") */
                public: NXOpen::Expression * InitialAccelerationZ
                (
                );
            };
        }
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
