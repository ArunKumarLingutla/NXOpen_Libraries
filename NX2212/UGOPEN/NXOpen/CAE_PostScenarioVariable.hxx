    ...
    namespace CAE
    {
        ...
        */
        class NXOPENCPP_CAEEXPORT  PostScenarioVariable : public NXOpen::TaggedObject
        {
            ...
            /**Returns  the locked property of the selection variable.
                        
                        When a variable is locked, this might filter values for other selection variables where relevant.
                        For example, if the file contains data for several loadcases and frequencies,
                        and for which the frequency values depend on the loadcase, locking a loadcase to a specific value
                        will filter the frequencies so that only the ones which are relevant to the locked loadcase
                        will be available as values for the Frequency selection variable.

                        If the variable is locked while assigning values to it in the future, it will be unlocked.

                        Locking a variable does not automatically refresh the values of other selection variables, since 
                        this might have a performance impact. When a variable is locked/unlocked, a call to 
                        @link CAE::PostScenarioSelectionParameters::RefreshVariables CAE::PostScenarioSelectionParameters::RefreshVariables@endlink  is needed, but this can
                        be done once, after all variables have been properly locked/unlocked.
            ...
            /**Sets  the locked property of the selection variable.
