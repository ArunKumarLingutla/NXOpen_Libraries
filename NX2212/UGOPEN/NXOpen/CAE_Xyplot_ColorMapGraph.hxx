#ifndef NXOpen_CAE_XYPLOT_COLORMAPGRAPH_HXX_INCLUDED
#define NXOpen_CAE_XYPLOT_COLORMAPGRAPH_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     CAE_Xyplot_ColorMapGraph.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/CAE_Xyplot_MarkerModel.hxx>
#include <NXOpen/CAE_Xyplot_Graph.hxx>
#include <NXOpen/CAE_Xyplot_I3DDataGraph.hxx>
#include <NXOpen/libnxopencpp_cae_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace CAE
    {
        namespace Xyplot
        {
            class ColorMapGraph;
        }
    }
    namespace CAE
    {
        namespace Xyplot
        {
            class Graph;
        }
    }
    namespace CAE
    {
        namespace Xyplot
        {
            class I3DDataGraph;
        }
    }
    namespace CAE
    {
        namespace Xyplot
        {
            class MarkerModel;
        }
    }
    namespace CAE
    {
        namespace Xyplot
        {
            class _ColorMapGraphBuilder;
            class ColorMapGraphImpl;
            /** Manages the display color map graph. <br> Not support KF.  <br> 
             <br>  Created in NX12.0.0.  <br>  
            */
            class NXOPENCPP_CAEEXPORT  ColorMapGraph : public NXOpen::CAE::Xyplot::Graph, public virtual NXOpen::CAE::Xyplot::I3DDataGraph
            {
                private: ColorMapGraphImpl * m_colormapgraph_impl;
                private: friend class  _ColorMapGraphBuilder;
                protected: ColorMapGraph();
                public: ~ColorMapGraph();
                /** Creates Order Marker and show it in a graph which plot type is @link CAE::Xyplot::PlotTypePlotCampbell CAE::Xyplot::PlotTypePlotCampbell@endlink . 
                 <br>  Created in NX1926.0.0.  <br>  
                 <br> License requirements : None */
                public: NXOpen::CAE::Xyplot::OrderMarkerModel * CreateOrderMarker
                (
                    int recordIndex /** Record index */
                );
                /** Creates the display order marker on the color map graph.  @return  Marker model 
                 <br>  Created in NX12.0.0.  <br>  
                 <br> License requirements : None */
                public: NXOpen::CAE::Xyplot::MarkerModel * CreateOrderMarker
                (
                    double orderValue /** Order Value */
                );
                /**  @brief  Gets the source point count of specified record.  

                 
                            
                                The record index is between 0 and @link CAE::Xyplot::Graph::RecordCount CAE::Xyplot::Graph::RecordCount@endlink , 0 is inclusive.
                            
                             @return  Point count 
                 <br>  Created in NX12.0.0.  <br>  
                 <br> License requirements : None */
                public: virtual int GetSourcePointCount
                (
                    int recordIndex /** Record index */,
                    int sectionIndex /** Section index */
                );
                /**  @brief  Creates a marker in the position of a source point.  

                 
                            
                                The record index is between 0 and @link CAE::Xyplot::Graph::RecordCount CAE::Xyplot::Graph::RecordCount@endlink , 0 is inclusive. 
                                The point index is between 0 and @link CAE::Xyplot::I3DDataGraph::GetSourcePointCount CAE::Xyplot::I3DDataGraph::GetSourcePointCount@endlink , 0 is inclusive. 
                            
                             @return  Marker model 
                 <br>  Created in NX12.0.0.  <br>  
                 <br> License requirements : None */
                public: virtual NXOpen::CAE::Xyplot::MarkerModel * CreateMarker
                (
                    int recordIndex /** Record index */,
                    int sectionIndex /** Section index */,
                    int pointIndex /** Point index */
                );
                /**  @brief  Creates a marker in the linear interpolation position between two source points.  

                 
                            
                                The record index is between 0 and @link CAE::Xyplot::Graph::RecordCount CAE::Xyplot::Graph::RecordCount@endlink , 0 is inclusive. 
                                The previous/next point index is between 0 and @link CAE::Xyplot::I3DDataGraph::GetSourcePointCount CAE::Xyplot::I3DDataGraph::GetSourcePointCount@endlink , 0 is inclusive.
                                The linear interpolation scale is between 0 and 1, both 0 and 1 are inclusive. 
                            
                             @return  Marker model 
                 <br>  Created in NX12.0.0.  <br>  
                 <br> License requirements : None */
                public: virtual NXOpen::CAE::Xyplot::MarkerModel * CreateMarker
                (
                    int recordIndex /** Record index */,
                    int sectionIndex /** Section index */,
                    int prePointIndex /** Previous Point index */,
                    int nextPointIndex /** Next Point index */,
                    double linearInterpoScale /** Interpolation Scale */
                );
            };
        }
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
