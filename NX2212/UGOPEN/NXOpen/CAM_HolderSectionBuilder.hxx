#ifndef NXOpen_CAM_HOLDERSECTIONBUILDER_HXX_INCLUDED
#define NXOpen_CAM_HOLDERSECTIONBUILDER_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     CAM_HolderSectionBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/TaggedObject.hxx>
#include <NXOpen/libnxopencpp_cam_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace CAM
    {
        class HolderSectionBuilder;
    }
    namespace CAM
    {
        class InheritableDoubleBuilder;
    }
    class NXObject;
    namespace CAM
    {
        class _HolderSectionBuilderBuilder;
        class HolderSectionBuilderImpl;
        /** Represents a Holder Section Builder 
         <br>  Created in NX5.0.0.  <br>  
        */
        class NXOPENCPP_CAMEXPORT  HolderSectionBuilder : public NXOpen::TaggedObject
        {
            private: HolderSectionBuilderImpl * m_holdersectionbuilder_impl;
            private: friend class  _HolderSectionBuilderBuilder;
            protected: HolderSectionBuilder();
            public: ~HolderSectionBuilder();
            /**Returns  the Offset (inheritable double) 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::CAM::InheritableDoubleBuilder * TlHolderOffsetBuilder
            (
            );
            /** Create a new holder section item @return  input the current selected item pointer and return new created item pointer.
                                                                                       if list is empty, should input NULL 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::NXObject * Create
            (
                double diameter /** the diameter */,
                double length /** the length */,
                double taperAngle /** the taper angle */,
                double cornerRadius /** the corner radius */
            );
            /** Modify an existing holder section item
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void Modify
            (
                NXOpen::NXObject * section /** the pointer of modified holder section */,
                double diameter /** the diameter */,
                double length /** the length */,
                double taperAngle /** the taper angle */,
                double cornerRadius /** the corner radius */
            );
            /** Modify an existing holder section item by index 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void Modify
            (
                int index /** the index of modified holder section */,
                double diameter /** the diameter */,
                double length /** the length */,
                double taperAngle /** the taper angle */,
                double cornerRadius /** the corner radius */
            );
            /** Delete an existing holder section item
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void Delete
            (
                NXOpen::NXObject * section /** the pointer of deleted holder section */
            );
            /** Delete an existing holder section item by index 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void Delete
            (
                int index /** the index of deleted holder section */
            );
            /** Move up a holder section item 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void MoveUp
            (
                int index /** the index of item which should be moved up*/
            );
            /** Move down a holder section item 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void MoveDown
            (
                int index /** the index of item which should be moved down*/
            );
            /** Get the values of an existing holder section item 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void Get
            (
                NXOpen::NXObject * section /** the section */,
                double* diameter /** the diameter */,
                double* length /** the length */,
                double* taperAngle /** the taper angle */,
                double* cornerRadius /** the corner radius */
            );
            /**Returns  the number of holder sections 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: int NumberOfSections
            (
            );
            /** Get a specific holder section  @return  the section 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: NXOpen::NXObject * GetSection
            (
                int position /** index in array */
            );
            /** Add a new holder section item @return  output the new created item actual index,if error happens, output will be -1 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: int Add
            (
                int inputIndex /** input the new created item index */,
                double diameter /** the diameter */,
                double length /** the length */,
                double taperAngle /** the taper angle */,
                double cornerRadius /** the corner radius */
            );
            /** Add a new holder section item by upper diameter @return  output the new created item actual index,if error happens, output will be -1 
             <br>  Created in NX8.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: int AddByUpperDiameter
            (
                int inputIndex /** input the new created item index */,
                double lowerDiameter /** the lower diameter */,
                double length /** the length */,
                double upperDiameter /** the upper diameter */,
                double cornerRadius /** the corner radius */
            );
            /** Modify an existing holder section item by upper diameter 
             <br>  Created in NX8.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void ModifyByUpperDiameter
            (
                int index /** the index of modified holder section */,
                double lowerDiameter /** the lower diameter */,
                double length /** the length */,
                double upperDiameter /** the upper diameter */,
                double cornerRadius /** the corner radius */
            );
            /** Get the values of an existing holder section item, include upper diameter 
             <br>  Created in NX8.0.0.  <br>  
             <br> License requirements : None */
            public: void GetAllParameters
            (
                NXOpen::NXObject * section /** the section */,
                double* lowerDiameter /** the lower diameter */,
                double* length /** the length */,
                double* taperAngle /** the taper angle */,
                double* upperDiameter /** the upper diameter */,
                double* cornerRadius /** the corner radius */
            );
            /** Create holder steps from solid 
             <br>  Created in NX1980.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void RetrieveStepsFromSolid
            (
            );
            /**Returns  the mounting coordinate system 
             <br>  Created in NX1980.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::NXObject * MountingCsys
            (
            );
            /**Sets  the mounting coordinate system 
             <br>  Created in NX1980.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void SetMountingCsys
            (
                NXOpen::NXObject * mountingCsys /** the new mounting csys */
            );
            /** Set selected solid bodies 
             <br>  Created in NX1980.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void SetSelectedBodies
            (
                const std::vector<NXOpen::NXObject *> & bodies /** bodies */ 
            );

            /// \cond NX_NO_DOC 
            /**Returns  This is an internal API and can be changed at any time 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : None */
            public: double ProfileStartPosition
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : cam_base ("CAM BASE") */
            public: void SetProfileStartPosition
            (
                double newValue /** the shank profile start position */
            );

            /// \endcond 
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
