#ifndef NXOpen_ANNOTATIONS_PMIMANAGER_HXX_INCLUDED
#define NXOpen_ANNOTATIONS_PMIMANAGER_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Annotations_PmiManager.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Annotations_ArbitraryAreaSeedBuilder.hxx>
#include <NXOpen/Annotations_ConvertPmiBuilder.hxx>
#include <NXOpen/Annotations_PMIExplicitOrderBuilder.hxx>
#include <NXOpen/Annotations_SuppressPMIBuilder.hxx>
#include <NXOpen/BasePart.hxx>
#include <NXOpen/TaggedObject.hxx>
#include <NXOpen/libnxopencpp_annotations_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace Annotations
    {
        class PmiManager;
    }
    class Part;
    namespace Annotations
    {
        class Annotation;
    }
    namespace Annotations
    {
        class ArbitraryAreaSeedBuilder;
    }
    namespace Annotations
    {
        class ConvertPmiBuilder;
    }
    namespace Annotations
    {
        class PMIExplicitOrderBuilder;
    }
    namespace Annotations
    {
        class Pmi;
    }
    namespace Annotations
    {
        class PmiAttributeCollection;
    }
    namespace Annotations
    {
        class PmiCollection;
    }
    namespace Annotations
    {
        class PmiLightweightSectionCollection;
    }
    namespace Annotations
    {
        class PmiRegionCollection;
    }
    namespace Annotations
    {
        class PmiWaveCollection;
    }
    namespace Annotations
    {
        class SearchModelViewCollection;
    }
    namespace Annotations
    {
        class SuppressPMIBuilder;
    }
    namespace Assemblies
    {
        class Component;
    }
    class INXObject;
    class TaggedObject;
    class View;
    namespace Annotations
    {
        class PmiManagerImpl;
        /** Represents an object that manages PMI objects.  <br> To obtain an instance of this class, refer to @link NXOpen::Part  NXOpen::Part @endlink  <br> 
         <br>  Created in NX5.0.0.  <br>  
        */
        class NXOPENCPP_ANNOTATIONSEXPORT  PmiManager
        {
            /** the display status for the PMIs inside a component of an assembly part */
            public: enum ComponentPmiDisplayStatus
            {
                ComponentPmiDisplayStatusDoDisplay/** PMIs are displayed in the component */,
                ComponentPmiDisplayStatusDoNotDisplay/** PMIs are not displayed in the component */,
                ComponentPmiDisplayStatusUnknown/** PMIs' display status is unknown in the component */
            };

            private: PmiManagerImpl * m_pmimanager_impl;
            private: NXOpen::Part* m_owner;
            /// \cond NX_NO_DOC 
            public: explicit PmiManager(NXOpen::Part *owner);


            /// \endcond 
            public: 
            /**Returns the tag of this object.  */
            tag_t Tag() const; 
            public: ~PmiManager();
            /** Creates copies of input PMI objects in the part. 
                        Only PMI objects which can be copied in the NX user interface will be copied.
                         <br> 
                        Client must perform update @link NXOpen::Update::DoUpdate NXOpen::Update::DoUpdate@endlink  after calling 
                        this method.  <br>   @return  Copies of PMI objects 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: std::vector<NXOpen::Annotations::Pmi *> CopyObjects
            (
                const std::vector<NXOpen::Annotations::Pmi *> & inputObjects /** PMI objects to be copied */
            );
            /** Creates copies of input PMI Display Instance objects in the part.             
                        Only PMI Display Instance objects which can be copied in the NX user interface
                        will be copied.
                         <br> 
                        Client must perform update @link NXOpen::Update::DoUpdate NXOpen::Update::DoUpdate@endlink ()
                        after calling this method.  <br>   @return  Copies of PMI Display Instance objects 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: std::vector<NXOpen::Annotations::Annotation *> CopyDisplayInstanceObjects
            (
                const std::vector<NXOpen::Annotations::Annotation *> & inputObjects /** PMI Display Instance objects to be copied */,
                NXOpen::View * view /** View in which PMI Display Instance are copied. */
            );
            /** Cuts the input PMI Display Instance objects in the part. Cut operation makes the PMI
                        Display Instance object invisible in the view of cut and makes it visible in 
                        the view of paste. Only PMI Display Instance objects which can be cut in the NX user
                        interface will be copied.
                         <br>                
                        Client must perform update @link NXOpen::Update::DoUpdate NXOpen::Update::DoUpdate@endlink () after calling this 
                        method.  <br>  
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void CutDisplayInstanceObjects
            (
                const std::vector<NXOpen::Annotations::Annotation *> & inputObjects /** PMI Display Instance objects to be cut */,
                const std::vector<NXOpen::View *> & cutViews /** List of cut views. Each PMI Display Instance
                                                                              object is cut from the corresponding view 
                                                                              in the list. 
                                                                              */,
                NXOpen::View * viewOfPaste /** View in which PMI Display Instances are pasted. */
            );
            /** Restores the unpasted PMI objects in the part. 
                         <br> 
                        This method is used by NX user interface to restore PMI objects that
                        were cut but not pasted. If called by an automation program, this
                        method can cause unexpected results.  <br>  
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: void RestoreUnpastedObjects
            (
            );
            /** The flag indicating whether the annotation is an inherited PMI on the drawing 
                        sheet/view. Inherited PMI is the associative copy of the PMI display 
                        instance in modeling.  @return  
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: bool IsInheritedPmi
            (
                NXOpen::Annotations::Annotation * object /** Annotation to examine */
            );
            /** Returns the inherited PMI parent. The parent is the PMI Display Instance in modeling.  @return  Parent PMI Display Instance 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Annotations::Annotation * GetInheritParent
            (
                NXOpen::Annotations::Annotation * inheritedPmi /** Inherited PMI */
            );
            /** Returns the PMI Display Instance parent. The parent is the PMI Object.  @return  Parent PMI object 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Annotations::Pmi * GetDisplayInstanceParent
            (
                NXOpen::Annotations::Annotation * displayInstance /** PMI Display Instance */
            );
            /** Creates a @link NXOpen::Annotations::ArbitraryAreaSeedBuilder NXOpen::Annotations::ArbitraryAreaSeedBuilder@endlink  
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Annotations::ArbitraryAreaSeedBuilder * CreateArbitraryAreaSeedBuilder
            (
            );
            /** Deletes or replaces one annotation with another.
                         <br> 
                        This will adjust the requirements ID of the annotation if one annotation replaces
                        another. This method may be used in NX generated automation programs but it is not
                        intended to be used in a customer  written automation program.
                         <br>  
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void DeleteReplaceAnnotation
            (
                NXOpen::Annotations::Annotation * oldAnnotation /** @link NXOpen::Annotations::Annotation NXOpen::Annotations::Annotation@endlink  Annotation or Dimension Set to be deleted */,
                NXOpen::Annotations::Annotation * newAnnotation /** @link NXOpen::Annotations::Annotation NXOpen::Annotations::Annotation@endlink  to replace the deleted annootation, can be NULL */
            );
            /** Resizes all PMI symbols displayed in the active view
             <br>  Created in NX8.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void Resize
            (
            );
            /** Resets the size of all the PMI displayed in the part.
                         <br> 
                        This will restore the size of the PMI to the part default.
                         <br> 
                    
             <br>  Created in NX11.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void ResetSize
            (
            );
            /** Creates a @link NXOpen::Annotations::PMIExplicitOrderBuilder NXOpen::Annotations::PMIExplicitOrderBuilder@endlink  
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Annotations::PMIExplicitOrderBuilder * CreateExplicitOrderBuilder
            (
            );
            /** Applies a saved explicit order
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void ApplyModelViewOrder
            (
                const NXString & savedOrderName /** savedordername */ 
            );
            /** Applies a saved explicit order
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            void ApplyModelViewOrder
            (
                const char * savedOrderName /** savedordername */ 
            );
            /** Saves an explicit order defined by the user 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void SaveModelViewOrder
            (
                std::vector<NXString> & orderList /** orderlist */ ,
                const NXString & saveName /** savename */ 
            );
            /** Saves an explicit order defined by the user 
             <br>  Created in NX9.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            void SaveModelViewOrder
            (
                std::vector<NXString> & orderList /** orderlist */ ,
                const char * saveName /** savename */ 
            );
            /** The flag indicates whether the PMI is an effectivity filtered PMI 
                        because of assembly component configuration from managed environment  @return  
             <br>  Created in NX8.5.1.  <br>  
             <br> License requirements : None */
            public: bool IsEffectivityFiltered
            (
                NXOpen::Annotations::Pmi * object /** PMI object to examine */
            );
            /** Creates a @link NXOpen::Annotations::SuppressPMIBuilder NXOpen::Annotations::SuppressPMIBuilder@endlink  
             <br>  Created in NX12.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Annotations::SuppressPMIBuilder * CreateSuppressPmibuilder
            (
            );
            /**Position all the PMIs in given view 
             <br>  Created in NX1980.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void Position
            (
                NXOpen::View * view /** view */ 
            );

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX1980.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void PositionSelected
            (
                NXOpen::View * view /** view */ ,
                const std::vector<NXOpen::TaggedObject *> & selectedPmisTag /** selectedpmistag */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX2007.0.0.  <br>  
             <br> License requirements : None */
            public: void LoadComponentPmi
            (
            );

            /// \endcond 
            /** Creates a @link NXOpen::Annotations::ConvertPmiBuilder NXOpen::Annotations::ConvertPmiBuilder@endlink  
             <br>  Created in NX2007.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: NXOpen::Annotations::ConvertPmiBuilder * CreateConvertPmiBuilder
            (
            );

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX2007.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void SetComponentPmiDisplay
            (
                const std::vector<NXOpen::Assemblies::Component *> & selectedComponents /** selectedcomponents */ ,
                bool doDisplay /** dodisplay */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX2007.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: NXOpen::Annotations::PmiManager::ComponentPmiDisplayStatus GetComponentPmiDisplayStatus
            (
                NXOpen::Assemblies::Component * component /** component */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX2007.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void ResetComponentPmiDisplayStatus
            (
                const std::vector<NXOpen::Assemblies::Component *> & selectedComponents /** selectedcomponents */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void GetAllPmiOccurrences
            (
                std::vector<NXOpen::INXObject *> & pmiOccs /** pmioccs */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : geometric_tol ("GDT") */
            public: void Orient
            (
                NXOpen::Annotations::Annotation * displayInstance /** PMI Display Instance */
            );

            /// \endcond 
            /** Returns whether or not the given annotation is a PMI Display Instance.  @return  
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : None */
            public: bool IsDisplayInstance
            (
                NXOpen::Annotations::Annotation * annotation /** Annotation */
            );
            /** Returns whether model view is having any PMI or not @return   <br>  True  - Model view is having PMI  <br> 
                                                        <br>  False - Model view is not having any PMI  <br>  
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : None */
            public: bool HasPmiInView
            (
            );


            /** Pmi collection 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Annotations::PmiCollection *Pmis();
            /** PmiAttribute collection 
             <br>  Created in NX3.0.0.  <br>  
            */
            public: NXOpen::Annotations::PmiAttributeCollection *PmiAttributes();
            /** Region collection 
             <br>  Created in NX6.0.2.  <br>  
            */
            public: NXOpen::Annotations::PmiRegionCollection *PmiRegions();
            /** PMI search view collection 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Annotations::SearchModelViewCollection *SearchModelViews();
            /** Lightweight Section View collection 
             <br>  Created in NX7.5.0.  <br>  
            */
            public: NXOpen::Annotations::PmiLightweightSectionCollection *PmiLightweightSections();
            /** WAVE collection 
             <br>  Created in NX8.0.0.  <br>  
            */
            public: NXOpen::Annotations::PmiWaveCollection *PmiWave();
        }; //lint !e1712 default constructor not defined for class  

    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
