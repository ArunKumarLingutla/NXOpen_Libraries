#ifndef NXOpen_DESIGNSIMULATION_VIEWTABULARRESULTBUILDER_HXX_INCLUDED
#define NXOpen_DESIGNSIMULATION_VIEWTABULARRESULTBUILDER_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     DesignSimulation_ViewTabularResultBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/DesignSimulation_Study.hxx>
#include <NXOpen/Builder.hxx>
#include <NXOpen/SelectObjectList.hxx>
#include <NXOpen/libnxopencpp_designsimulation_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace DesignSimulation
    {
        class ViewTabularResultBuilder;
    }
    class Builder;
    namespace DesignSimulation
    {
        class Study;
    }
    class SelectTaggedObjectList;
    namespace DesignSimulation
    {
        class _ViewTabularResultBuilderBuilder;
        class ViewTabularResultBuilderImpl;
        /**
            Represents a builder to specify tabular result contents and list tabular results according to the contents
             <br> To create a new instance of this class, use @link NXOpen::DesignSimulation::PostManager::CreateViewTabularResultBuilder  NXOpen::DesignSimulation::PostManager::CreateViewTabularResultBuilder @endlink  <br> 
         <br>  Created in NX2206.0.0.  <br>  
        */
        class NXOPENCPP_DESIGNSIMULATIONEXPORT  ViewTabularResultBuilder : public NXOpen::Builder
        {
            /** Report contents option */
            public: enum ReportContentOption
            {
                ReportContentOptionOptimizationObjective/** optimization objective */ ,
                ReportContentOptionOptimizationConstraint/** optimization constraint */ ,
                ReportContentOptionAll/** all */ 
            };

            private: ViewTabularResultBuilderImpl * m_viewtabularresultbuilder_impl;
            private: friend class  _ViewTabularResultBuilderBuilder;
            protected: ViewTabularResultBuilder();
            public: ~ViewTabularResultBuilder();
            /** Gets the selected studies. 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : None */
            public: std::vector<NXOpen::DesignSimulation::Study *> GetStudies
            (
            );
            /** Sets the selected studies. 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : des_top_opt ("Topology Optimization for Designers") OR sc_des_topol_opt (" Topology Optimization for Designers") */
            public: void SetStudies
            (
                const std::vector<NXOpen::DesignSimulation::Study *> & studies /** studies */ 
            );
            /**Returns  the report content choice 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::DesignSimulation::ViewTabularResultBuilder::ReportContentOption ReportContent
            (
            );
            /**Sets  the report content choice 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : des_top_opt ("Topology Optimization for Designers") OR sc_des_topol_opt (" Topology Optimization for Designers") */
            public: void SetReportContent
            (
                NXOpen::DesignSimulation::ViewTabularResultBuilder::ReportContentOption reportContent /** reportcontent */ 
            );
            /**Returns  the selected solid bodies for scenery bodies or design spaces 
             <br>  Created in NX2206.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::SelectTaggedObjectList * Bodies
            (
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
