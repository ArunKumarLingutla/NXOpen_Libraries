#ifndef NXOpen_FEATURES_AOCSBUILDER_HXX_INCLUDED
#define NXOpen_FEATURES_AOCSBUILDER_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Features_AOCSBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/GeometricUtilities_CurveFitData.hxx>
#include <NXOpen/GeometricUtilities_CurveFitJoin.hxx>
#include <NXOpen/GeometricUtilities_LawBuilder.hxx>
#include <NXOpen/Features_FeatureBuilder.hxx>
#include <NXOpen/libnxopencpp_features_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace Features
    {
        class AOCSBuilder;
    }
    class Direction;
    class Expression;
    class ExpressionSectionSetList;
    namespace Features
    {
        class FeatureBuilder;
    }
    namespace GeometricUtilities
    {
        class CurveFitData;
    }
    namespace GeometricUtilities
    {
        class CurveFitJoin;
    }
    namespace GeometricUtilities
    {
        class LawBuilder;
    }
    class NXObject;
    class Point;
    class ScCollector;
    class Section;
    namespace Features
    {
        class _AOCSBuilderBuilder;
        class AOCSBuilderImpl;
        /** Represents a Offset In Face(aka Associative offset curve on face)feature builder.
             <br> To create a new instance of this class, use @link NXOpen::Features::FeatureCollection::CreateAocsBuilder  NXOpen::Features::FeatureCollection::CreateAocsBuilder @endlink  <br> 
        Default values.
        <list type="table"> 
         
        <listheader><term> 
        Property</term>  <description> 
        Value</description> </listheader>
        <item><term> 
         
        CurveFitData.CurveJoinMethod </term> <description> 
         
        No </description> </item> 

        <item><term> 
         
        CurveFitData.Degree </term> <description> 
         
        7 </description> </item> 

        <item><term> 
         
        CurveFitData.FitMethod </term> <description> 
         
        DegreeAndSegments </description> </item> 

        <item><term> 
         
        CurveFitData.IsAdvancedFit </term> <description> 
         
        0 </description> </item> 

        <item><term> 
         
        CurveFitData.IsAlignShape </term> <description> 
         
        0 </description> </item> 

        <item><term> 
         
        CurveFitData.MaximumDegree </term> <description> 
         
        7 </description> </item> 

        <item><term> 
         
        CurveFitData.MaximumSegments </term> <description> 
         
        1 </description> </item> 

        <item><term> 
         
        CurveFitData.MinimumDegree </term> <description> 
         
        5 </description> </item> 

        <item><term> 
         
        CurveFitData.Segments </term> <description> 
         
        1 </description> </item> 

        <item><term> 
         
        CurveFitJoinData.CurveFitOptions.FitOption (deprecated) </term> <description> 
         
        Cubic </description> </item> 

        <item><term> 
         
        CurveFitJoinData.CurveFitOptions.MaximumDegree (deprecated) </term> <description> 
         
        7 </description> </item> 

        <item><term> 
         
        CurveFitJoinData.CurveFitOptions.MaximumSegments (deprecated) </term> <description> 
         
        1 </description> </item> 

        <item><term> 
         
        Law.Function </term> <description> 
         
        ft </description> </item> 

        <item><term> 
         
        Law.LawType </term> <description> 
         
        Constant </description> </item> 

        <item><term> 
         
        Law.Parameter </term> <description> 
         
        t </description> </item> 

        </list> 

         <br>  Created in NX4.0.0.  <br>  
        */
        class NXOPENCPP_FEATURESEXPORT  AOCSBuilder : public NXOpen::Features::FeatureBuilder
        {
            /** AOCS builder offset type enum. */
            public: enum OffsetType
            {
                OffsetTypeChordal/** offset_type_chordal */,
                OffsetTypeArclength/** offset_type_arclength */,
                OffsetTypeGeodesic/** offset_type_geodesic */,
                OffsetTypeTangential/** offset_type_tangential */,
                OffsetTypeProjectdistance/** offset_type_projectdistance */
            };

            /** AOCS builder trim to each other within section type enum. */
            public: enum Trim
            {
                TrimNone/** no trimming */,
                TrimWithinSection/** allow trimming */
            };

            /** AOCS builder extend to each other within section type enum. */
            public: enum Extend
            {
                ExtendNone/** no extension */,
                ExtendWithinSection/** allow extension */
            };

            /** AOCS builder extend to face edges type enum. */
            public: enum ExtendToFace
            {
                ExtendToFaceNone/** no extentsion */,
                ExtendToFaceBoundary/** extend till face boundary */
            };

            /** AOCS builder offset distance type enum. */
            public: enum OffsetDistanceType
            {
                OffsetDistanceTypeConstant/** constant offset*/,
                OffsetDistanceTypeVariable/** variable offset */
            };

            /** AOCS builder offset direction type enum. */
            public: enum OffsetDirection
            {
                OffsetDirectionNormalToCurve/** traditional curve normal*/,
                OffsetDirectionNormalToVector/** offset pependicular to vector */
            };

            /** AOCS builder   fillet options type enum. */
            public: enum FilletOptions
            {
                FilletOptionsNoFillet/** No fillet*/,
                FilletOptionsVector/**  Vector given*/,
                FilletOptionsBestFit/** Best fitting*/,
                FilletOptionsProjectedVector/** Use Project Vector to fillet*/
            };

            /** AOCS builder curve offset type enum*/
            public: enum CurveOffsetTypes
            {
                CurveOffsetTypesValue/** Value*/,
                CurveOffsetTypesThroughPoint/** Through Point*/
            };

            private: AOCSBuilderImpl * m_aocsbuilder_impl;
            private: friend class  _AOCSBuilderBuilder;
            protected: AOCSBuilder();
            public: ~AOCSBuilder();
            /**Returns  the face collector. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::ScCollector * FaceCollector
            (
            );
            /**Sets  the face collector. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetFaceCollector
            (
                NXOpen::ScCollector * collector /** collector */ 
            );
            /**Returns  the list of offsets. 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::ExpressionSectionSetList * Offsets
            (
            );
            /**Returns  the offset mode for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Features::AOCSBuilder::OffsetType OffsetMode
            (
            );
            /**Sets  the offset mode for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetOffsetMode
            (
                NXOpen::Features::AOCSBuilder::OffsetType offsetMode /** offset mode */ 
            );
            /**Returns  the trim method option for the AOCS operation. 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Features::AOCSBuilder::Trim TrimMethod
            (
            );
            /**Sets  the trim method option for the AOCS operation. 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetTrimMethod
            (
                NXOpen::Features::AOCSBuilder::Trim trimMethod /** trim method */ 
            );
            /**Returns  the extend method option for the AOCS operation. 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Features::AOCSBuilder::Extend ExtendMethod
            (
            );
            /**Sets  the extend method option for the AOCS operation. 
             <br>  Created in NX5.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetExtendMethod
            (
                NXOpen::Features::AOCSBuilder::Extend extendMethod /** extend method */ 
            );
            /**Returns  the cross boundary option for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : None */
            public: bool TrimToFaceEdgesOption
            (
            );
            /**Sets  the cross boundary option for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetTrimToFaceEdgesOption
            (
                bool trimToFaceEdges /** trim to face edges */ 
            );
            /**Returns  the span option for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Features::AOCSBuilder::ExtendToFace ExtendToFaceOption
            (
            );
            /**Sets  the span option for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetExtendToFaceOption
            (
                NXOpen::Features::AOCSBuilder::ExtendToFace extendToFaceOption /** extend to face option */ 
            );
            /**Returns  the distance tolerance for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : None */
            public: double DistanceTolerance
            (
            );
            /**Sets  the distance tolerance for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetDistanceTolerance
            (
                double distanceTolerance /** distance tolerance */ 
            );
            /**Returns  the angular tolerance for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : None */
            public: double AngularTolerance
            (
            );
            /**Sets  the angular tolerance for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetAngularTolerance
            (
                double angularTolerance /** angular tolerance */ 
            );
            /**Returns  the associative output option for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : None */
            public: bool AssociativeOutputOption
            (
            );
            /**Sets  the associative output option for the AOCS operation. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetAssociativeOutputOption
            (
                bool associative /** associative */ 
            );
            /** Reverse the direction of all offsets of the section. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void FlipAllOffsetsOfSection
            (
                NXOpen::Section * section /** input section */
            );
            /** Reverse the offset direction. 
             <br>  Created in NX4.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void FlipOffset
            (
                NXOpen::NXObject * seldimitem /** input list item */
            );
            /**Sets  the offset distance type 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetOffsetDistType
            (
                NXOpen::Features::AOCSBuilder::OffsetDistanceType type /** type */ 
            );
            /**Returns  the offset distance type 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Features::AOCSBuilder::OffsetDistanceType OffsetDistType
            (
            );
            /**Sets  the law string
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetLawString
            (
                NXOpen::Section * lawString /** law string */ 
            );
            /**Returns  the law string
             <br>  Created in NX7.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Section * LawString
            (
            );
            /**Sets  the law string flip flag 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetLawStringFlip
            (
                bool flip /** flip */ 
            );
            /**Returns  the law string flip flag 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: bool LawStringFlip
            (
            );
            /**Returns  the  law.
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::GeometricUtilities::LawBuilder * Law
            (
            );
            /**Sets  the offset direction vector
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetOffsetDirectionOption
            (
                NXOpen::Features::AOCSBuilder::OffsetDirection direction /** direction */ 
            );
            /**Returns  the offset direction vector
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Features::AOCSBuilder::OffsetDirection OffsetDirectionOption
            (
            );
            /**Sets  the offset diection vector 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetDirectionVector
            (
                NXOpen::Direction * offsetDirection /** offset direction */ 
            );
            /**Returns  the offset diection vector 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Direction * DirectionVector
            (
            );
            /**Sets  the fillet option 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetFilletOption
            (
                NXOpen::Features::AOCSBuilder::FilletOptions options /** options */ 
            );
            /**Returns  the fillet option 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Features::AOCSBuilder::FilletOptions FilletOption
            (
            );
            /**Sets  the fillet vector 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetFilletDirectionVector
            (
                NXOpen::Direction * tipDirection /** tip direction */ 
            );
            /**Returns  the fillet vector 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Direction * FilletDirectionVector
            (
            );
            /**Sets  the project plane vector 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetProjectPlaneNormal
            (
                NXOpen::Direction * projectPlaneNormal /** project plane normal */ 
            );
            /**Returns  the project plane vector 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Direction * ProjectPlaneNormal
            (
            );
            /**Sets  the fillet radius 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetFilletRadius
            (
                NXOpen::Expression * radius /** radius */ 
            );
            /**Returns  the fillet radius 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Expression * FilletRadius
            (
            );
            /** Update the post-process status flag.
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void UpdatePostStatusFlag
            (
                bool postStatusChanged /** whether the post status changed*/
            );
            /** Update result curve status. 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void UpdateCurvesStatus
            (
                bool updateLawParm /** whether to update law parm */
            );
            /**Returns  the remove self intersections flag . 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : None */
            public: bool RemoveSelfIntersections
            (
            );
            /**Sets  the remove self intersections flag . 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetRemoveSelfIntersections
            (
                bool removeSelfIntersections /** remove self intersections */ 
            );
            /**Returns  the split curve at face boundary option . 
             <br>  Created in NX10.0.0.  <br>  
             <br> License requirements : None */
            public: bool SplitCurveOption
            (
            );
            /**Sets  the split curve at face boundary option . 
             <br>  Created in NX10.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") */
            public: void SetSplitCurveOption
            (
                bool splitCurveOption /** split curve option */ 
            );
            /**Returns  the curve fit join method 
             <br>  @deprecated Deprecated in NX10.0.0.  Use @link Features::AOCSBuilder::CurveFitData Features::AOCSBuilder::CurveFitData@endlink  instead <br>  

             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NX_DEPRECATED("Deprecated in NX10.0.0.  Use Features::AOCSBuilder::CurveFitData instead") NXOpen::GeometricUtilities::CurveFitJoin * CurveFitJoinData
            (
            );
            /**Returns  the curve fit settings 
             <br>  Created in NX10.0.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") */
            public: NXOpen::GeometricUtilities::CurveFitData * CurveFitData
            (
            );
            /** Clone section. 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Section * CloneSection
            (
                bool constToVariable /** whether the type change is from constant to variable */,
                NXOpen::Section * sectionFrom /**  section to be cloned*/
            );
            /** Synchronize the second section data  consistent  to the first one 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SynchronizeSections
            (
                bool constToVariable /** whether the type change is from constant to variable */,
                NXOpen::Section * firstSection /**  section  as input*/,
                NXOpen::Section * secondSection /** section to be st=ynchronized*/
            );
            /** Update face. 
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void UpdateFaces
            (
            );
            /** Update section data for offset @return  flag indicating if offset direction flipped
             <br>  Created in NX7.5.0.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: bool UpdateSectionData
            (
                NXOpen::Section * section /**  section  as input*/
            );
            /**Returns  the curve offset type
             <br>  Created in NX11.0.1.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Features::AOCSBuilder::CurveOffsetTypes CurveOffsetType
            (
            );
            /**Sets  the curve offset type
             <br>  Created in NX11.0.1.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetCurveOffsetType
            (
                NXOpen::Features::AOCSBuilder::CurveOffsetTypes type /** type */ 
            );
            /**Returns  the point string
             <br>  Created in NX11.0.1.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Section * PointString
            (
            );
            /**Sets  the point string
             <br>  Created in NX11.0.1.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetPointString
            (
                NXOpen::Section * pointString /** pointstring */ 
            );
            /**Returns  the through point
             <br>  Created in NX11.0.1.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: NXOpen::Point * ThroughPoint
            (
            );
            /**Sets  the through point
             <br>  Created in NX11.0.1.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SetThroughPoint
            (
                NXOpen::Point * throughPoint /** throughpoint */ 
            );
            /**  @brief 
                    Update the section data of Value or Through-Point type.  

             
                    
                    Call this method when it is really needed to copy section between value and point.
                    
                     
             <br>  Created in NX11.0.1.  <br>  
             <br> License requirements : solid_modeling ("SOLIDS MODELING") OR insp_programming ("INSPECTION PROGRAMMING") */
            public: void SynchronizeSectionForValueOrPoint
            (
                bool pointType /** value or point type*/
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
