    ...
    namespace CAE
    {
        ...
        */
        class NXOPENCPP_CAEEXPORT  PostScenarioVariableDomain : public NXOpen::TransientObject
        {
            ...
            /**  @brief Check if this VariableDomain matches a domain pattern that may contain wildcards. 

             
                        
                         <br> This is useful for detecting rational measures such as Response Functions, Power Spectral Density Functions and so on. <br> 
                         <br> The comparison works as such:<ul>
                            <li>It checks each base domain in <tt>domain_pattern</tt> is present in this value</li>
                            <li>For each base domain, it checks that all subdomains listed in <tt>domain_pattern</tt> are present in this value</li>
                            <li>A domain of <tt>rational_support { wildcard }</tt> is matched to anything</li>
                            <li>If the first domain in <tt>domain_pattern</tt> is <tt>rational_support { not }</tt>, then the comparison will be reversed for the remaining domains</li>
                            <li>Base domains are checked in order</li>
                            <li>Subdomains are compared via set inclusion, so order does not matter</li>
                           </ul>
                          <br> 
             ...
             <br> License requirements : None */
            public: bool MatchDomainPattern
            (
                NXOpen::CAE::PostScenarioVariableDomain * domainPattern /** domain pattern */ 
            );
