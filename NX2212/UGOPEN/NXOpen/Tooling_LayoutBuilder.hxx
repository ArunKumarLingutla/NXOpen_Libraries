#ifndef NXOpen_TOOLING_LAYOUTBUILDER_HXX_INCLUDED
#define NXOpen_TOOLING_LAYOUTBUILDER_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     Tooling_LayoutBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Expression.hxx>
#include <NXOpen/Builder.hxx>
#include <NXOpen/SelectObjectList.hxx>
#include <NXOpen/libnxopencpp_tooling_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace Tooling
    {
        class LayoutBuilder;
    }
    namespace Assemblies
    {
        class Component;
    }
    class Builder;
    class Direction;
    class Expression;
    class Point;
    class SelectNXObjectList;
    namespace Tooling
    {
        class _LayoutBuilderBuilder;
        class LayoutBuilderImpl;
        /** Represents a Tooling::LayoutBuilder builder  <br> To create a new instance of this class, use @link NXOpen::Tooling::MWLayoutCollection::CreateLayoutBuilder  NXOpen::Tooling::MWLayoutCollection::CreateLayoutBuilder @endlink  <br> 
        Default values.
        <list type="table"> 
         
        <listheader><term> 
        Property</term>  <description> 
        Value</description> </listheader>
        <item><term> 
         
        CirCavityNumber </term> <description> 
         
        2 </description> </item> 

        <item><term> 
         
        CirRadius </term> <description> 
         
        0 </description> </item> 

        <item><term> 
         
        CirRotateAngle </term> <description> 
         
        360 </description> </item> 

        <item><term> 
         
        CirStartAngle </term> <description> 
         
        0 </description> </item> 

        <item><term> 
         
        DimBalFirst.Value </term> <description> 
         
        0 (millimeters part), 0 (inches part) </description> </item> 

        <item><term> 
         
        LinXnumber </term> <description> 
         
        2 </description> </item> 

        <item><term> 
         
        LinYnumber </term> <description> 
         
        1 </description> </item> 

        </list> 

         <br>  Created in NX6.0.0.  <br>  
        */
        class NXOPENCPP_TOOLINGEXPORT  LayoutBuilder : public NXOpen::Builder
        {
            /** Represents the type of layout. */
            public: enum LayoutType
            {
                LayoutTypeRectangle/** Rectangle layout */,
                LayoutTypeCircular/** Circle layout */,
                LayoutTypeSmart/** smart */ 
            };

            /** Represents the the sub type of layout. */
            public: enum LayoutSubType
            {
                LayoutSubTypeBalance/** Balance type */,
                LayoutSubTypeLinear/** Linear type */,
                LayoutSubTypeRadial/** Radial circular type */,
                LayoutSubTypeConstant/** Constanct circular type */
            };

            /** Represents the number of balance cavity. */
            public: enum BalNumber
            {
                BalNumberTwo/** Two balance cavities */,
                BalNumberFour/** Four balance cavities */
            };

            /** Represents the type of x move refer. */
            public: enum XrefNum
            {
                XrefNumBlock/** Block type */,
                XrefNumMove/** Move type */
            };

            /** Represents the type of y move refer. */
            public: enum YrefNum
            {
                YrefNumBlock/** Block type */,
                YrefNumMove/** Move type */
            };

            private: LayoutBuilderImpl * m_layoutbuilder_impl;
            private: friend class  _LayoutBuilderBuilder;
            protected: LayoutBuilder();
            public: ~LayoutBuilder();
            /**Returns  the select cavity 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::SelectNXObjectList * SelectCavity
            (
            );
            /**Returns  the layout pattern type 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Tooling::LayoutBuilder::LayoutType MwLayoutType
            (
            );
            /**Sets  the layout pattern type 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetMwLayoutType
            (
                NXOpen::Tooling::LayoutBuilder::LayoutType enumLayoutType /** enumlayouttype */ 
            );
            /**Returns  the layout transform type 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Tooling::LayoutBuilder::LayoutSubType SubType
            (
            );
            /**Sets  the layout transform type 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetSubType
            (
                NXOpen::Tooling::LayoutBuilder::LayoutSubType enumSubType /** enumsubtype */ 
            );
            /**Returns  the rotate point 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Point * RotatePoint
            (
            );
            /**Sets  the rotate point 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetRotatePoint
            (
                NXOpen::Point * rotatePoint /** rotatepoint */ 
            );
            /**Returns  the rotate vector 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Direction * LayoutVector
            (
            );
            /**Sets  the rotate vector 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetLayoutVector
            (
                NXOpen::Direction * layoutVector /** layoutvector */ 
            );
            /**Returns  the balance cavity number 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Tooling::LayoutBuilder::BalNumber BalCavityNumber
            (
            );
            /**Sets  the balance cavity number 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetBalCavityNumber
            (
                NXOpen::Tooling::LayoutBuilder::BalNumber enumBalCavityNumber /** enumbalcavitynumber */ 
            );
            /**Returns  the first dimension for balance transform 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Expression * DimBalFirst
            (
            );
            /**Returns  the second dimension for balance transform 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Expression * DimBalSecond
            (
            );
            /**Returns  the copy number for linear transform in X-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: int LinXnumber
            (
            );
            /**Sets  the copy number for linear transform in X-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetLinXnumber
            (
                int linXNumber /** linxnumber */ 
            );
            /**Returns  the layout cavity number in X-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Tooling::LayoutBuilder::XrefNum Xref
            (
            );
            /**Sets  the layout cavity number in X-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetXref
            (
                NXOpen::Tooling::LayoutBuilder::XrefNum enumXRef /** enumxref */ 
            );
            /**Returns  the cavity distance dimension in X-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Expression * DimXdist
            (
            );
            /**Returns  the copy number for linear transform in Y-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: int LinYnumber
            (
            );
            /**Sets  the copy number for linear transform in Y-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetLinYnumber
            (
                int linYNumber /** linynumber */ 
            );
            /**Returns  the layout cavity number in Y-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Tooling::LayoutBuilder::YrefNum Yref
            (
            );
            /**Sets  the layout cavity number in Y-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetYref
            (
                NXOpen::Tooling::LayoutBuilder::YrefNum enumYRef /** enumyref */ 
            );
            /**Returns  the cavity distance dimension in Y-axis 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::Expression * DimYdist
            (
            );
            /**Returns  the copy number for circular transform 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: int CirCavityNumber
            (
            );
            /**Sets  the copy number for circular transform 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetCirCavityNumber
            (
                int cirCavityNumber /** circavitynumber */ 
            );
            /**Returns  the start angle 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: double CirStartAngle
            (
            );
            /**Sets  the start angle 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetCirStartAngle
            (
                double cirStartAngle /** cirstartangle */ 
            );
            /**Returns  the rotate angle 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: double CirRotateAngle
            (
            );
            /**Sets  the rotate angle 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetCirRotateAngle
            (
                double cirRotateAngle /** cirrotateangle */ 
            );
            /**Returns  the radius value 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: double CirRadius
            (
            );
            /**Sets  the radius value 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void SetCirRadius
            (
                double cirRadius /** cirradius */ 
            );
            /** Perform cavity layout 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void Layout
            (
            );
            /** Insert pocket 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void Insertpocket
            (
            );
            /** Reposition cavity 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void Transform
            (
            );
            /** Remove cavity 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void Remove
            (
            );
            /** Make center of cavity 
             <br>  Created in NX6.0.0.  <br>  
             <br> License requirements : None */
            public: void Autocenter
            (
            );
            /** Creates cavity layout
             <br>  Created in NX1872.0.0.  <br>  
             <br> License requirements : ug_mold_design ("UG MOLD") */
            public: void CreateLayout
            (
            );
            /** Puts layout information
             <br>  Created in NX1872.0.0.  <br>  
             <br> License requirements : ug_mold_design ("UG MOLD") */
            public: void PutLayoutInfo
            (
            );
            /** Deletes cavities from layout
             <br>  Created in NX1872.0.0.  <br>  
             <br> License requirements : ug_mold_design ("UG MOLD") */
            public: void RemoveCavitiesFromLayout
            (
            );
            /** Adds a cavity to selected cavities
             <br>  Created in NX1872.0.0.  <br>  
             <br> License requirements : ug_mold_design ("UG MOLD") */
            public: void AddACavity
            (
                NXOpen::Assemblies::Component * aCavity /**selected cavity*/
            );
            /** Removes a cavity from selected cavities
             <br>  Created in NX1872.0.0.  <br>  
             <br> License requirements : ug_mold_design ("UG MOLD") */
            public: void RemoveACavity
            (
                NXOpen::Assemblies::Component * aCavity /**removed cavity*/
            );
            /** Clears all cavities in the selected cavities
             <br>  Created in NX1872.0.0.  <br>  
             <br> License requirements : ug_mold_design ("UG MOLD") */
            public: void ClearSelectedCavities
            (
            );
            /** Gets cavity number along X direction
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : None */
            public: int GetCavityNumberAlongX
            (
            );
            /** Sets cavity number along X direction
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : ug_mold_design ("UG MOLD") */
            public: void SetCavityNumberAlongX
            (
                int cavityNumber /** cavitynumber */ 
            );
            /** Gets cavity number along Y direction
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : None */
            public: int GetCavityNumberAlongY
            (
            );
            /** Sets cavity number along Y direction
             <br>  Created in NX2212.0.0.  <br>  
             <br> License requirements : ug_mold_design ("UG MOLD") */
            public: void SetCavityNumberAlongY
            (
                int cavityNumber /** cavitynumber */ 
            );
        };
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
