#ifndef NXOpen_CLOUDDM_NEWPARTBUILDER_HXX_INCLUDED
#define NXOpen_CLOUDDM_NEWPARTBUILDER_HXX_INCLUDED
//--------------------------------------------------------------------------
// Copyright 2023 Siemens
//--------------------------------------------------------------------------
//  Header for C++ interface to JA API
//--------------------------------------------------------------------------
//
//  Source File:
//     CloudDM_NewPartBuilder.ja
//
//  Generated by:
//     apiwrap
//
//  WARNING:
//    This file is automatically generated - do not edit by hand
//
#ifdef _MSC_VER
#pragma once
#endif

#include <NXOpen/NXDeprecation.hxx>
#include <vector>
#include <NXOpen/NXString.hxx>
#include <NXOpen/Callback.hxx>
#include <NXOpen/Builder.hxx>
#include <NXOpen/libnxopencpp_clouddm_exports.hxx>

#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable:4996)
#endif
#ifdef __GNUC__
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

namespace NXOpen
{
    namespace CloudDM
    {
        class NewPartBuilder;
    }
    class Builder;
    class NXObject;
    namespace CloudDM
    {
        class _NewPartBuilderBuilder;
        class NewPartBuilderImpl;

        /// \cond NX_NO_DOC 
        /** A builder that holds specific parameters for a part being
                created and to be managed in Cloud  <br> To create a new instance of this class, use @link NXOpen::PartCollection::CreateCloudDMNewPartBuilder  NXOpen::PartCollection::CreateCloudDMNewPartBuilder @endlink  <br> 
         <br>  Created in NX1899.0.0.  <br>  
        */
        class NXOPENCPP_CLOUDDMEXPORT  NewPartBuilder : public NXOpen::Builder
        {

            /// \cond NX_NO_DOC 
             /** This is an internal API and can be changed at any time */
            public:
            struct TemplatePartContextinfo
            {
                public: /** Specifies CollaborationSpace ID */NXString CollaborationSpaceId;
                public: /** Specifies Container ID */NXString ContainerId;
                public: /** Specifies Container type */NXString ContainerType;
                public: TemplatePartContextinfo() :
                    CollaborationSpaceId(),
                    ContainerId(),
                    ContainerType()
                {
                }
                /** Constructor for the TemplatePartContextinfo struct. */ 
                public: TemplatePartContextinfo(const NXString &collaborationSpaceIdInitial /** Specifies CollaborationSpace ID */, 
                        const NXString &containerIdInitial /** Specifies Container ID */, 
                        const NXString &containerTypeInitial /** Specifies Container type */) :
                    CollaborationSpaceId(collaborationSpaceIdInitial),
                    ContainerId(containerIdInitial),
                    ContainerType(containerTypeInitial)
                {
                }
            };


            /// \endcond 
            private: NewPartBuilderImpl * m_newpartbuilder_impl;
            private: friend class  _NewPartBuilderBuilder;
            protected: NewPartBuilder();
            public: ~NewPartBuilder();

            /// \cond NX_NO_DOC 
            /**Returns  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXString Project
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void SetProject
            (
                const NXString & project /** project */ 
            );
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            void SetProject
            (
                const char * project /** project */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Returns  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXString Folder
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void SetFolder
            (
                const NXString & folder /** folder */ 
            );
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            void SetFolder
            (
                const char * folder /** folder */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Returns  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXString DesignName
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void SetDesignName
            (
                const NXString & name /** name */ 
            );
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            void SetDesignName
            (
                const char * name /** name */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Returns  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXString DesignNumber
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void SetDesignNumber
            (
                const NXString & number /** number */ 
            );
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            void SetDesignNumber
            (
                const char * number /** number */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void AutoAssignDesignNumber
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Returns  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXString DesignRevision
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void SetDesignRevision
            (
                const NXString & revision /** revision */ 
            );
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            void SetDesignRevision
            (
                const char * revision /** revision */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Returns  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXString DesignDescription
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void SetDesignDescription
            (
                const NXString & description /** description */ 
            );
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            void SetDesignDescription
            (
                const char * description /** description */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Returns  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXOpen::NXObject * AttributeHolder
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void SynchronizeAttributes
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Returns  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: NXString TemplatePart
            (
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void SetTemplatePart
            (
                const NXString & templatePart /** templatepart */ 
            );
            /**Sets  This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            void SetTemplatePart
            (
                const char * templatePart /** templatepart */ 
            );

            /// \endcond 

            /// \cond NX_NO_DOC 
            /** This is an internal API and can be changed at any time 
             <br>  Created in NX1899.0.0.  <br>  
             <br> License requirements : None */
            public: void SetTemplatePartContext
            (
                const NXOpen::CloudDM::NewPartBuilder::TemplatePartContextinfo & templatePartContext /** templatepartcontext */ 
            );

            /// \endcond 
        };

        /// \endcond 
    }
}

#ifdef _MSC_VER
#pragma warning(pop)
#endif
#ifdef __GNUC__
#ifndef NX_NO_GCC_DEPRECATION_WARNINGS
#pragma GCC diagnostic warning "-Wdeprecated-declarations"
#endif
#endif

#undef EXPORTLIBRARY
#endif
