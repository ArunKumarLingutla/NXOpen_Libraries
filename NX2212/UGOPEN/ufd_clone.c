...
#include <schema/PDM_ExportFromTeamcenter.hxx>
...
static void handle_mixed_name_rule(int argc, char** argv);
...
/*   switch name                min   handler function          multi seen
    ...
    {"default_naming",            9,  default_naming,           false, false,
     "=autogen|autotranslate|autotranslate_using_altid|name_rule|user_name|mixed_name_rule"},
...
// <wrkab7> 02-Mar-2022
// PR#10281751: tag to hold export builder object
static UGS::PDM::ExportFromTeamcenter* exportBuilder = nullptr;

// <kalirajan> 08-Jun-2022
// For mixed name rule support in export
static bool isMixedNameRule = false;
/*================================================================================
...
static switch_descriptor_p_t match_switch(const char * switch_spec)
{
    ...
    */
    ...
    while (--i >= 0)
    {
        ...
        {
            // <kalirajan> 09-Jun-2022 TCIN221204
            // For mixed rule there can be more than one switches with "name_rule" option so enable multi flag
            // Example: -n=prepend:PRE_ -n=append:_APP -n=replace:top:NewTop
            if (strcmp(switch_descriptors[i].switch_name, "name_rule") == 0)
                switch_descriptors[i].multi = isMixedNameRule;
...
static void operation(const char * switch_value)
{
    ...
    */
    ...
    if (op == UF_CLONE_export_operation)
    {
        ...
        // <wrkab7> 02-Mar-2022
        // PR#10281751: Create export builder, so that export observers can be invoked through callbacks 
        exportBuilder = UGS::PDM::ExportFromTeamcenterFactory::Create();
    }
}
...
static void assembly(const char * switch_value)
{
    ...
    else
    {
        ...
        if( !is_the_value_dfa( switch_value, UF_KF_UGMGR_EXPORT_DFA_IN_ASSEMBLY ) )
        {
            ...
            // <wrkab7> 02-Mar-2022
            // PR#10281751: If the operation is export, then give callback to observers to add the related parts
            else if(exportBuilder != nullptr)
            {
                exportBuilder->AddApplicationRelatedParts();
            }
        }
    }
}
...
static void default_naming(const char *switch_value)
{
    ...
    else if (is_prefix(switch_value, "mixed_name_rule", 5))
    {
        naming = UF_CLONE_naming_rule;
        isMixedNameRule = true;
    }
    else
...
{
    // <kalirajan> 09-Jun-2022 TCIN221204
    // At this point of time we do not have all the required naming rule strings to process mixed rule so skip it
    // We will process this at the end after all other flags processed
    if (isMixedNameRule)
    {
        return;
    }
...
static void family_treatment(const char* switch_value)
{
    ...
    else if (is_prefix(switch_value, "preserve_status", 0))
        treatment = UF_CLONE_preserve_family_status;
...
extern void UFD_clone_main(int argc, char **argv)
{
    ...
    */
    ...
    ERROR_FINALLY_PROTECT
    {
        ...
        if (isMixedNameRule)
            handle_mixed_name_rule(argc, argv);
        ...
        else
        {
            ...
            */
            ...
            // <wrkab7> 02-Mar-2022
            // PR#10281751: If the operation is export, then give callback to observers to perform precommit tasks
            if (exportBuilder != nullptr)
            {
                // apply default values, so that application knows the final name assigned for the parts to be exported,
                // Earlier apply defaults use to happen from UF_CLONE_perform_clone
                UF_CLONE_apply_defaults(&naming_failures);
                exportBuilder->PerformPreCommit();
            }
            ...
            // <wrkab7> 02-Mar-2022
            // PR#10281751: If the operation is export, then give callback to observers to perform post commit tasks
            if (exportBuilder != nullptr)
            {
                exportBuilder->PerformPostCommit();
            }
            ...
            */
            ...
            // <wrkab7> 02-Mar-2022
            // PR#10281751: Delete the export builder, as it is no longer needed
            if (exportBuilder != nullptr)
            {
                OM_delete_object(exportBuilder);
            }
        }
    }
    ERROR_FINALLY
...
//--------------------------------------------------------------------------------------------------------------
// <kalirajan> 09-Jun-2022 TCIN221204
// Funtion to handle mixed name rule switch
static void handle_mixed_name_rule(int argc, char** argv)
{
    bool isNameRuleProvided{ false };
    UF_CLONE_name_rule_def_t rule_def = { UF_CLONE_mixed,NULL,NULL,NULL,NULL };

    for (int iargIndx = 1; iargIndx < argc; iargIndx++)
    {
        switch_descriptor_p_t descriptor = match_switch(argv[iargIndx]);
        if (strcmp(descriptor->switch_name, "name_rule") == 0)
        {
            isNameRuleProvided = true;
            UGS::UString conversionRuleString = UGS::UString::from_locale(find_value(argv[iargIndx]));
            if (!conversionRuleString.empty())
            {
                UGS::StlUStringVector nameRuleVector;
                conversionRuleString.split(':', nameRuleVector);
                if (!nameRuleVector.empty())
                {
                    UGS::UString typeString = nameRuleVector[0];
                    if (!typeString.empty())
                    {
                        if (is_prefix(LOCALE_ARG(typeString), "prepend", 0))
                        {
                            if (nameRuleVector.size() == 2)
                                rule_def.mixed_rule_prefix_string = nameRuleVector[1].to_locale();
                        }
                        else if (is_prefix(LOCALE_ARG(typeString), "append", 0))
                        {
                            if (nameRuleVector.size() == 2)
                                rule_def.mixed_rule_suffix_string = nameRuleVector[1].to_locale();
                        }
                        else if (is_prefix(LOCALE_ARG(typeString), "replace", 3))
                        {
                            if (nameRuleVector.size() == 3)
                            {
                                rule_def.base_string = nameRuleVector[1].to_locale();
                                rule_def.new_string = nameRuleVector[2].to_locale();
                            }
                        }
                    }
                }
    ...
    {
        UGS::UString errorMsg{ LOCALIZE("MixedNamingRuleError", "\nError: Atleast one -n[ame_rule] switch is required with mixed_name_rule.", "ug_clone") };
        log_and_stderr_printf(const_cast<char*>LOCALE_ARG(errorMsg));
        quit(EXIT_FAILURE);
    }
}
